...
//
define('INSTALLER_VERSION', '3.0.3');
define('INSTALLER_NAME', 'MX-Publisher-IWizard');

//
// These URLs are used in the footer installation panels.
//
define('U_RELEASE_NOTES', 'http://www.mx-publisher.com/phpBB2/viewtopic.php?t=10949');
//define('U_WELCOME_PACK', 'http://www.mx-publisher.com/index.php?page=136');
define('U_ONLINE_MANUAL', 'http://www.mx-publisher.com/docs/manual');
define('U_ONLINE_KB', 'http://www.mx-publisher.com/docs/kb');
define('U_ONLINE_SUPPORT', 'http://www.mx-publisher.com/forum');
define('U_TERMS_OF_USE', 'http://www.mx-publisher.com/toe');
...
if($confirm)
{
	...
	switch($install_mode)
	{
		case 'install':
			...
			if ($mx_request_vars->is_post('mxbb'))
			{
				...
				$phpbb_path = $mx_request_vars->post('phpbb_path', MX_TYPE_NO_TAGS);
				...
				$default_dbms = ($portal_backend === 'internal') ? 'dbms_mxbb' : 'dbms';				
				
				//$phpbb_url = $mx_request_vars->post('phpbb_url', MX_TYPE_NO_TAGS);
				$dbms = $mx_request_vars->post($default_dbms, MX_TYPE_NO_TAGS);
			...
			if ($mx_request_vars->is_post('phpbb'))
			{
				$phpbb_info = get_phpbb_info($mx_root_path . $phpbb_path . "config.$phpEx");

				//$portal_backend = file_exists($mx_root_path . $phpbb_path . "modcp.$phpEx") ? 'phpbb2' : 'phpbb3';
				if( !isset($phpbb_info['dbms']) || $phpbb_info['dbms'] != $dbms || $phpbb_info['dbhost'] != $dbhost ||
					$phpbb_info['dbname'] != $dbname || $phpbb_info['dbuser'] != $dbuser ||
					$phpbb_info['dbpasswd'] != $dbpasswd || $phpbb_info['table_prefix'] != $table_prefix )
				{
	...
	if( count($schemas) > 0 )
	{
		...
		if( count($schemas) > 0 && file_exists($mx_root_path . "includes/mx_functions." . $phpEx) )
		{
		...
		if ($install_mode == 'install')
		{
			...
			switch (PORTAL_BACKEND)
			{
				...
				case 'phpbb2':
					...
					$sql[] = "INSERT INTO " . $mx_table_prefix . "module VALUES('30', 'phpBB2 Blocks', 'modules/mx_phpbb2blocks/', 'MXP Portal phpBB2 blocks', '', 'MX-Publisher Core Module', 'Original MXP <i>phpBB2 Blocks</i> module by <a href=\"http://www.mx-publisher.com\" target=\"_blank\"> The MXP Development Team</a>')";
					$message .= mx_install_cmd_sql($sql) . '<br />';
				break;

				case 'phpbb3':
		
					$sql[] = "INSERT INTO " . $mx_table_prefix . "module VALUES('30', 'phpBB3 Blocks', 'modules/mx_phpbb3blocks/', 'MXP Portal phpBB3 blocks', '', 'MX-Publisher Core Module', 'Original MXP <i>phpBB3 Blocks</i> module by <a href=\"http://www.mx-publisher.com\" target=\"_blank\">OryNider & Culpit</a>')";
					$message .= mx_install_cmd_sql($sql) . '<br />';
				break;
			}			
		}		
...
// That's why we first try to guess the $phpbb_search_path value from $document_root.
//
if( substr($mx_absolute_path, 0, strlen($document_root)) == $document_root )
{
	$phpbb_search_path = $document_root;
}
else
{
	// FYI:
	...
	else
	{
		...
		$phpbb_search_path = $mx_absolute_path;
	}
	//
...
//
$mx_base_path = substr($mx_absolute_path, strlen($phpbb_search_path));
...
//
$debuginfo = array(
	array('mxBB Absolute Path', $mx_absolute_path),
	array('Document Root', $document_root),
	array('phpBB Search Path', $phpbb_search_path),
	array('mxBB Root Path', $mx_base_path),
	array('Website URL', $server_url),
	array('PHP SELF', $_SERVER['PHP_SELF']),
	array('mxBB URL Path', $mx_self_path),
	array('Portal URL', $portal_url),
	array('','')
);

//
// Find the phpBB root path
//
$files_ary = find_phpbb($phpbb_search_path);
...
//
$select_phpbb_path = '<select name="select_phpbb_path" onchange="check_phpbb_path();">';
...
$template->assign_block_vars('datarow', array(
	...
	'PHPBB_URL'		=> '',	
	'DBMS'			=> $mx_info['dbms'] ? $mx_info['dbms'] : '',
...
$select_phpbb_path .= '<option value="-1">'.'internal'.'</option>';
...
for($i = 0; $i < $files_cnt; $i++)
{
	...
	//
	// Get the DB related information for this phpBB
	//
	if (file_exists($phpbb_root_path . "config.$phpEx"))
	{
		$phpbb_info = get_phpbb_info($phpbb_root_path . "config.$phpEx");

		if(!empty($phpbb_info['acm_type']))
		{
	...
	{	
		echo("File: " . $phpbb_root_path . "config.$phpEx" . " not found.");
		$portal_backend = 'internal';
	}	

	//
	// Save phpBB information for debuging purposes
	//
	$debuginfo[] = array('phpBB Info'.( $files_cnt > 1 ? " #$i" : '' ),
	...
	// Get the phpBB URL
	//
	$phpbb_url = get_phpbb_url($phpbb_info['table_prefix'], $portal_backend);
	...
	//
	// Note: to get the phpBB URL we have read the config table,
	// so it seems safe to assume this board is installed. :-)
	//
	$template->assign_block_vars('datarow', array(
		'INFO'			=> $lang['Install_Instruction_phpBB'],
		'PHPBB_PATH'	=> $phpbb_relative,
		'PORTAL_URL'	=> $portal_url,
		'PHPBB_URL'		=> $phpbb_url,
	...
	$select_phpbb_path .= '<option value="'.$i.'">'.$phpbb_relative.'</option>';
	$phpbb_found = true;
}
$select_phpbb_path .= '</select><br />';
...
if ($mx_request_vars->is_post('debug'))
{
	...
	if( $files_cnt <= 0 )
	{
		install_die($lang['Install_phpbb_not_found'], $debuginfo);
	}
	if( $phpbb_failed )
	{
		install_die($lang['Install_phpbb_db_failed'], $debuginfo);
	}
	install_die($lang['Install_phpbb_unsupported'], $debuginfo);
}
// --------------------
...
$template->assign_vars(array(
	...
	'L_Backend_explain'			=> $phpbb_found ? $lang['Session_backend_explain'] : '',
	...
	'S_PHPBB_PATH'				=> $select_phpbb_path,
	'MX_DB_PREFIX'				=> ( !empty($mx_table_prefix) ? $mx_table_prefix : 'mx_' ),

	'PHPBB_PATH'				=> $phpbb_relative,
	'PORTAL_URL'				=> $portal_url,
	'PHPBB_URL'					=> $phpbb_url,
	'ACM_TYPE'					=> ( !empty($phpbb_info['acm_type']) ? $phpbb_info['acm_type'] : 'file' ),
	
	'DBMS_SELECT'				=> dbms_select('mysql4'),	
