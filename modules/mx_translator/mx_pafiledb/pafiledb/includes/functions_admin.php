...
class pafiledb_admin extends pafiledb_public
{
	...
	function adminmodule( $module_name )
	{
		if ( !class_exists( 'pafiledb_' . $module_name ) )
		{
			...
			require_once( $module_root_path . 'pafiledb/admin/admin_' . $module_name . '.' . $phpEx );
		...
		if ( isset( $this->subcat_rowset[$cat_parent] ) )
		{
			foreach( $this->subcat_rowset[$cat_parent] as $sub_cat_id => $cat_data )
			{
				$template->assign_block_vars( 'cat_row', array(
					...
					'PRE' => $pre,
					'U_CAT' => mx_append_sid( "admin_pafiledb.$phpEx?action=catauth_manage&cat_parent=$sub_cat_id" )
				));
		...
		if ( isset( $this->subcat_rowset[$cat_parent] ) )
		{
			foreach( $this->subcat_rowset[$cat_parent] as $sub_cat_id => $cat_data )
			{
				$template->assign_block_vars( 'cat_row', array(
					...
					'IS_HIGHER_CAT' => ( $cat_data['cat_allow_file'] ) ? false : true,
					'PRE' => $pre,
					'U_CAT' => mx_append_sid( "admin_pafiledb.$phpEx?action=catauth&cat_id=$sub_cat_id" ),
		...
		if ( isset( $this->subcat_rowset[$cat_parent] ) )
		{
			foreach( $this->subcat_rowset[$cat_parent] as $subcat_id => $cat_data )
			{
				$template->assign_block_vars( 'cat_row', array(
					...
					'U_CAT' => mx_append_sid( "admin_pafiledb.$phpEx?action=cat_manage&cat_id=$subcat_id" ),
					'U_CAT_EDIT' => mx_append_sid( "admin_pafiledb.$phpEx?action=cat_manage&mode=edit&amp;cat_id=$subcat_id" ),
					'U_CAT_DELETE' => mx_append_sid( "admin_pafiledb.$phpEx?action=cat_manage&mode=delete&amp;cat_id=$subcat_id" ),
					'U_CAT_MOVE_UP' => mx_append_sid( "admin_pafiledb.$phpEx?action=cat_manage&mode=cat_order&amp;move=-15&amp;cat_id_other=$subcat_id" ),
					'U_CAT_MOVE_DOWN' => mx_append_sid( "admin_pafiledb.$phpEx?action=cat_manage&mode=cat_order&amp;move=15&amp;cat_id_other=$subcat_id" ),
					'U_CAT_RESYNC' => mx_append_sid( "admin_pafiledb.$phpEx?action=cat_manage&mode=sync&amp;cat_id_other=$subcat_id" ),
					'CAT_NAME' => $cat_data['cat_name'],
					'PRE' => $pre
				));
