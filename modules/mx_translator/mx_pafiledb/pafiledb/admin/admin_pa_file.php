...
* @version $Id: admin_pa_file.php,v 1.17 2005/12/11 16:19:20 jonohlsson Exp $
...
class pafiledb_file_manage extends pafiledb_admin
{
	function main( $module_id = false )
	{
		...
		elseif ( $mode == 'do_add' && $file_id )
		{
			...
			if ( !$mirrors )
			{
				$message = $lang['Fileedited'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( "admin_pa_file.$phpEx" ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( $mx_root_path . "admin/index.$phpEx?pane=right" ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
		}
		elseif ( $mode == 'delete' )
		{
			if ( is_array( $file_ids ) && !empty( $file_ids ) )
			{
				...
				{
					$this->delete_files( $temp_file_id );
				}
			}
			else
			{
		...
		$template->assign_vars( array( 'L_FILE_TITLE' => $l_title,
		...
		if ( in_array( $mode, array( '', 'approved', 'broken', 'do_approve', 'do_unapprove', 'delete', 'file_cat', 'all_file' ) ) )
		{
			...
			if ( $mode == '' || $mode == 'approved' || $mode == 'broken' || $mode == 'file_cat' || $mode == 'all_file' )
			{
				...
				if ( $mode == '' || $mode == 'approved' )
				{
					...
					if ( !( $result = $db->sql_query_limit( $sql, $limit, $temp_start ) ) )
					{
						mx_message_die( GENERAL_ERROR, 'Couldn\'t get file info', '', __LINE__, __FILE__, $sql );
					}
			...
			if ( !isset($this->cat_rowset[$cat_id]['cat_parent']) )
			{
			...
			$template->assign_vars( array( 'L_EDIT' => $lang['Edit'],
					...
					'PAGINATION' => generate_pagination( mx_append_sid( "admin_pa_file.$phpEx?mode=$mode&amp;sort_method=$sort_method&amp;sort_order=$sort_order&cat_id=$cat_id" ), $total_files, $pafiledb_config['pagination'], $start ),
		...
		elseif ( $mode == 'add' || $mode == 'edit' || $mirrors )
		{
			if ( $mode == 'add' )
			{
				...
				$file_cat_list = $this->generate_jumpbox( 0, 0, '', true );
			...
			$template->assign_vars( array( 'U_MIRRORS_PAGE' => mx_append_sid( "admin_pa_file.$phpEx?mode=mirrors&file_id=$file_id" ),
		...
		elseif ( $mode == 'mirrors' )
		{
			...
			if ( isset( $_POST['add_new'] ) )
			{
				...
				$this->mirror_add_update( $file_id, $file_upload, $file_remote_url, $file_local, $file_realname, $file_size, $file_type, $mirror_location );
			}
			...
			if ( isset( $_POST['modify'] ) )
			{
				...
				foreach( $data as $mirror_id => $mirror_data )
				{
					...
					$this->mirror_add_update( $file_id, $file_upload, $file_remote_url, $file_local, $file_realname, $file_size, $file_type, $mirror_location, $mirror_id );
				}
