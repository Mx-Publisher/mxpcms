...
@define('IN_PORTAL', 1);
$mx_root_path = './../../../';
$module_root_path = $mx_root_path . 'modules/mx_pafiledb/';
...
include($module_root_path . 'pafiledb/includes/pafiledb_common.' . $phpEx);
...
$sort_order = $mx_request_vars->check_var_value($sort_order, array('DESC', 'ASC'), $pafiledb_config['sort_order']);
...
if(($mode == 'do_add') && !$file_id)
{
	...
	if(!$mirrors)
	{
		$message = $lang['Fileadded'] . '<br /><br />' . sprintf($lang['Click_return'], '<a href="' . append_sid('admin_pa_file.' . $phpEx) . '">', '</a>');
...
if(in_array($mode, array('', 'approved', 'broken', 'do_approve', 'do_unapprove', 'delete', 'file_cat', 'all_file')))
{
	...
	if($mode != 'approved' && $mode != 'broken')
	{
		...
		if (!($result = $db->sql_query_limit($sql, $pafiledb_config['settings_file_page'], $start)))
		{
...
elseif($mode == 'mirrors')
{
	...
	{
		$mirror_ids = request_var('mirror_ids', array(0));
		if(!empty($mirror_ids))
		{
	...
	if(isset($_POST['add_new']))
	{
		...
		$file_remote_url = request_var('new_download_url', '');
		...
		$mirror_location = request_var('new_location', '');
	...
	{
		$file_urls = request_var('download_url', array(''));
		$userfiles = (!empty($_FILES['userfile'])) ? $_FILES['userfile'] : array();
		$locations = request_var('location', array(''));
...
include($mx_root_path . ADM . '/page_footer_admin.' . $phpEx);
