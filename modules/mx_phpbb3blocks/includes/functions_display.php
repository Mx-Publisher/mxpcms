...
function mx_display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	if( $mark_read == 'forums' )
	{
		if( $mx_userdata['session_logged_in'] )
		{
	...
	if( ( $total_categories = count($category_rows) ) )
	{
		...
		{
			// 60 days limit
			if ($mx_userdata['user_lastvisit'] < (time() - 5184000))
			{
				$mx_userdata['user_lastvisit'] = time() - 5184000;
			}

			$sql = "SELECT t.forum_id, t.topic_id, p.post_time 
				FROM " . TOPICS_TABLE . " t, " . POSTS_TABLE . " p 
				WHERE p.post_id = t.topic_last_post_id 
					AND p.post_time > " . $mx_userdata['user_lastvisit'] . " 
		...
		//
		$is_auth_ary = array();
		$is_auth_ary = auth(AUTH_VIEW, AUTH_LIST_ALL, $mx_userdata, $forum_data);

		// Generate birthday list if required ...
		$show_birthdays = ($phpbb_auth->acl_gets('u_viewprofile', 'a_user', 'a_useradd', 'a_userdel'));
		...
		$template->assign_vars(array(
			'L_STATISTICS' 		=> $mx_user->lang['Statistics'],
			
			'L_LEGEND' 			=> $mx_user->lang['Legend'],			
			'LEGEND'			=> $legend,	
			...
			'BIRTHDAY_LIST'	=> (empty($birthday_list)) ? '' : implode($mx_user->lang['COMMA_SEPARATOR'], $birthday_list),		
			'S_DISPLAY_BIRTHDAY_LIST'	=> $show_birthdays,
			
			'S_IS_LINK'		=> false,		
			
			'L_FORUM'			=> $mx_user->lang['Forum'],
			// Begin Simple Subforums MOD
			'L_SUBFORUMS'	=> $mx_user->lang['Subforums'],
			// End Simple Subforums MOD		
			'L_TOPICS'			=> $mx_user->lang['Topics'],
			'L_REPLIES'		=> $mx_user->lang['Replies'],
			'L_VIEWS'			=> $mx_user->lang['Views'],
			'L_POSTS'			=> $mx_user->lang['Posts'],
			'L_LASTPOST'		=> $mx_user->lang['Last_Post'], 
			'L_NO_NEW_POSTS'			=> $mx_user->lang['No_new_posts'],
			'L_NEW_POSTS'				=> $mx_user->lang['New_posts'],
			'L_NO_NEW_POSTS_LOCKED'	=> $mx_user->lang['No_new_posts_locked'], 
			'L_NEW_POSTS_LOCKED'			=> $mx_user->lang['New_posts_locked'], 
			'L_ONLINE_EXPLAIN'				=> $mx_user->lang['Online_explain'], 

			'L_MODERATOR'					=> $mx_user->lang['Moderators'], 
			'L_FORUM_LOCKED'				=> $mx_user->lang['Forum_is_locked'],
			'L_MARK_FORUMS_READ'		=> $mx_user->lang['Mark_all_forums'],
			
			'U_TEAM'				=> ($mx_user->data['user_id'] != ANONYMOUS) ? '' : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=team'),
			'U_TERMS_USE'		=> append_sid("{$phpbb_root_path}profile.$phpEx?mode=terms"),		
			'U_CANONICAL'		=> generate_board_url() . '/' . append_sid("index.$phpEx"),			
			'U_MARK_FORUMS'	=> ($mx_user->data['user_id'] != ANONYMOUS) ? append_sid("{$phpbb_root_path}index.$phpEx", 'hash=' . generate_link_hash('global') . '&amp;mark=forums&amp;mark_time=' . time()) : '', 
							...
							if ( $is_auth_ary[$forum_id]['auth_view'] )
							{
								...
								$template->assign_block_vars('catrow.forumrow',	array(
									...
									'FORUM_FOLDER_IMG' => $folder_image,
	 								'FORUM_FOLDER_IMG_SRC' 		=> $mx_user->img($folder_image, '', '27', '', 'src'),			
									'FORUM_FOLDER_IMG_FULL_TAG' 	=> $mx_user->img($folder_image, '', '27', '', 'full_tag'),	
									'FORUM_FOLDER_IMG_HTML' 		=> $mx_user->img($folder_image, '', '27', '', 'html'),				
								...
								if ($forum_data[$j]['forum_parent'] == $forum_data[$j]['forum_id'] && $forum_data[$j]['forum_type'] == FORUM_CAT)
								{
									$template->assign_block_vars('forumrow', array(
										...
										'FORUM_IMAGE'			=> ($forum_data[$j]['forum_image']) ? '<img src="' . $phpbb_root_path . $forum_data[$j]['forum_image'] . '" alt="' . $mx_user->lang['FORUM_CAT'] . '" />' : '',
	...
	// Find which forums are visible for this user
	$is_auth_ary = array();
	$is_auth_ary = auth(AUTH_VIEW, AUTH_LIST_ALL, $mx_user->data, $root_data);
	...
	foreach ($forum_rows as $row)
	{
		...
		if (isset($subforums[$forum_id]))
		{
			foreach ($subforums[$forum_id] as $subforum_id => $subforum_row)
			{
				...
				{
					$subforums_list[] = array(
						'link'		=> mx_append_sid(PHPBB_URL . "viewforum.$phpEx", 'f=' . $subforum_id),
			...
			$l_subforums = (sizeof($subforums[$forum_id]) == 1) ? $mx_user->lang['SUBFORUM'] . ': ' : $mx_user->lang['SUBFORUMS'] . ': ';
		...
		if ($row['forum_last_post_id'])
		{
			...
			$last_post_url = mx_append_sid(PHPBB_URL . "viewtopic.$phpEx", 'f=' . $row['forum_id_last_post'] . '&amp;p=' . $row['forum_last_post_id']) . '#p' . $row['forum_last_post_id'];
		}
		else
		{
		...
		{
			$l_moderator = (sizeof($forum_moderators[$forum_id]) == 1) ? $mx_user->lang['MODERATOR'] : $mx_user->lang['MODERATORS'];
		...
		if ($row['forum_type'] != FORUM_LINK)
		{
			$u_viewforum = mx_append_sid(PHPBB_URL . "viewforum.$phpEx", 'f=' . $row['forum_id']);
		}
		else
		{
	...
	// End Simple Subforums MOD
	$template->assign_vars(array(
		'U_MARK_FORUMS'		=> ($mx_user->data['is_registered'] || $board_config['load_anon_lastread']) ? mx_append_sid(PHPBB_URL . "viewforum.$phpEx", 'f=' . $root_data['forum_id'] . '&amp;mark=forums') : '',
...
function mx_generate_forum_nav(&$forum_data)
{
	...
	if (!empty($forum_parents))
	{
		foreach ($forum_parents as $parent_forum_id => $parent_data)
		{
			...
			$template->assign_block_vars('navlinks', array(
				...
				'U_VIEW_FORUM'	=> mx_append_sid(PHPBB_URL . "viewforum.$phpEx", 'f=' . $parent_forum_id))
			);
		}
	}
	...
	$template->assign_block_vars('navlinks', array(
		...
		'U_VIEW_FORUM'	=> mx_append_sid(PHPBB_URL . "viewforum.$phpEx", 'f=' . $forum_data['forum_id']))
	);
...
function mx_get_moderators(&$forum_moderators, $forum_id = false)
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		{
			$forum_moderators[$row['forum_id']][] = '<a' . (($row['group_colour']) ? ' style="color:#' . $row['group_colour'] . ';"' : '') . ' href="' . mx_append_sid(PHPBB_URL . "memberlist.$phpEx", 'mode=group&amp;g=' . $row['group_id']) . '">' . (($row['group_type'] == GROUP_SPECIAL) ? $mx_user->lang['G_' . $row['group_name']] : $row['group_name']) . '</a>';
		}
	}
	$db->sql_freeresult($result);
...
function mx_display_user_activity(&$mx_userdata)
{
	...
	$template->assign_vars(array(
		...
		'U_ACTIVE_FORUM'		=> mx_append_sid(PHPBB_URL . "viewforum.$phpEx", 'f=' . $active_f_id),
		'U_ACTIVE_TOPIC'		=> mx_append_sid(PHPBB_URL . "viewtopic.$phpEx", 't=' . $active_t_id),
...
function mx_watch_topic_forum_old($mode, &$s_watching, &$s_watching_img, $mx_user_id, $forum_id, $topic_id, $notify_status = 'unset', $start = 0)
{
	...
	if ($mx_user_id != ANONYMOUS)
	{
		...
		if (!is_null($notify_status))
		{
			if (isset($_GET['unwatch']))
			{
				...
				$redirect_url = mx_append_sid(PHPBB_URL . "view$mode.$phpEx", "$u_url=$match_id&amp;start=$start");
	...
	if ($can_watch)
	{
		$s_watching['link'] = mx_append_sid(PHPBB_URL . "view$mode.$phpEx", "$u_url=$match_id&amp;" . (($is_watching) ? 'unwatch' : 'watch') . "=$mode&amp;start=$start");
