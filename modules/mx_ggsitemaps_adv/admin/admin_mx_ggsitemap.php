...
* @version $Id: admin_mx_ggsitemap.php,v 1.2 2013/06/17 18:23:38 orynider Exp $
...
if ( file_exists( 'admin_board.' . $phpEx ) )
{
	...
	$module_root_path =  './../mx_ggsitemaps_adv/';
	$pathkb = '';
}
else
{
	...
	$mx_root_path = './../../../';
	$admin_root_path1 = 'admin/';
	$admin_root_path2 = 'modules/mx_ggsitemaps_adv/admin/';
...
{
	mx_message_die( CRITICAL_ERROR, "Could not query Google sitemap configuration information ", "", __LINE__, __FILE__, $sql );
}
else
{
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		if( $mode == "submit" ) {
			...
			if ( ($config_name === 'rss_image' ) && ( trim($config_value) == ''  ) )  {
				$new[$config_name] = 'rss_mxp_big.gif';
			}
			...
			if ( ($config_name === 'ggs_ver' ) && ( $config_value != "v1.3.0" ) ) {
			...
			if ( ($config_name === 'ggs_gzip' ) ) {
				if ($board_config['ggs_gzip'] !== $new[$config_name] ) {
					...
					if( !$db->sql_query($sql) )
					{
						mx_message_die(GENERAL_ERROR, "Failed to synchronize Google sitemap configuration ", "", __LINE__, __FILE__, $sql);
					}
				}
			}
			...
			if( !$db->sql_query($sql) )
			{
				mx_message_die(GENERAL_ERROR, "Failed to update Google sitemap configuration ", "", __LINE__, __FILE__, $sql);
			}
		}
	}
	// Cache settings checks
	$cache_dir = defined('IN_PORTAL') ? $mx_root_path . 'modules/mx_ggsitemaps_adv/' . $new['ggs_cache_dir'] : $phpbb_root_path .'mx_ggsitemaps_adv/' . $new['ggs_cache_dir'];
	$cache_dir = phpBB2::phpbb_realpath($cache_dir);
	...
	if( $mode == "submit" )
	{
		$message = $lang['Google_Config_updated'] . "<br /><br />" . sprintf($lang['Click_return_ggsitemap_config'], "<a href=\"" . mx_append_sid("admin_mx_ggsitemap.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . mx_append_sid($mx_root_path.$admin_root_path1 . "index.$phpEx?pane=right") . "\">", "</a>");
	...
	if ($mode == "clear_cache") {
		...
		$message .= "<br />" . sprintf($lang['Click_return_ggsitemap_config'], "<a href=\"" . mx_append_sid("admin_mx_ggsitemap.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . mx_append_sid($mx_root_path.$admin_root_path1 . "index.$phpEx?pane=right") . "\">", "</a>");
		mx_message_die(GENERAL_MESSAGE, $message);
	}
...
$template->assign_vars(array(
	...
	"S_CONFIG_ACTION" => mx_append_sid("admin_mx_ggsitemap.$phpEx"),
	...
	// Common
	"VER_INFO" => 'mx_ggsitemaps_adv ' . $new['ggs_ver'] . '<br/>' .  make_clickable($new['ggs_c_info']),
	"L_YES" => $lang['Yes'],
	"L_NO" => $lang['No'],
...
// If MXP is installed with mx, let's show some more options
if (defined('IN_PORTAL'))
{
	$template -> assign_block_vars('mx_config', array(
			"L_SITEMAP_MX_SET" => $lang['gen_mx_set'],
			"L_SITEMAP_MX_SET_EXPLAIN" => sprintf($lang['gen_mx_set_explain'], "<a href=\"http://www.mx-publisher.com/\" target=\"_MXP\">", "</a>"),
...
if ( file_exists( $mx_root_path . $pathkb . 'includes/functions_kb.' . $phpEx ) && defined('IN_PORTAL') )
{
	$template -> assign_block_vars('kb_config', array(
			...
			"L_KB_MX_PAGE_EXPLAIN" => sprintf($lang['ggs_kb_mx_page_explain'], "<a href=\"http://www.mx-publisher.com/\" target=\"_MXP\">", "</a>"),
