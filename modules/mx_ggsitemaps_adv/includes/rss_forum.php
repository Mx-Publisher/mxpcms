...
* @version $Id: rss_forum.php,v 1.2 2008/09/21 15:58:56 orynider Exp $
...
if (($this->actions['action'] === 'forum' && $this->actions['cat'] === 'cat')|| ($this->actions['action'] === 'channels')) {
	...
	if ($this->actions['action'] === 'forum' && $this->actions['list_id'] > 0 ) {
		...
		$sql = "SELECT COUNT(t.topic_id) AS topic
			...
			$time_limit_sql_ptc
			$not_in_id_sql_ptc t.topic_status <> 0";
		if ( !($result = $db->sql_query($sql)) ) {
			mx_message_die(GENERAL_ERROR, 'Could not obtain limited posts count information', '', __LINE__, __FILE__, $sql);
		}
		$row = $db->sql_fetchrow($result);
	...
	if (!(PORTAL_BACKEND == 'phpbb3'))
	{
		if ( !function_exists('obtain_word_list') )
		{
			...
			$this->obtain_word_list($orig_word, $replacement_word);
		}
		else
		{
			$orig_word = array();
