...
* @version $Id: google_forum.php,v 1.7 2011/05/01 22:19:08 orynider Exp $
...
$forumsallow = $phpbb_auth->get_auth_forum();
	
$forumsignore = implode(',', $this->output_data['exclude_list']); //mx_acl_getfignore($auth_level, implode(',', $this->output_data['exclude_list']));

if (!empty($this->output_data['exclude_list'])) {
	$not_in_id_sql = " f.forum_id NOT IN (" . implode(",", $this->output_data['exclude_list']) . ") AND f.forum_id IN (" . $forumsallow . ")";
} else {
	$not_in_id_sql = " f.forum_id IN (" . $forumsallow . ")";
}
