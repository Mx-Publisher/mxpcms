...
* @version $Id: admin_prune_user_posts.php,v 1.4 2008/07/15 22:05:25 jonohlsson Exp $
...
if ( !empty( $setmodules ) )
{
	if (PORTAL_BACKEND == 'phpbb2')
	{
		...
		$module['phpbb2admin']['4_Prune_user_posts'] = 'modules/mx_users/admin/' . $filename;
	}
	return;
}
...
if (!$mx_request_vars->is_post('submit'))
{
	...
	if ( !$result )
	{
		mx_message_die(CRITICAL_ERROR, 'Could not obtain group information', '', __LINE__, __FILE__, $sql);
	}
	...
	if ( !$result )
	{
		mx_message_die(CRITICAL_ERROR, 'Could not obtain banned users information', '', __LINE__, __FILE__, $sql);
	}
	...
	if ( !$result )
	{
		mx_message_die(CRITICAL_ERROR, 'Could not obtain banned IPs information', '', __LINE__, __FILE__, $sql);
	}
	...
	$template->assign_vars(array(
		...
		'S_SEARCH_ACTION' => mx_append_sid($phpbb_root_path.'search.php?mode=searchuser'),
		'S_PRUNE_ACTION' => mx_append_sid("admin_prune_user_posts.php"))
	);
	...
	if ( !$result )
	{
		mx_message_die(CRITICAL_ERROR, 'Could not obtain category information', '', __LINE__, __FILE__, $sql);
	}
	...
	for( $i = 0; $i < $cat_rows; $i++ )
	{
		...
		if ( !$result )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not obtain forum information', '', __LINE__, __FILE__, $sql);
		}
...
else
{
	...
	switch ($mx_request_vars->post('prune_type', MX_TYPE_NO_TAGS))
	{
		case 'user':
			...
			$sql = "SELECT user_id
						FROM ".USERS_TABLE."
							WHERE username = '".addslashes($mx_request_vars->post('username', MX_TYPE_NO_TAGS))."'";

			$result = $db->sql_query($sql);

			if ( !$result )
			{
				mx_message_die(CRITICAL_ERROR, 'Could not obtain user information', '', __LINE__, __FILE__, $sql);
			}
		...
		case 'all_users':
			...
			if ( !$result )
			{
				mx_message_die(CRITICAL_ERROR, 'Could not select all users', '', __LINE__, __FILE__, $sql);
			}
		...
		case 'banned_users':
			...
			if ( !$result )
			{
				mx_message_die(CRITICAL_ERROR, 'Could not query ban lists table', '', __LINE__, __FILE__, $sql);
			}

			// If the user doesn't exist
			if($db->sql_numrows($result)==0)
			{
				mx_message_die(GENERAL_MESSAGE, $lang['No_banned_users']);
			}
		...
		case 'group':
			...
			$group_id = $mx_request_vars->post('prune_group', MX_TYPE_INT);
			...
			switch (PORTAL_BACKEND)
			{
				...
				case 'phpbb3':
					$sql = "SELECT COUNT(group_id) as numrows
								FROM ".GROUPS_TABLE."
									WHERE group_id = ".$group_id." AND group_name NOT IN ('BOTS', 'GUESTS')";
					break;
			}
			...
			if ( !$result )
			{
				mx_message_die(CRITICAL_ERROR, 'Could not query groups table', '', __LINE__, __FILE__, $sql);
			}
			...
			if ( !$result )
			{
				mx_message_die(CRITICAL_ERROR, 'Could not query user groups table', '', __LINE__, __FILE__, $sql);
			}

			// If the user doesn't exist
			if($db->sql_numrows($result)==0)
			{
				mx_message_die(GENERAL_MESSAGE, $lang['No_group_members']);
			}
		...
		case 'ip':
			...
			$prune_ip = $mx_request_vars->post('prune_ip');
			...
			else if( preg_match('/^([0-9]{1,2}|[0-2][0-9]{0,2})(\.([0-9]{1,2}|[0-2][0-9]{0,2})){0,2}\.\*/', $prune_ip) )
			{
			...
			else if( preg_match('/^([0-9]{1,2}|[0-2][0-9]{0,2})(\.([0-9]{1,2}|[0-2][0-9]{0,2})){3}(\s)*-(\s)*([0-9]{1,2}|[0-2][0-9]{0,2})(\.([0-9]{1,2}|[0-2][0-9]{0,2})){3}$/', $prune_ip) )
			{
				// We will split the two ranges
				$range = preg_split('/[-\s]+/', $prune_ip);
				...
				{
					mx_message_die(GENERAL_MESSAGE, $lang['Prune_invalid_range']);
				}
			...
			{
				mx_message_die(GENERAL_MESSAGE, $lang['Prune_invalid_IP']);
			}
			break;
		case 'banned_ips':
			...
			if($db->sql_numrows($result)==0)
			{
				mx_message_die(GENERAL_MESSAGE, $lang['No_banned_IPs']);
			}
		...
		case 'guest_users':
			...
			break;
		default:
			mx_message_die(GENERAL_MESSAGE, $lang['Prune_invalid_mode']);
	}
	...
	if ( !$result )
	{
		mx_message_die(CRITICAL_ERROR, 'Could not query forums table', '', __LINE__, __FILE__, $sql);
	}
	...
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		if( $mx_request_vars->post('prune_forumid_'.$row['forum_id'], MX_TYPE_NO_TAGS) == 'true' )
		{
	...
	if( empty($where_sql) )
	{
		mx_message_die(GENERAL_MESSAGE, $lang['No_forums_selected']);
	}
	...
	if ($mx_request_vars->post('prune_stickies', MX_TYPE_NO_TAGS) == 'true' )
	{
		$where_sql .= ' AND t.topic_type <> '.POST_STICKY;
	}

	// Are we going to prune announcements?
	if ($mx_request_vars->post('prune_announcements', MX_TYPE_NO_TAGS) == 'true')
	{
		$where_sql .= ' AND t.topic_type <> '.POST_ANNOUNCE;
	}

	// How about open topics?
	if ($mx_request_vars->post('prune_open', MX_TYPE_NO_TAGS) == 'true')
	{
		$where_sql .= ' AND t.topic_status <> '.TOPIC_UNLOCKED;
	}

	// How about polls?
	if ($mx_request_vars->post('prune_polls', MX_TYPE_NO_TAGS) == 'true')
	{
		$where_sql .= ' AND t.topic_vote <> 0';
	}

	// Let's sort out dates now
	switch ($mx_request_vars->post('prune_date_type', MX_TYPE_NO_TAGS))
	{
		// We are pruning before are certain amount of time
		case 'time':
			$time_value = $mx_request_vars->post('prune_time_value', MX_TYPE_INT);

			switch ($mx_request_vars->post('prune_time_type', MX_TYPE_NO_TAGS))
			{
				case 'seconds':
			...
			if ($mx_request_vars->post('prune_time_order', MX_TYPE_NO_TAGS) == 'before' )
			{
				$operator = '<';
			}
			else
			...
			break;
		case 'by_date':
			$date = $mx_request_vars->post('prune_dateby_value', MX_TYPE_NO_TAGS);
			...
			if( preg_match('/^(0?[1-9]{1}|[1-2]{1}[0-9]{1}|3[0-1]{1})\/(0?[1-9]{1}|1[0-2]{1})\/(19[7-9]{1}[0-9]{1}|20([0-2]{1}[0-9]{1}|3[0-8]{1}))$/', $date) )
			{
				...
				if($unix_time == 0)
				{
					mx_message_die(GENERAL_MESSAGE, $lang['Prune_invalid_date']);
				}
			}
			else
			{
			...
			if ($mx_request_vars->post('prune_dateby_type', MX_TYPE_NO_TAGS) == 'before' )
			{
				$operator = '<';
			}
			else if ($mx_request_vars->post('prune_dateby_type', MX_TYPE_NO_TAGS) == 'after' )
			{
				$operator = '>';
				$unix_time += 86400;
			}
		...
		case 'range':
			...
			$start_range = $mx_request_vars->post('prune_daterange_start', MX_TYPE_NO_TAGS);
			$end_range = $mx_request_vars->post('prune_daterange_stop', MX_TYPE_NO_TAGS);
	...
	for( $i = 0; $i < count($sql_array); $i++ )
	{
		...
		if ( !$result )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not get posts table data', '', __LINE__, __FILE__, $sql);
		}
	...
	if( count($posts) == 0 )
	{
		mx_message_die(GENERAL_MESSAGE, $lang['Prune_no_posts']);
	}
	...
	for($i = 0; $i < count($posts); $i++)
	{
		...
		if( !isset($topics_to_delete[$posts[$i]['topic_id']]) )
		{
			...
			if($posts[$i]['topic_first_post_id'] == $posts[$i]['post_id'])
			{
				...
				if ($mx_request_vars->post('prune_topic_started', MX_TYPE_NO_TAGS) == 'true')
				{
					$delete_this_topic = true;
				}
			}
		...
		if($delete_this_topic == true)
		{
			...
			if($posts[$i]['topic_last_post_id'] != $posts[$i]['topic_last_post_id'])
			{
				...
				if ( !$result )
				{
					mx_message_die(CRITICAL_ERROR, 'Could not query topics table', '', __LINE__, __FILE__, $sql);
				}
	...
	if(	!$db->sql_query($sql) )
	{
		mx_message_die(CRITICAL_ERROR, 'Could not delete posts table data', '', __LINE__, __FILE__, $sql);
	}
	...
	if(	!$db->sql_query($sql) )
	{
		mx_message_die(CRITICAL_ERROR, 'Could not delete posts text table data', '', __LINE__, __FILE__, $sql);
	}

	// If we're going to update the search tables
	if ($mx_request_vars->post('prune_update_search', MX_TYPE_NO_TAGS) == 'true' )
	{
	...
	foreach( $topics_to_update as $topic_id => $topic_vote )
	{
		...
		if ( !$result )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not count posts data', '', __LINE__, __FILE__, $sql);
		}
		...
		else
		{
			...
			if($row['numrows'] == 1)
			{
				...
				if ( !$result )
				{
					mx_message_die(CRITICAL_ERROR, 'Could not select post data', '', __LINE__, __FILE__, $sql);
				}
			...
			else
			{
				...
				if ( !$result )
				{
					mx_message_die(CRITICAL_ERROR, 'Could not select first post data', '', __LINE__, __FILE__, $sql);
				}
				...
				if ( !$result )
				{
					mx_message_die(CRITICAL_ERROR, 'Could not select last post data', '', __LINE__, __FILE__, $sql);
				}
			...
			if ( !$db->sql_query($sql) )
			{
				mx_message_die(CRITICAL_ERROR, 'Could not update topics table', '', __LINE__, __FILE__, $sql);
			}
		}
	}
	...
	foreach( $topics_to_delete as $topic_id => $topic_vote )
	{
		...
		if($topic_vote == 1)
		{
			...
			if ( !$result )
			{
				mx_message_die(CRITICAL_ERROR, 'Could not poll descriptions table', '', __LINE__, __FILE__, $sql);
			}
	...
	if( $vote_ids_sql != '' )
	{
		...
		if( !$db->sql_query($sql) )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not remove poll descriptions data', '', __LINE__, __FILE__, $sql);
		}
		...
		if( !$db->sql_query($sql) )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not remove poll results data', '', __LINE__, __FILE__, $sql);
		}
		...
		if( !$db->sql_query($sql) )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not remove poll voters data', '', __LINE__, __FILE__, $sql);
		}
	}
	...
	if( $topic_ids_sql != '' )
	{
		...
		if( !$db->sql_query($sql) )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not remove topic watch data', '', __LINE__, __FILE__, $sql);
		}
		...
		if( !$db->sql_query($sql) )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not remove topic data', '', __LINE__, __FILE__, $sql);
		}
	}
	...
	foreach( $forums_to_update as $forum_id => $value )
	{
		...
		if ( !$result )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not select posts count forum total', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( !$result )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not select topics count forum total', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( !$result )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not select last post in forum', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( !$db->sql_query($sql) )
		{
			mx_message_die(CRITICAL_ERROR, 'Could not update forum data', '', __LINE__, __FILE__, $sql);
		}
	}

	// Update user post counts if we need to
	if ($mx_request_vars->post('prune_update_post_counts', MX_TYPE_NO_TAGS) == 'true' )
	{
		foreach( $users_post_counts as $user_id => $post_count )
		{
			...
			if ( !$db->sql_query($sql) )
			{
				mx_message_die(CRITICAL_ERROR, 'Could not update user post count', '', __LINE__, __FILE__, $sql);
			}

		}
	}

	$bye_message = sprintf($lang['Prune_finished'],mx_append_sid("admin_prune_user_posts.".$phpEx), mx_append_sid("index.".$phpEx."?pane=right"));

	// Say bye bye
	mx_message_die(GENERAL_MESSAGE, $bye_message);
}
