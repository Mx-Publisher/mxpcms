...
else
{
	...
	else
	{
		...
		if(!$db->sql_query($sql))
		{
			mx_message_die(GENERAL_ERROR, 'Failed to update emails', '', __LINE__, __FILE__, $sql);
		}
		...
		if(!$db->sql_query($sql))
		{
			mx_message_die(CRITICAL_ERROR, 'Could not query email information', '', __LINE__, __FILE__, $sql);
		}
		...
		if(empty($row['email']))
		{
			...
			if(!$db->sql_query($sql))
			{
				mx_message_die(CRITICAL_ERROR, 'Could not update email information', '', __LINE__, __FILE__, $sql);
			}
		}
		else
		{
			if(($row['sent'] >= 3) && !($userdata['user_level'] == ADMIN))
			{
				mx_message_die(GENERAL_ERROR, $lang['Thankyou_limit']);
			}
			else
			{
