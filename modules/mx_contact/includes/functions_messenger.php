...
*
* @package Emailer
* @version $Id: functions_messenger.php,v 1.1 2011/12/06 10:00:23 orynider Exp $
* @copyright (c) 2002-2008 MX-Publisher Project Team & ( C) 2001 The phpBB Group
* @license http://opensource.org/licenses/gpl-license.php GNU Public License
* @link http://www.mxp.souceforge.net/
*
...
class messenger
{
	...
	*/
	function messenger($use_queue = true)
	{
		...
		$this->use_queue = (!$board_config['email_package_size']) ? false : $use_queue;
		$this->use_html = false;		
		$this->subject = '';
	...
	*/
	...
	{
		global $board_config, $portal_config;
		...
		if (!$portal_config['smtp_delivery'] && DIRECTORY_SEPARATOR == '\\')
		{
	...
	*/
	...
	{
		global $board_config, $portal_config, $phpbb_root_path, $mx_root_path, $module_root_path, $mx_user;
		...
		if (!trim($template_lang))
		{
			...
			$template_lang = basename($board_config['default_lang']);
		}
		...
		if (!isset($this->tpl_msg[$template_lang . $template_file]))
		{
			...
			if (!$template_path)
			{
				if (@file_exists($mx_root_path . 'language/lang_' . $board_config['default_lang'] . '/email/' . $template_file . '.txt') )
				{
				  	$fallback_template_path = $template_path = $mx_root_path . 'language/lang_' . $board_config['default_lang'] . '/email';
				}
				
				if (@file_exists($module_root_path . 'language/lang_' . $board_config['default_lang'] . '/email/' . $template_file . '.txt') )
				{
				  	$fallback_template_path = $template_path = $module_root_path . 'language/lang_' . $board_config['default_lang'] . '/email';
				}					
			}			
			if (!$template_path)
			{
				$template_path = (!empty($mx_user->lang_path)) ? $mx_user->lang_path : $phpbb_root_path . 'language/';
				$template_path .= $template_lang . '/email';

				// we can only specify default language fallback when the path is not a custom one for which we
				// do not know the default language alternative
				if ($template_lang !== basename($board_config['default_lang']))
				{
					$fallback_template_path = (!empty($mx_user->lang_path)) ? $mx_user->lang_path : $phpbb_root_path . 'language/';
					$fallback_template_path .= basename($board_config['default_lang']) . '/email';
				}
			}
	...
	*/
	function send($method = NOTIFY_EMAIL, $break = false)
	{
		...
		{
			$this->assign_vars(array(
				'EMAIL_SIG'	=> str_replace('<br />', "\n", "-- \n" . htmlspecialchars_decode($board_config['board_email_sig'])),
			));
		}
		...
		{
			$this->assign_vars(array(
				'SITENAME'	=> htmlspecialchars_decode($board_config['sitename']),
			));
		}
		...
		{
			$this->subject = (trim($match[2]) != '') ? trim($match[2]) : (($this->subject != '') ? $this->subject : $mx_user->lang['NO_EMAIL_SUBJECT']);
			$drop_header .= '[\r\n]*?' . preg_quote($match[1], '#');
		}
		else
		{
			$this->subject = (($this->subject != '') ? $this->subject : $mx_user->lang['NO_EMAIL_SUBJECT']);
		}
	...
	*/
	...
	{
		global $mx_user, $phpEx, $phpbb_root_path, $board_config; $portal_config;

		// Session doesn't exist, create it
		if (!isset($mx_user->session_id) || $mx_user->session_id === '')
		{
		...
		{
			case 'EMAIL':
				$message = '<strong>EMAIL/' . (($portal_config['smtp_delivery']) ? 'SMTP' : 'PHP/' . $board_config['email_function_name'] . '()') . '</strong>';
			break;
		...
		phpBB3::add_log('critical', 'LOG_ERROR_' . $type, $message);
	}
	...
	*/
	function save_queue()
	{
		...
		if ($board_config['email_package_size'] && $this->use_queue && !empty($this->queue))
		{
			$this->queue->save();
			return;
		}
	...
	*/
	function build_header($to, $cc, $bcc)
	{
		...
		$headers[] = 'Sender: <' . $board_config['board_email'] . '>';
		$headers[] = 'MIME-Version: 1.0';
		$headers[] = 'Message-ID: <' . md5(phpBB3::unique_id(time())) . '@' . $board_config['server_name'] . '>';
		$headers[] = 'Date: ' . date('r', time());
		// Build html header ?
		$headers[] = ($this->use_html) ? 'Content-Type: text/html; charset=UTF-8' : 'Content-Type: text/plain; charset=UTF-8'; // format=flowed		
		$headers[] = 'Content-Transfer-Encoding: 8bit'; // 7bit
		$headers[] = 'X-Priority: ' . $this->mail_priority;
		$headers[] = 'X-MSMail-Priority: ' . (($this->mail_priority == MAIL_LOW_PRIORITY) ? 'Low' : (($this->mail_priority == MAIL_NORMAL_PRIORITY) ? 'Normal' : 'High'));
		$headers[] = 'X-Mailer: phpBB3';
		$headers[] = 'X-MimeOLE: phpBB3';
		$headers[] = 'X-phpBB-Origin: phpbb://' . str_replace(array('http://', 'https://'), array('', ''), generate_portal_url());
	...
	*/
	function msg_email()
	{
		...
		if (empty($board_config['email_enable']))
		{
			return false;
		}
		...
		$use_queue = false;
		if ($board_config['email_package_size'] && $this->use_queue)
		{
			if (empty($this->queue))
			{
				$this->queue = new queue();
				$this->queue->init('email', $board_config['email_package_size']);
			}
			$use_queue = true;
		}
		...
		{
			$this->replyto = '<' . $board_config['board_contact'] . '>';
		}

		if (empty($this->from))
		{
			$this->from = '<' . $board_config['board_contact'] . '>';
		}

		$encode_eol = ($board_config['smtp_delivery']) ? "\r\n" : $this->eol;
	...
	*/
	function msg_jabber()
	{
		...
		if (empty($board_config['jab_enable']) || empty($board_config['jab_host']) || empty($board_config['jab_username']) || empty($board_config['jab_password']))
		{
			return false;
		}
		...
		$use_queue = false;
		if ($board_config['jab_package_size'] && $this->use_queue)
		{
			if (empty($this->queue))
			{
				$this->queue = new queue();
				$this->queue->init('jabber', $board_config['jab_package_size']);
			}
			$use_queue = true;
		}
		...
		if (!$use_queue)
		{
			...
			$this->jabber = new jabber($board_config['jab_host'], $board_config['jab_port'], $board_config['jab_username'], $board_config['jab_password'], $board_config['jab_use_ssl']);

			if (!$this->jabber->connect())
			{
				$this->error('JABBER', $mx_user->lang['ERR_JAB_CONNECT'] . '<br />' . $this->jabber->get_log());
				return false;
			}

			if (!$this->jabber->login())
			{
				$this->error('JABBER', $mx_user->lang['ERR_JAB_AUTH'] . '<br />' . $this->jabber->get_log());
				return false;
			}
...
class queue
{
	...
	*/
	function process()
	{
		global $db, $board_config, $portal_config, $phpEx, $phpbb_root_path, $mx_user;

		$lock_fp = $this->lock();

		set_config('last_queue_run', time(), true);

		if (!file_exists($this->cache_file) || filemtime($this->cache_file) > time() - $board_config['queue_interval'])
		{
			$this->unlock($lock_fp);
			return;
		}
			...
			*/
			...
			switch ($object)
			{
				...
				case 'jabber':
					...
					if (!$this->jabber->connect())
					{
						messenger::error('JABBER', $mx_user->lang['ERR_JAB_CONNECT']);
						continue 2;
					}

					if (!$this->jabber->login())
					{
						messenger::error('JABBER', $mx_user->lang['ERR_JAB_AUTH']);
						continue 2;
					}
			...
			for ($i = 0; $i < $num_items; $i++)
			{
				...
				switch ($object)
				{
					case 'email':
						...
						if ($portal_config['smtp_delivery'])
						{
...
function smtpmail($addresses, $subject, $message, &$err_msg, $headers = false)
{
	...
	if (trim($subject) == '')
	{
		$err_msg = (isset($mx_user->lang['NO_EMAIL_SUBJECT'])) ? $mx_user->lang['NO_EMAIL_SUBJECT'] : 'No email subject specified';
		return false;
	}

	if (trim($message) == '')
	{
		$err_msg = (isset($mx_user->lang['NO_EMAIL_MESSAGE'])) ? $mx_user->lang['NO_EMAIL_MESSAGE'] : 'Email message was blank';
		return false;
	}
	...
	$smtp->add_backtrace('Connecting to ' . $board_config['smtp_host'] . ':' . $board_config['smtp_port']);

	// Ok we have error checked as much as we can to this point let's get on it already.
	ob_start();
	$smtp->socket = fsockopen($board_config['smtp_host'], $board_config['smtp_port'], $errno, $errstr, 20);
	...
	if (!$smtp->socket)
	{
		...
		$err_msg = (isset($mx_user->lang['NO_CONNECT_TO_SMTP_HOST'])) ? sprintf($mx_user->lang['NO_CONNECT_TO_SMTP_HOST'], $errno, $errstr) : "Could not connect to smtp host : $errno : $errstr";
	...
	if ($err_msg = $smtp->log_into_server($board_config['smtp_host'], $board_config['smtp_username'], $board_config['smtp_password'], $board_config['smtp_auth_method']))
	{
		$smtp->close_session($err_msg);
		return false;
	}
	...
	$smtp->server_send('MAIL FROM:<' . $board_config['board_email'] . '>');
	...
	if (!$rcpt)
	{
		$mx_user->session_begin();
		$err_msg .= '<br /><br />';
		$err_msg .= (isset($mx_user->lang['INVALID_EMAIL_LOG'])) ? sprintf($mx_user->lang['INVALID_EMAIL_LOG'], htmlspecialchars($mail_to_address)) : '<strong>' . htmlspecialchars($mail_to_address) . '</strong> possibly an invalid email address?';
		$smtp->close_session($err_msg);
		return false;
	}
...
class smtp_class
{
	...
	*/
	function server_parse($response, $line)
	{
		...
		while (substr($this->server_response, 3, 1) != ' ')
		{
			...
			{
				return (isset($mx_user->lang['NO_EMAIL_RESPONSE_CODE'])) ? $mx_user->lang['NO_EMAIL_RESPONSE_CODE'] : 'Could not get mail server response codes';
			}
		...
		{
			$this->numeric_response_code = (int) substr($this->server_response, 0, 3);
			return (isset($mx_user->lang['EMAIL_SMTP_ERROR_RESPONSE'])) ? sprintf($mx_user->lang['EMAIL_SMTP_ERROR_RESPONSE'], $line, $this->server_response) : "Ran into problems sending Mail at <strong>Line $line</strong>. Response: $this->server_response";
		}
	...
	*/
	function log_into_server($hostname, $mx_username, $password, $default_auth_method)
	{
		global $mx_user;
		...
		if ($default_auth_method == 'POP-BEFORE-SMTP' && $mx_username && $password)
		{
			global $board_config;
			...
			$result = $this->pop_before_smtp($hostname, $mx_username, $password);
			$mx_username = $password = $default_auth_method = '';

			// We need to close the previous session, else the server is not
			// able to get our ip for matching...
			if (!$this->socket = @fsockopen($board_config['smtp_host'], $board_config['smtp_port'], $errno, $errstr, 10))
			{
				if ($errstr)
				{
					$errstr = utf8_convert_message($errstr);
		...
		{
			return (isset($mx_user->lang['SMTP_NO_AUTH_SUPPORT'])) ? $mx_user->lang['SMTP_NO_AUTH_SUPPORT'] : 'SMTP server does not support authentication';
		}
		...
		if (!$method)
		{
			return (isset($mx_user->lang['NO_SUPPORTED_AUTH_METHODS'])) ? $mx_user->lang['NO_SUPPORTED_AUTH_METHODS'] : 'No supported authentication methods';
		}

		$method = strtolower(str_replace('-', '_', $method));
		return $this->$method($mx_username, $password);
	}

	/**
	* Pop before smtp authentication
	*/
	function pop_before_smtp($hostname, $mx_username, $password)
	{
		global $mx_user;
		...
		if (!$this->socket = @fsockopen($hostname, 110, $errno, $errstr, 10))
		{
			...
			return (isset($mx_user->lang['NO_CONNECT_TO_SMTP_HOST'])) ? sprintf($mx_user->lang['NO_CONNECT_TO_SMTP_HOST'], $errno, $errstr) : "Could not connect to smtp host : $errno : $errstr";
		}

		$this->server_send("USER $mx_username", true);
	...
	*/
	function plain($mx_username, $password)
	{
		...
		$base64_method_plain = base64_encode("\0" . $mx_username . "\0" . $password);
	...
	*/
	function login($mx_username, $password)
	{
		...
		$this->server_send(base64_encode($mx_username), true);
		if ($err_msg = $this->server_parse('334', __LINE__))
		{
			return $err_msg;
		}
	...
	*/
	function cram_md5($mx_username, $password)
	{
		...
		$base64_method_cram_md5 = base64_encode($mx_username . ' ' . $md5_digest);
	...
	* A real pain in the ***
	*/
	function digest_md5($mx_username, $password)
	{
		global $board_config, $mx_user;
		...
		{
			$tokens['realm'] = (function_exists('php_uname')) ? php_uname('n') : $mx_user->host;
		}
		...
		if (!empty($md5_challenge))
		{
			...
			$digest_uri = 'smtp/' . $board_config['smtp_host'];

			$auth_1 = sprintf('%s:%s:%s', pack('H32', md5(sprintf('%s:%s:%s', $mx_username, $md5_challenge['realm'], $password))), $md5_challenge['nonce'], $cnonce);
			...
			$input_string = sprintf('username="%s",realm="%s",nonce="%s",cnonce="%s",nc="00000001",qop=auth,digest-uri="%s",response=%s,%d', $mx_username, $md5_challenge['realm'], $md5_challenge['nonce'], $cnonce, $digest_uri, $response_value, $md5_challenge['maxbuf']);
		}
		else
		{
			return (isset($mx_user->lang['INVALID_DIGEST_CHALLENGE'])) ? $mx_user->lang['INVALID_DIGEST_CHALLENGE'] : 'Invalid digest challenge';
		}
...
function phpbb_mail($to, $subject, $msg, $headers, $eol, &$err_msg)
{
	...
	$email_function_name = ($board_config['email_function_name']) ? $board_config['email_function_name'] : '@mail';
	
	ob_start();
	// On some PHP Versions mail() *may* fail if there are newlines within the subject.
	// Newlines are used as a delimiter for lines in mail_encode() according to RFC 2045 section 6.8.
	// Because PHP can't decide what is wanted we revert back to the non-RFC-compliant way of separating by one space (Use '' as parameter to mail_encode() results in SPACE used)
	$result = $board_config['email_function_name']($to, mail_encode($subject, ''), wordwrap(utf8_wordwrap($msg), 997, "\n", true), $headers);
	$err_msg = ob_get_clean();
	
	//$result = ($result) ? $result : mail($to, mail_encode($subject, ''), wordwrap(utf8_wordwrap($msg), 997, "\n", true), $headers);
