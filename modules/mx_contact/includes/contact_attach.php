...
if($contact_config['contact_hash'] == 0)
{
	...
	$attach = $server_url . $script_path . "/" . trim($contact_config['contact_file_root']) . "/" . contact_contact_decode_ip($user_ip) . "/" . $attachment;
}
else
{
	//
	...
	$attach = $server_url . $script_path . "/" . trim($contact_config['contact_file_root']) . "/" . contact_decode_ip($user_ip) . "/" . $attachment;
}
...
else
{
	...
	elseif(file_exists(@phpbb_realpath($phpbb_root_path . $contact_config['contact_file_root'] . "/" . contact_decode_ip($user_ip) . "/" . $attachment)))
	{
	...
	if(!file_exists(@phpbb_realpath($module_root_path . $contact_config['contact_file_root'] . "/" . contact_decode_ip($user_ip))))
	{
		if(!file_exists(@phpbb_realpath($module_root_path . $contact_config['contact_file_root'] . "/")))
		{
			@mkdir($module_root_path . $contact_config['contact_file_root'] . "/", 0755);

			$fd =	@fopen($module_root_path . $contact_config['contact_file_root'] . "/index.html", 'a');
				@fclose($fd);
		}
		@mkdir($module_root_path . $contact_config['contact_file_root'] . "/" . contact_decode_ip($user_ip), 0755);
	}

	if(@is_uploaded_file($HTTP_POST_FILES['attachment']['tmp_name']))
	{
		@move_uploaded_file($HTTP_POST_FILES['attachment']['tmp_name'],  $module_root_path . $contact_config['contact_file_root'] . "/" . contact_decode_ip($user_ip) . "/" . $attachment);

		$fa =	@fopen($module_root_path . $contact_config['contact_file_root'] . "/" . contact_decode_ip($user_ip) . "/index.html", 'a');
			@fclose($fa);
