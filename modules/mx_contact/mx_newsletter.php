...
* @version $Id: mx_newsletter.php,v 1.5 2011/04/17 08:37:07 orynider Exp $
...
$real_name		= (!isset($_POST['real_name'])) ? '' : stripslashes(trim(htmlspecialchars($_POST['real_name'])));
$institution	= (!isset($_POST['institution'])) ? '' : stripslashes(trim(htmlspecialchars($_POST['institution'])));
$fax			= (!isset($_POST['fax'])) ? '' : stripslashes(trim(htmlspecialchars($_POST['fax'])));
$phone			= (!isset($_POST['phone'])) ? '' : stripslashes(trim(htmlspecialchars($_POST['phone'])));
$email			= (!isset($_POST['email'])) ? '' : stripslashes(trim(htmlspecialchars($_POST['email'])));
$comments		= (!isset($_POST['feedback'])) ? '' : stripslashes(trim(htmlspecialchars($_POST['feedback'])));
$attachment		= (!isset($_FILES['attachment']['name'])) ? '' : basename($_FILES['attachment']['name']);
$code			= (!isset($_POST['code'])) ? '' : htmlspecialchars(trim($_POST['code']));
$newsletter			= (!isset($_POST['newsletter'])) ? 0 : 1;
...
//
if($mx_request_vars->is_get('delete'))
{
...
// Start send script
//
if($mx_request_vars->is_request('submit'))
{
	function error_check()
	{
		global $_FILES, $lang, $phpEx, $module_root_path, $mx_root_path;
	...
	// Send the e-mail
	//
	include_once($module_root_path . 'includes/contact_emailer.'.$phpEx);
	...
	$default_lang = $userdata['user_lang'] ? $userdata['user_lang'] : $board_config['default_lang'];	
	
	$emailer->extra_headers($email_headers);	
	$emailer->use_template('contact', $default_lang);
	...
	$emailer->assign_vars(array(
		...
		'USER_IP' => contact_decode_ip($user_ip),
		'TIMEDATE' => $timedate,
	...
	{
		include_once($module_root_path . 'includes/contact_extend.'.$phpEx);
	}
	...
	//
	$send_time = time();
	$getfile = (!empty($attachment)) ? $contact_config['contact_file_root'] . "/" . contact_decode_ip($user_ip) . "/" . $attachment : '';
	...
	$sql = "INSERT INTO " . CONTACT_MSGS_TABLE . $db->sql_build_array('INSERT', $sql_array);
...
$template->set_filenames(array( 
	'newsletter_body' => 'newsletter_body.'.$tplEx) 
);

$template->assign_vars(array(
	'L_CONTACT_FORM' => $lang['Contact_form'],
	
	'L_NEWSLETTER_FORM' => $lang['Newsletter_form'],
	'L_EXPLAIN_NEWSLETTER_FORM' => $lang['Newsletter_explain'],

	'L_INTRODUCTION' => $lang['Newsletter_intro'],	

	'L_REAL_NAME' => $rname,
	'L_REAL_NAME_EXPLAIN' => $lang['Real_name_explain'],

	'L_INSTITUTION' => $institution,
	'L_EXPLAIN_INSTITUTION' => $lang['Explain_institution'],
	
	'L_PHONE' => $phone,
	'L_EXPLAIN_PHONE' => $lang['Explain_phone'],
	
	'L_FAX' => $fax,
	'L_EXPLAIN_FAX' => $lang['Explain_fax'],	
	...
	'L_AGREEMENT_EXPLAIN' => $lang['Agreement_explain'],
	...
	'USERNAME' => $user_name,
	'CAPTCHA' => mx_append_sid($module_root_path . "mx_captcha.$phpEx"),
...
$template->pparse('newsletter_body');
