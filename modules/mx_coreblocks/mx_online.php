...
//if (defined('SHOW_ONLINE'))
{
	...
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		if ( ($row['session_logged_in'] && PORTAL_BACKEND != 'phpbb3' || ( $row['user_id'] != 1 && PORTAL_BACKEND == 'phpbb3' ) ) )
		{
			...
			if ( $row['user_id'] != $prev_user_id )
			{
				...
				switch (PORTAL_BACKEND)
				{
					...
					case 'phpbb3':
						...
						{
							$user_online_link = mx_get_username_string('full', $row['user_id'], $row['username'], $row['user_colour']);
...
switch (PORTAL_BACKEND)
{
	...
	case 'phpbb2':
		...
		$newest_user = sprintf($lang['Newest_user'], '<a href="' . mx_append_sid(PHPBB_URL."profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$newest_uid") . '"' . $style_color .'>', $newest_username, '</a>');
	break;
	...
	case 'phpbb3':
		...
		$newest_user = sprintf($mx_user->lang['NEWEST_USER'], mx_get_username_string('full', $newest_uid, $newest_username, $newest_color));
	break;
}
