...
function checkForm() {
	...
	if (document.post.Blog.value.length < 2) {
...
function emoticon(text) {
	text = ' ' + text + ' ';
	if (document.post.Blog.createTextRange && document.post.Blog.caretPos) {
		var caretPos = document.post.Blog.caretPos;
		caretPos.text = caretPos.text.charAt(caretPos.text.length - 1) == ' ' ? text + ' ' : text;
		document.post.Blog.focus();
	} else {
	document.post.Blog.value  += text;
	document.post.Blog.focus();
	}
}
...
function bbfontstyle(bbopen, bbclose) {
	if ((clientVer >= 4) && is_ie && is_win) {
		...
		if (!theSelection) {
			document.post.Blog.value += bbopen + bbclose;
			document.post.Blog.focus();
			return;
		}
...
function bbstyle(bbnumber) {
	...
	if (bbnumber == -1) { // Close all open tags & default button names
		while (bbcode[0]) {
			...
			document.post.Blog.value += bbtags[butnumber + 1];
	...
	if (donotinsert) {		// Close all open tags up to the one just clicked & default button names
		...
		if (imageTag && (bbnumber != 14)) {		// Close image tag before adding another
			document.post.Blog.value += bbtags[15];
		...
		// Open tag
		document.post.Blog.value += bbtags[bbnumber];
...
<table border="0" cellpadding="3" cellspacing="1" width="100%" class="forumline" style="border-top:none;">
	<tr> 
...
<!-- END switch_bbcodes -->
		  <tr> 
			<td colspan="9"><span class="gen"> 
			  <textarea name="{BLOCK_TEXT_NAME}" rows="15" cols="35" wrap="virtual" style="width:450px" tabindex="3" class="post" onselect="storeCaret(this);" onclick="storeCaret(this);" onkeyup="storeCaret(this);">{BLOCK_INFO}</textarea>
			  </span></td>
		  </tr>
		</table>
		</span></td>
	...
	<tr> 
	  ...
	  <td class="row2"><span class="gen"> </span> 
		<table cellspacing="0" cellpadding="1" border="0">
		  ...
		  <!-- BEGIN switch_show_title -->
			<tr>
				<td class="row2" width="50%"><span class="genmed">{L_SHOW_TITLE}</span><br /><span class="gensmall">{L_SHOW_TITLE_EXPLAIN}</span></td>
				<td class="row2" width="50%"><input type="radio" name="show_title" value="TRUE" {S_SHOW_TITLE_YES} /> <span class="gensmall">{L_YES}</span>&nbsp;&nbsp;<input type="radio" name="show_title" value="FALSE" {S_SHOW_TITLE_NO} /> <span class="gensmall">{L_NO}</span></td>
			</tr>
		  <!-- END switch_show_title -->
		  <!-- BEGIN switch_blog_id -->
		<tr> 
	 		<td class="row1_none"><span class="gen">{L_BLOG_ID}</span></td>
	 		<td class="row2_none"><input type="text" size="40" name="blog_id" value="{BLOG_ID}" class="post" /></td>
		</tr>
		  <!-- END switch_blog_id -->
		</table>
	  </td>
