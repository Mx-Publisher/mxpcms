...
if ($mx_request_vars->is_post('default_style') && $mx_user->data['user_level'] == ADMIN )
{
	$mx_default_style = $mx_request_vars->post('default_style', MX_TYPE_INT);
	$board_config['default_style'] = $mx_default_style;
	
	/*
	$sql = "UPDATE " . CONFIG_TABLE . " SET	config_value = '$mx_default_style'	WHERE config_name = 'default_style'";
	...
	*/
	...
	$mx_backend->set_forum_config('default_style', $mx_default_style, false);
...
if ($mx_request_vars->is_post('user_style') && $userdata['session_logged_in'] )
{
	...
	$mx_user->data['user_style'] = $mx_user_style;
	/*
	...
	*/
	$mx_backend->set_userdata('user_style', $mx_user_style);
}
