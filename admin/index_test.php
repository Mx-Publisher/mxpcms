...
* @version $Id: index_test.php,v 1.4 2013/06/28 15:32:37 orynider Exp $
	...
	function assign_menu()
	{
		...
		while ( list( $category, $action_array ) = each( $menu_array ) )
		{
			...
			while ( list( $panel, $file ) = each( $action_array ) )
			{
				...
				{
					$menu_link = mx_append_sid(PORTAL_URL . $menu_link . ( ( strpos($menu_link, '?') !== false ) ? '&sid=' : '?sid=' ) . $userdata['session_id']);
				}
