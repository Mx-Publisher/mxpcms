...
if (!empty($setmodules))
{
	$module['4_Panel_system'][' phpBBMyAdmin'] = 'admin/' . basename(__FILE__);
	return;
}
...
if ((@include $phpbb_root_path . "language/lang_" . $language . "/lang_phpbbmyadmin.$phpEx") === false)
{
	if ((@include $phpbb_root_path . "language/lang_english/lang_phpbbmyadmin.$phpEx") === false)
	{
		mx_message_die(CRITICAL_ERROR, 'Language file ' . $phpbb_root_path . "language/lang_" . $language . "/lang_phpbbmyadmin.$phpEx" . ' couldn\'t be opened.');
	}
	$language = 'english'; 
} 
...
$file = mx_append_sid(basename(__FILE__));
...
if(!strcmp($mode, 'submit'))
{
	if (empty($this_query))
	{
		mx_message_die(GENERAL_MESSAGE, sprintf($lang['SQL_Admin_No_Query'], '<a href="' . $file . '" class="gen">', '</a>' , '<a href="index.php?pane=right&amp;' . $userdata['session_id'] . '" class="gen">', '</a>'));
	}
	...
	foreach($queries as $query)
	{
		...
		if (!$result)
		{
			mx_message_die(GENERAL_ERROR, sprintf($lang['SQL_Admin_Error_In_Query'], $query, '<a href="' . $file . '" class="gen">', '</a>' , '<a href="index.php?pane=right&amp;' . $userdata['session_id'] . '" class="gen">', '</a>'));
		}
		// Show results or not?
		$donormaldie = TRUE;
	...
	if ($donormaldie)
	{
		mx_message_die(GENERAL_MESSAGE, sprintf($lang['SQL_Admin_Success_Query'], $queriesdone, '<a href="' . $file . '" class="gen">', '</a>' , '<a href="index.php?pane=right&amp;' . $userdata['session_id'] . '" class="gen">', '</a>'));
	}
}
// END submit
...
if(!strcmp($mode,'repairall'))
{
	...
	if (!$result = $db->sql_query($sql))
	{
		...
		mx_message_die(GENERAL_ERROR, $lang['SQL_Admin_Tables_Error']);
	}
	$tables = array();
	$counter = 0;
	while ($row = $db->sql_fetchrow($result))
	...
	for ($i = 1; $i <= $tablecount; $i++)
	{
		...
		if (!$result = $db->sql_query($sql))
		{
			mx_message_die(GENERAL_ERROR, sprintf($lang['SQL_Admin_Repair_Error'], $tables[$i]));
		}
	}

	// Successfully repaired all.
	mx_message_die(GENERAL_MESSAGE, sprintf($lang['SQL_Admin_Repair_Done'], $tablecount, '<a href="' . $file . '" class="gen">', '</a>' , '<a href="index.php?pane=right&amp;' . $userdata['session_id'] . '" class="gen">', '</a>'));
}
// END repair all
...
if(!strcmp($mode,'optimizeall'))
{
	...
	for ($i = 1; $i <= $tablecount; $i++)
	{
		...
		if (!$result = $db->sql_query($sql))
		{
			mx_message_die(GENERAL_ERROR, sprintf($lang['SQL_Admin_Optimize_Error'], $tables[$i]));
		}
	}

	// Successfully optimized all.
	mx_message_die(GENERAL_MESSAGE, sprintf($lang['SQL_Admin_Optimize_Done'], $tablecount, '<a href="' . $file . '" class="gen">', '</a>' , '<a href="index.php?pane=right&amp;' . $userdata['session_id'] . '" class="gen">', '</a>'));
}
// END optimize all
...
// Browse a table...
{
	...
	// No table?
	{
		mx_message_die(GENERAL_ERROR, $lang['SQL_Admin_No_Table']);
	}
	// Switch browse on:
	...
	{
		// No results?
		mx_message_die(GENERAL_ERROR, sprintf($lang['SQL_Admin_Browse_Error'], $tablename));
	}
	// Remember the number of fields (aka columns) and the number of rows:
	$field_count = $db->sql_numfields($result);
	$row_count = $db->sql_numrows($result);
...
// Show a specific table:
{
	...
	if (!$result)
	{
		...
		mx_message_die(GENERAL_ERROR, $lang['SQL_Admin_Columns_Error']);
	}
	// some general stuff:
	$template->assign_vars(array(
...
$template->assign_vars(array(
	'L_PHPMYADMIN_EXPLAIN' => isset($lang['SQL_Admin_EXPLAIN']) ? $lang['SQL_Admin_EXPLAIN'] : $lang['SQL_Admin_Title'],
	'CURRENT_TIME' => sprintf($lang['SQL_Admin_Current_Time'], time (), mx_create_date($board_config['default_dateformat'], time (), $board_config['board_timezone'])),
	'SQL_ACTION' => $file,
