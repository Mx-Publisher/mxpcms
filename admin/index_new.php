...
function cache_filename( $part = 'NONE', $file = '')
{
	...
	return "{$mx_root_path}cache/acp_{$part}{$file}.{$phpEx}";

}

function cache_file( $code, $part = 'NONE', $file = '')
{
	$filename = cache_filename( $part, $file);
...
define( 'PHPBB2X_BACKEND', 'phpbb2' );

switch ( PORTAL_BACKEND )
{
	case 'internal':
	case 'phpbb2':
		define( 'OLYMPUS_BACKEND', 'phpbb2' );
	break;
...
if ( ( empty( $_SID ) || $_SID == 'sid=' ) )
{
	$_SID = 'sid='.$mx_request_vars->get('sid', MX_TYPE_NO_TAGS);
}

if ( ( empty( $SID ) || $SID == 'sid=' ) )
{
	$SID = 'sid='.$mx_request_vars->get('sid', MX_TYPE_NO_TAGS);
}

if ( PORTAL_BACKEND == PHPBB2X_BACKEND )
{
	mx_cache::load_file( 'functions' );
	mx_cache::load_file( 'functions_selects' );
	mx_cache::load_file( 'functions_validate' );
}

if ( PORTAL_BACKEND == OLYMPUS_BACKEND )
{
	...
	mx_cache::load_file( 'functions_hook', 'phpbb3' );
...
class mx_acp
{
	...
	function mx_acp()
	{
		global $template, $db, $lang, $phpEx, $mx_root_path;
		...
		if ( OLYMPUS_CATEGORY == PORTAL_BACKEND )
		{
	...
	function assign_tabs()
	{
		...
		switch ( PORTAL_BACKEND )
		{
			case PHPBB2X_BACKEND: // phpBB2 Backend TABS
				$this->tabs[] = array( // #
			...
			case OLYMPUS_BACKEND: // phpBB3 Backend TABS
		...
		{
			$this->template->assign_block_vars( 'tab', $tab );
		}
	}
	...
	{
		global $lang, $db, $mx_user, $mx_root_path, $phpbb_root_path, $phpEx;
		...
		while ( list( $category, $action_array ) = each( $menu_array ) )
		{
			...
			while ( list( $panel, $file ) = each( $action_array ) )
			{
				...
				$this->template->assign_block_vars( 'category.panel', array( "ROW_COLOR" => "#" . $row_color,
						"ROW_CLASS" => $row_class,
						// +MOD: DHTML Menu for ACP
						'ROW_COUNT' => $row_count,
						// -MOD: DHTML Menu for ACP
	...
	function display()
	{
		...
		$this->template->pparse( 'adm_header' );
		$this->template->pparse( 'adm_body' );
		$this->template->pparse( 'adm_footer' );
	}

	function _template_set_filenames()
	{
		$this->template->set_filenames( array( 'adm_header' => 'admin/mainpage_header.html',
	...
	function assign_content_acp( $acp_html )
	{
		...
		$this->template->assign_var( 'CONTENT_ACP', $acp_html );
	}

	function _template_assign_vars()
	{
		global $board_config, $userdata, $lang, $mx_user, $admincp_nav_icon_url;
		global $theme, $images, $mx_starttime, $db, $phpBB2, $phpEx;

		$l_timezone = explode( '.', $board_config['board_timezone'] );
		$l_timezone = ( count( $l_timezone ) > 1 && $l_timezone[count( $l_timezone )-1] != 0 ) ? $lang[sprintf( '%.1f', $board_config['board_timezone'] )] : $lang[number_format($mx_user->timezone)];
		// Generate stats
		...
		$this->template->assign_vars( $global_vars );
	}
}

$mx_acp = new mx_acp();
$mx_acp->decode_url( $_GET );

$mx_acp->assign_tabs();
$mx_acp->assign_menu();

//$cache_acp_file = "{$mx_root_path}cache"
