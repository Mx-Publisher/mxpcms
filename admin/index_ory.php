...
* @version $Id: index_ory.php,v 1.4 2013/06/28 15:32:37 orynider Exp $
...
class mx_acp
{
	...
	function assign_menu()
	{
		...
		while ( list( $category, $action_array ) = each( $menu_array ) )
		{
			...
			while ( list( $panel, $file ) = each( $action_array ) )
			{
				...
				$action = ( !empty($lang[$panel]) ) ? $lang[$panel] : preg_replace("/_/", " ", $panel);
				$module_link = mx_append_sid(PORTAL_URL . $file . ( ( strpos($file, '?') !== false ) ? '&sid=' : '?sid=' ) . $userdata['session_id']);
	...
	function _template_set_filenames()
	{
		...
		$template->set_filenames( array( 'adm_header' => 'admin/mainpage_header.tpl',
				'adm_body' => 'admin/mainpage_body.tpl',
				'adm_footer' => 'admin/mainpage_footer.tpl',
				) );
	}
