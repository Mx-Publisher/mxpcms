...
*
* @package Style
* @version $Id: session.php,v 1.1 2014/09/15 21:14:57 orynider Exp $
...
class session
{
	...
	function load()
	{
		...
		$session_logged_in = $this->data['session_logged_in'] = $this->data['user_id'] != ANONYMOUS ? 1 : 0;
		
		//print_r($session_logged_in);
	...
	*/
	...
	{
		global $phpEx, $SID, $_SID, $_EXTRA_URL, $db, $board_config, $mx_root_path, $phpbb_root_path, $page_id;
			...
			if (isset($this->data['user_id']))
			{
				...
				if ($u_ip === $s_ip && $s_browser === $u_browser && $s_forwarded_for === $u_forwarded_for)
				{
					...
					if ((@include_once $mx_root_path . "includes/shared/phpbb3/includes/auth/auth_" . $method . ".$phpEx") === false)
					{
						if ((@include_once $mx_root_path . "includes/shared/phpbb3/includes/auth/auth_" . $method . ".$phpEx") === false)
						{
							mx_message_die(CRITICAL_ERROR, 'File(s1) ' . $mx_root_path . "includes/shared/phpbb3/includes/auth/auth/auth_" . $method . ".$phpEx" . ' couldn\'t be opened.');
						}
					}
	...
	*/
	...
	{
		global $SID, $_SID, $db, $board_config, $cache, $mx_root_path, $phpbb_root_path, $phpEx, $mx_backend;
		...
		*/
		...
		if ((@include_once $mx_root_path . "includes/shared/phpbb3/includes/auth/auth_" . $method . ".$phpEx") === false)
		{
			if ((@include_once $mx_root_path . "includes/shared/phpbb3/includes/auth/auth_" . $method . ".$phpEx") === false)
			{
				mx_message_die(CRITICAL_ERROR, 'File(s2) ' . $mx_root_path . "includes/shared/phpbb3/includes/auth/auth_" . $method . ".$phpEx" . ' couldn\'t be opened.');
			}
		}		
	...
	*/
	...
	{
		global $SID, $_SID, $db, $board_config, $mx_root_path, $php_root_path, $phpEx;
		...
		if ((@include_once $mx_root_path . "includes/shared/phpbb3/includes/auth/auth_" . $method . ".$phpEx") === false)
		{
			if ((@include_once $mx_root_path . "includes/shared/phpbb3/includes/auth/auth_" . $method . ".$phpEx") === false)
			{
				mx_message_die(CRITICAL_ERROR, 'File(s3) ' . $mx_root_path . "includes/shared/phpbb3/includes/auth/auth_" . $method . ".$phpEx" . ' couldn\'t be opened.');
			}
		}
		//$provider = $phpbb_container->get('auth.provider.' . $method);
	...
	*/
	function images($img, $alt = '', $width = false, $suffix = '', $type = 'src')
	{
		...
		if (empty($img_data))
		{
			...
			$img_data['image_lang'] = $this->img_array[$img]['image_lang'];
			$img_data['src'] = (!empty($img_data['image_lang']) ? (PHPBB_URL . 'styles/' . $this->theme['imageset_path'] . '/theme/' . $img_data['image_lang'] . '/' . $this->img_array[$img]['image_filename']) : (PHPBB_URL . 'styles/' . $this->theme['imageset_path'] . '/theme/images/' . $this->img_array[$img]['image_filename']));
	...
	*/
	function img($img, $alt = '', $width = false, $suffix = '', $type = 'full_tag')
	{
		...
		if (empty($img_data))
		{
			...
			$img_data['src'] = (!empty($this->img_array[$img]['image_lang']) ? (PHPBB_URL . 'styles/' . $this->theme['imageset_path'] . '/theme/' . $this->img_array[$img]['image_lang'] . '/' . $this->img_array[$img]['image_filename']) : (PHPBB_URL . 'styles/' . $this->theme['imageset_path'] . '/theme/images/' . $this->img_array[$img]['image_filename']));
			$img_data['width'] = $this->img_array[$img]['image_width'];
			$img_data['height'] = $this->img_array[$img]['image_height'];
		}
