	...
	function bbcode_tpl($tpl_name, $bbcode_id = -1, $skip_bitfield_check = false)
	{
		...
		if (empty($bbcode_hardtpl))
		{
			...
			$bbcode_hardtpl = array(
				...
				'table' 		=> '<div class="forumbg forumbg-table tables1 {IDENTIFIER}" style="width: {NUMBER}%"><div class="inner"><table class="table1">{TEXT}</table></div></div>',
				'tr' 			=> '<tr class="{IDENTIFIER}">{TEXT}</tr>',
				'th' 			=> '<th class="{IDENTIFIER}" style="width: {NUMBER}%">{TEXT}</th>',
				'td' 			=> '<td class="{IDENTIFIER}" colspan="{NUMBER}">{TEXT}</td>',
				'thead' 		=> '<thead>{TEXT}</thead>',
				'tbody' 		=> '<tbody>{TEXT}</tbody>',
				'center' 		=> '<div style="width: 100%;text-align: center">{TEXT}</div>',
				'align' 		=> '<span style="text-align:{IDENTIFIER}; display:block;">{TEXT}</span>',
				'float' 		=> '<span style="float:{IDENTIFIER}">{TEXT}</span>',
				'fa' 			=> '<i class="icon fa-{TEXT} fa-fw" aria-hidden="true"></i>',				
			);
		}
	...
	*/
	function bbcode_tpl_replace($tpl_name, $tpl)
	{
		...
		static $replacements = array(
			...
			'img'					=> array('{WIDTH}'		=> '$1', '{HEIGHT}'			=> '$2', '{URL}'		=> '$3'),
			'scribd'				=> array('{URL}'		=> '$1'),
			'flash'					=> array('{WIDTH}'		=> '$1', '{HEIGHT}'			=> '$2', '{SCRIBDURL}'	=> '$3'),
			...
			'table'					=> array('{NUMBER}'		=> '$1', '{TEXT}'			=> '$2'),			
			'tr'					=> array('{TEXT}'		=> '$1'),
			'th'					=> array('{NUMBER}'		=> '$1', '{TEXT}'			=> '$2'),			
			'td'					=> array('{NUMBER}'		=> '$1', '{TEXT}'			=> '$2'),			
			'thead'					=> array('{TEXT}'		=> '$1'),
			'tbody'					=> array('{TEXT}'		=> '$1'),
			'center'				=> array('{TEXT}'		=> '$1'),
			'align'					=> array('{TEXT}'		=> '$1'),
					...
					if (isset($rowset[$bbcode_id]))
					{
						if ($this->template_bitfield->get($bbcode_id))
						{
							...
							else
							{
								...
								if (preg_match_all('/\{(URL|LOCAL_URL|WEB|IPAPERLINK|RAM|EMAIL|TEXT|SIMPLETEXT|IDENTIFIER|COLOR|NUMBER)[0-9]*\}/', $rowset[$bbcode_id]['bbcode_match'], $m))
								{
