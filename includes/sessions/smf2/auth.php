...
*
* @package Auth
* @version $Id: auth.php,v 1.1 2014/05/18 06:26:59 orynider Exp $
...
/**
* Permission/Auth class for SMF3 forums
...
/*					This is uncommented, because i suspect this being slightly wrong due to mixed permission classes being possible
	...
	*/
	...
	{
		global $board_config, $db, $mx_user, $smf_root_path, $phpEx;

		$method = trim(basename($board_config['auth_method']));
		include_once($smf_root_path . 'includes/auth/auth_' . $method . '.' . $phpEx);
		...
		if (function_exists($method))
		{
			...
			if ($login['status'] == LOGIN_SUCCESS_CREATE_PROFILE)
			{
				...
				{
					include($smf_root_path . 'includes/functions_user.' . $phpEx);
				}
			...
			{
				$old_session_id = $mx_user->session_id;
				...
				if ($admin)
				{
					...
					$mx_user->set_cookie('u', '', $cookie_expire);
					$mx_user->set_cookie('sid', '', $cookie_expire);
					unset($cookie_expire);

					$SID = '?sid=';
					$mx_user->session_id = $_SID = '';
				}

				$result = $mx_user->session_create($login['user_row']['user_id'], $admin, $autologin, $viewonline);
