...
class session
{
	...
	*/
	...
	function __construct()
	{
        ...
        //$this->browser = $this->setUserAgent($userAgent);
	...
	function load()
	{
		...
		//returns array($mobile_browser, $status);
		...
		*/
		...
		if ($this->isCrawler($this->browser) && ($this->data['is_bot'] == 1)) 
		{
			...
			else
			{
				...
				{
					// Register user...
					print('<p><span style="color: red;"></span></p>
					<i><p>Registering You as bot to the dabadase...!</p>
					</i>');
					...
					*/
					//$this->db->sql_return_on_error(true);
					$sql = 'INSERT INTO ' . BOTS_TABLE . ' ' . $this->db->sql_build_array('INSERT', $ary);
					if ((!$result = $this->db->sql_query($sql)))
					{
						$sql_error = $this->db->sql_error('');
						mx_message_die(GENERAL_ERROR, "Couldn\'t insert data into bots table " . BOTS_TABLE . ".", '', __LINE__, __FILE__,  $sql . '<br /><br />SQL Error : ' . $sql_error['code'] . ' ' . $sql_error['message']);
					}
					//$this->db->sql_return_on_error(false);
					/*
					*/
