...
*
* @package Auth
* @version $Id: core.php,v 1.19 2009/09/28 11:24:28 orynider Exp $
...
class mx_backend
{
	...
	function setup_backend()
	{
		...
		$server_name = 'caleacrestina.uv.ro';
		...
		$server_url = 'http://caleacrestina.uv.ro/';
		...
		$server_url_phpbb = 'http://caleacrestina.uv.ro/forum/';
	...
	*/
	...
	function generate_session_online_sql($guest = false)
	{
		...
		{
			$sql = "SELECT u.*, s.*, s.session_time AS user_session_time
				FROM " . USERS_TABLE . " u, " . SESSIONS_TABLE . " s
				WHERE u.user_id <> " . ANONYMOUS . "
					AND u.user_id = s.session_user_id
					AND s.session_time >= " . ( time() - 300 ) . "
	...
	*/
	...
	function phpbb_version_check()
	{
		...
		$minor_phpbb_revision = (int) $current_phpbb_version[3];
		...
		if ($fsock = @fsockopen('www.phpbb.com', 80, $errno, $errstr, 10))
		{
			...
			$latest_phpbb_head_revision = trim($phpbb_version_info[0]);
			...
			if ($latest_phpbb_head_revision == 3 && $minor_phpbb_revision == $latest_phpbb_minor_revision)
			{
