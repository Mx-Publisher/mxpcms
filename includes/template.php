...
* @version $Id: template.php,v 1.42 2024/04/10 06:13:17 orynider Exp $
* @copyright (c) 2002-2024 [CyberAlien, OryNider] MX-Publisher Project Team
...
class Template 
{
	...
	var $xs_versiontxt = '2.4.1-rc3'; // text version
	...
	public function assign_block_vars($blockname, array $vararray)
	{
		...
		if (strstr($blockname, '.'))
		{
			...
			for ($i = 0; $i < $blockcount; $i++)
			{
				...
				$blockcount = (null === $block) ? 0 : count($block) - 1;
				$index = (!$pos || strpos($blocks[$i], '[]') === $pos) ? $blockcount : (min((int) substr($blocks[$i], $pos + 1, -1), $blockcount));
				$block = &$block[$index];
				
				$str = &$str[$blocks[$i] . '.'];			
				$str = is_array($str) ? $str[sizeof($str) - 1] : $str; 				
			}
	...
	*/
	...
	function loadfile($handle)
	{
		...
		{
			$filename = $this->files[$handle] . 'index_'.$handle.'.html';
			//$str = @implode('', file($filename));
			$str = '';		
		}
	...
	*/	
	function compile_code($filename, $code, $use_isset = false)
	{
		...
		//Do not break: onMenuCatClick('0','menuPortal_');
		$code = str_replace("{{ ", "{", $code);
		$code = str_replace("') }}", "}", $code);
		$code = str_replace(" }}", "}", $code);
		...
		for($i = 0; $i < $line_count; $i++)
		{
			...
			if($pos2 !== false)
			{
				...
				if($pos3 !== false && $pos3 <= $pos2)
				{
					...
					elseif($keyword === 'INCLUDEPHP')
					{
						$keyword_type = XS_TAG_INCLUDEPHP;
					}
					elseif($keyword === 'INCLUDEJS')
					{
						$keyword_type = XS_TAG_INCLUDEJS;
					}
					elseif($keyword === 'INCLUDECSS')
					{
						$keyword_type = XS_TAG_INCLUDECSS;
					}
					elseif($keyword === 'IF')
					{
