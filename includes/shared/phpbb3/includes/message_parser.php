...
* @version $Id: message_parser.php,v 1.6 2008/09/07 18:53:53 orynider Exp $
...
{
	require($mx_root_path . 'includes/shared/phpbb3/includes/bbcode.' . $phpEx); //Tempfix for forum_hack
}
...
class bbcode_firstpass extends bbcode
{
		...
		foreach ($this->bbcodes as $bbcode_name => $bbcode_data)
		{
			if (isset($bbcode_data['disabled']) && $bbcode_data['disabled'])
			{
				foreach ($bbcode_data['regexp'] as $regexp => $replacement)
				{
					...
					{
						$this->warn_msg[] = sprintf($mx_user->lang['UNAUTHORISED_BBCODE'] , '[' . $bbcode_name . ']');
						continue;
					}
				}
			}
	...
	*/
	function bbcode_size($stx, $in)
	{
		...
		if ($board_config['max_' . $this->mode . '_font_size'] && $board_config['max_' . $this->mode . '_font_size'] < $stx)
		{
			$this->warn_msg[] = sprintf($mx_user->lang['MAX_FONT_SIZE_EXCEEDED'], $board_config['max_' . $this->mode . '_font_size']);
	...
	*/
	function bbcode_img($in)
	{
		...
		if ($board_config['max_' . $this->mode . '_img_height'] || $board_config['max_' . $this->mode . '_img_width'])
		{
			...
			{
				$error = true;
				$this->warn_msg[] = $mx_user->lang['UNABLE_GET_IMAGE_SIZE'];
			}
			else
			{
				if ($board_config['max_' . $this->mode . '_img_height'] && $board_config['max_' . $this->mode . '_img_height'] < $stats[1])
				{
					$error = true;
					$this->warn_msg[] = sprintf($mx_user->lang['MAX_IMG_HEIGHT_EXCEEDED'], $board_config['max_' . $this->mode . '_img_height']);
				}

				if ($board_config['max_' . $this->mode . '_img_width'] && $board_config['max_' . $this->mode . '_img_width'] < $stats[0])
				{
					$error = true;
					$this->warn_msg[] = sprintf($mx_user->lang['MAX_IMG_WIDTH_EXCEEDED'], $board_config['max_' . $this->mode . '_img_width']);
				}
			}
		}
	...
	*/
	function bbcode_flash($width, $height, $in)
	{
		...
		if ($board_config['max_' . $this->mode . '_img_height'] || $board_config['max_' . $this->mode . '_img_width'])
		{
			if ($board_config['max_' . $this->mode . '_img_height'] && $board_config['max_' . $this->mode . '_img_height'] < $height)
			{
				$error = true;
				$this->warn_msg[] = sprintf($mx_user->lang['MAX_FLASH_HEIGHT_EXCEEDED'], $board_config['max_' . $this->mode . '_img_height']);
			}

			if ($board_config['max_' . $this->mode . '_img_width'] && $board_config['max_' . $this->mode . '_img_width'] < $width)
			{
				$error = true;
				$this->warn_msg[] = sprintf($mx_user->lang['MAX_FLASH_WIDTH_EXCEEDED'], $board_config['max_' . $this->mode . '_img_width']);
			}
		}
		...
		do
		{
			...
			if ($tok == ']')
			{
				...
				else if (preg_match('#^quote(?:=&quot;(.*?)&quot;)?$#is', $buffer, $m))
				{
					...
					if ($board_config['max_quote_depth'] && sizeof($close_tags) >= $board_config['max_quote_depth'])
					{
						// there are too many nested quotes
						$error_ary['quote_depth'] = sprintf($mx_user->lang['QUOTE_DEPTH_EXCEEDED'], $board_config['max_quote_depth']);
					...
					if (isset($m[1]) && $m[1])
					{
						$mx_username = str_replace(array('&#91;', '&#93;'), array('[', ']'), $m[1]);
						$mx_username = preg_replace('#\[(?!b|i|u|color|url|email|/b|/i|/u|/color|/url|/email)#iU', '&#91;$1', $mx_username);

						$end_tags = array();
						$error = false;

						preg_match_all('#\[((?:/)?(?:[a-z]+))#i', $mx_username, $tags);
						...
						$out .= 'quote=&quot;' . $mx_username . '&quot;:' . $this->bbcode_uid . ']';
					}
					else
					{
...
*/
	...
	*/
	...
	{
		global $board_config, $phpEx, $mx_user;

		if ($board_config['force_server_vars'])
		{
			$check_path = $board_config['script_path'];
		}
		else
		{
			$check_path = ($mx_user->page['root_script_path'] != '/') ? substr($mx_user->page['root_script_path'], 0, -1) : '/';
		}
...
class parse_message extends bbcode_firstpass
{
	...
	*/
	function parse($allow_bbcode, $allow_magic_url, $allow_smilies, $allow_img_bbcode = true, $allow_flash_bbcode = true, $allow_quote_bbcode = true, $allow_url_bbcode = true, $update_this_message = true, $mode = 'post')
	{
		...
		if ($board_config['max_' . $mode . '_chars'] > 0)
		{
			$msg_len = ($mode == 'post') ? utf8_strlen($this->message) : utf8_strlen(preg_replace('#\[\/?[a-z\*\+\-]+(=[\S]+)?\]#ius', ' ', $this->message));

			if ((!$msg_len && $mode !== 'sig') || $board_config['max_' . $mode . '_chars'] && $msg_len > $board_config['max_' . $mode . '_chars'])
			{
				$this->warn_msg[] = (!$msg_len) ? $mx_user->lang['TOO_FEW_CHARS'] : sprintf($mx_user->lang['TOO_MANY_CHARS_' . strtoupper($mode)], $msg_len, $board_config['max_' . $mode . '_chars']);
		...
		{
			$this->warn_msg[] = $mx_user->lang['TOO_FEW_CHARS'];
			return $this->warn_msg;
		}
		...
		if ($allow_smilies)
		{
			$this->smilies($board_config['max_' . $mode . '_smilies']);
		}
		...
		if ($board_config['max_' . $mode . '_urls'] && $num_urls > $board_config['max_' . $mode . '_urls'])
		{
			$this->warn_msg[] = sprintf($mx_user->lang['TOO_MANY_URLS'], $board_config['max_' . $mode . '_urls']);
			return $this->warn_msg;
		}
	...
	*/
	...
	{
		global $board_config, $phpbb_auth, $mx_user, $phpbb_root_path, $phpEx, $db;
		...
		$cfg = array();
		$cfg['max_attachments'] = ($is_message) ? $board_config['max_attachments_pm'] : $board_config['max_attachments'];
		$forum_id = ($is_message) ? 0 : $forum_id;

		if ($submit && in_array($mode, array('post', 'reply', 'quote', 'edit')) && $upload_file)
		{
			if ($num_attachments < $cfg['max_attachments'] || $phpbb_auth->acl_get('a_') || $phpbb_auth->acl_get('m_', $forum_id))
			{
				...
				if ($filedata['post_attach'] && !sizeof($error))
				{
					$sql_ary = array(
						...
						'poster_id'			=> $mx_user->data['user_id'],
					);
			...
			{
				$error[] = sprintf($mx_user->lang['TOO_MANY_ATTACHMENTS'], $cfg['max_attachments']);
			}
		}
		...
		if ($preview || $refresh || sizeof($error))
		{
			...
			{
				if ($num_attachments < $cfg['max_attachments'] || $phpbb_auth->acl_gets('m_', 'a_', $forum_id))
				{
					$filedata = upload_attachment($form_name, $forum_id, false, '', $is_message);
	...
	*/
	function get_submitted_attachment_data($check_user_id = false)
	{
		...
		$check_user_id = ($check_user_id === false) ? $mx_user->data['user_id'] : $check_user_id;
	...
	*/
	...
	{
		global $phpbb_auth, $mx_user, $board_config;
		...
		$poll['poll_option_text'] = $this->parse($poll['enable_bbcode'], ($board_config['allow_post_links']) ? $poll['enable_urls'] : false, $poll['enable_smilies'], $poll['img_status'], false, false, $board_config['allow_post_links'], false);
		...
		{
			$this->warn_msg[] = $mx_user->lang['NO_POLL_TITLE'];
		}
		else
		{
			if (utf8_strlen(preg_replace('#\[\/?[a-z\*\+\-]+(=[\S]+)?\]#ius', ' ', $this->message)) > 100)
			{
				$this->warn_msg[] = $mx_user->lang['POLL_TITLE_TOO_LONG'];
			}
			$poll['poll_title'] = $this->parse($poll['enable_bbcode'], ($board_config['allow_post_links']) ? $poll['enable_urls'] : false, $poll['enable_smilies'], $poll['img_status'], false, false, $board_config['allow_post_links'], false);
			if (strlen($poll['poll_title']) > 255)
			{
				$this->warn_msg[] = $mx_user->lang['POLL_TITLE_COMP_TOO_LONG'];
			}
		}
		...
		{
			$this->warn_msg[] = $mx_user->lang['TOO_FEW_POLL_OPTIONS'];
		}
		else if ($poll['poll_options_size'] > (int) $board_config['max_poll_options'])
		{
			$this->warn_msg[] = $mx_user->lang['TOO_MANY_POLL_OPTIONS'];
		}
		else if ($poll_max_options > $poll['poll_options_size'])
		{
			$this->warn_msg[] = $mx_user->lang['TOO_MANY_USER_OPTIONS'];
		}

		$poll['poll_max_options'] = ($poll['poll_max_options'] < 1) ? 1 : (($poll['poll_max_options'] > $board_config['max_poll_options']) ? $board_config['max_poll_options'] : $poll['poll_max_options']);
	}
}
