...
*
* @package phpBB3
* @version $Id: functions_posting.php,v 1.11 2009/07/28 20:27:57 orynider Exp $
...
{
	global $phpbb_auth, $db, $mx_user, $config, $template;
	...
	if ($mode == 'window')
	{
		...
		page_header($mx_user->lang['SMILIES']);
	...
	if ($mode == 'inline' && $display_link)
	{
		$template->assign_vars(array(
			...
			'U_MORE_SMILIES' 		=> mx3_append_sid("{$phpbb_root_path}posting.$phpEx", 'mode=smilies&amp;f=' . $forum_id))
		);
	}
...
{
	global $phpbb_root_path, $config, $template, $mx_cache;
...
{
	global $phpbb_auth, $mx_user, $template, $topic_type;
	...
	foreach ($topic_types as $phpbb_auth_key => $topic_value)
	{
		// We do not have a special post global announcement permission
		$phpbb_auth_key = ($phpbb_auth_key == 'global') ? 'announce' : $phpbb_auth_key;

		if ($phpbb_auth->acl_get('f_' . $phpbb_auth_key, $forum_id))
		{
			$toggle = true;
			...
			$topic_type_array[] = array(
				...
				'L_TOPIC_TYPE'	=> $mx_user->lang[$topic_value['lang']]
			);
		}
	}
	...
	if ($toggle)
	{
		$topic_type_array = array_merge(array(0 => array(
			...
			'L_TOPIC_TYPE'	=> $mx_user->lang['POST_NORMAL'])),
		...
		$template->assign_vars(array(
			'S_TOPIC_TYPE_STICKY'	=> ($phpbb_auth->acl_get('f_sticky', $forum_id)),
			'S_TOPIC_TYPE_ANNOUNCE'	=> ($phpbb_auth->acl_get('f_announce', $forum_id)))
		);
	}
...
{
	global $phpbb_auth, $mx_user, $config, $db, $mx_cache, $mx_backend;
	...
	{
		$filedata['error'][] = $mx_user->lang['NO_UPLOAD_FORM_FOUND'];
		return $filedata;
	}

	$extensions = $mx_backend->obtain_attach_extensions((($is_message) ? false : (int) $forum_id));
	...
	if ($cat_id == ATTACHMENT_CATEGORY_IMAGE && !$file->is_image())
	{
		...
		trigger_error($mx_user->lang['ATTACHED_IMAGE_NOT_IMAGE']);
	}
	...
	if (!$phpbb_auth->acl_get('a_') && !$phpbb_auth->acl_get('m_', $forum_id) && $cat_id == ATTACHMENT_CATEGORY_IMAGE)
	{
		$file->upload->set_allowed_dimensions(0, 0, $config['img_max_width'], $config['img_max_height']);
	}

	// Admins and mods are allowed to exceed the allowed filesize
	if (!$phpbb_auth->acl_get('a_') && !$phpbb_auth->acl_get('m_', $forum_id))
	{
	...
	$file->clean_filename('unique', $mx_user->data['user_id'] . '_');
...
{
	global $template, $config, $phpbb_root_path, $phpEx, $mx_user;
	...
	if (sizeof($attachment_data))
	{
		...
		foreach ($attachment_data as $count => $attach_row)
		{
			...
			$download_link = mx3_append_sid("{$phpbb_root_path}download/file.$phpEx", 'mode=view&amp;id=' . (int) $attach_row['attach_id'], true, ($attach_row['is_orphan']) ? $mx_user->session_id : false);
...
{
	global $mx_user, $db, $template, $phpbb_auth;
	global $phpbb_root_path, $phpEx;
	...
	$sql = 'SELECT d.*, f.forum_id, f.forum_name
		...
		LEFT JOIN ' . FORUMS_TABLE . ' f ON (f.forum_id = d.forum_id)
			WHERE d.user_id = ' . $mx_user->data['user_id'] . "
			$sql_and
	...
	foreach ($draft_rows as $draft)
	{
		...
		if (isset($topic_rows[$draft['topic_id']])
			&& (
				($topic_rows[$draft['topic_id']]['forum_id'] && $phpbb_auth->acl_get('f_read', $topic_rows[$draft['topic_id']]['forum_id']))
				||
				(!$topic_rows[$draft['topic_id']]['forum_id'] && $phpbb_auth->acl_getf_global('f_read'))
			))
		{
			$topic_forum_id = ($topic_rows[$draft['topic_id']]['forum_id']) ? $topic_rows[$draft['topic_id']]['forum_id'] : $forum_id;

			$link_topic = true;
			$view_url = mx3_append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $topic_forum_id . '&amp;t=' . $draft['topic_id']);
			$title = $topic_rows[$draft['topic_id']]['topic_title'];

			$insert_url = mx3_append_sid("{$phpbb_root_path}posting.$phpEx", 'f=' . $topic_forum_id . '&amp;t=' . $draft['topic_id'] . '&amp;mode=reply&amp;d=' . $draft['draft_id']);
		}
		else if ($draft['forum_id'] && $phpbb_auth->acl_get('f_read', $draft['forum_id']))
		{
			$link_forum = true;
			$view_url = mx3_append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $draft['forum_id']);
			$title = $draft['forum_name'];

			$insert_url = mx3_append_sid("{$phpbb_root_path}posting.$phpEx", 'f=' . $draft['forum_id'] . '&amp;mode=post&amp;d=' . $draft['draft_id']);
		}
		else
		{
			// Either display as PM draft if forum_id and topic_id are empty or if access to the forums has been denied afterwards...
			$link_pm = true;
			$insert_url = mx3_append_sid("{$phpbb_root_path}ucp.$phpEx", "i=$id&amp;mode=compose&amp;d={$draft['draft_id']}");
		}

		$template->assign_block_vars('draftrow', array(
			'DRAFT_ID'		=> $draft['draft_id'],
			'DATE'			=> $mx_user->format_date($draft['save_time']),
...
{
	global $mx_user, $phpbb_auth, $db, $template, $bbcode, $mx_cache, $mx_backend;
	...
	$sql = 'SELECT p.post_id
		...
		WHERE p.topic_id = $topic_id
			" . ((!$phpbb_auth->acl_get('m_approve', $forum_id)) ? 'AND p.post_approved = 1' : '') . '
	...
	if ($has_attachments && $phpbb_auth->acl_get('u_download') && $phpbb_auth->acl_get('f_download', $forum_id))
	{
	...
	for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
	{
		...
		$template->assign_block_vars($mode . '_row', array(
			...
			'MINI_POST_IMG'		=> $mx_user->img('icon_post_target', $mx_user->lang['POST']),
			'POST_DATE'			=> $mx_user->format_date($row['post_time']),
			'MESSAGE'			=> $message,
	...
	{
		$template->assign_var('QUOTE_IMG', $mx_user->img('icon_post_quote', $mx_user->lang['REPLY_WITH_QUOTE']));
	}
...
{
	global $db, $mx_user, $config, $phpbb_root_path, $phpEx, $phpbb_auth;
	...
	$sql_ignore_users = ANONYMOUS . ', ' . $mx_user->data['user_id'];
	while ($row = $db->sql_fetchrow($result))
	{
	...
	foreach ($phpbb_auth->acl_get_list(array_keys($notify_rows), 'f_read', $forum_id) as $forum_id => $forum_ary)
	{
		foreach ($forum_ary as $phpbb_auth_option => $mx_user_ary)
		{
			foreach ($mx_user_ary as $mx_user_id)
			{
				$notify_rows[$mx_user_id]['allowed'] = true;
			}
		}
	}
	...
	foreach ($notify_rows as $mx_user_id => $row)
	{
		...
		case 'delete_last_post':
			...
			else
			{
				$sql = 'SELECT MAX(post_id) as last_post_id
					FROM ' . POSTS_TABLE . "
					WHERE topic_id = $topic_id " .
						((!$phpbb_auth->acl_get('m_approve', $forum_id)) ? 'AND post_approved = 1' : '');
				$result = $db->sql_query($sql);
				$row = $db->sql_fetchrow($result);
				$db->sql_freeresult($result);
...
* Submit Post
*/
function submit_post($mode, $subject, $mx_username, $topic_type, &$poll, &$data, $update_message = true)
{
