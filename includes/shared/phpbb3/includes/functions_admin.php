...
*
* @package acp
* @version $Id: functions_admin.php,v 1.7 2008/10/31 18:55:14 jonohlsson Exp $
...
function make_forum_select($select_id = false, $ignore_id = false, $ignore_acl = false, $ignore_nonpost = false, $ignore_emptycat = true, $only_acl_post = false, $return_array = false)
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		if ($acl && !$phpbb_auth->acl_gets($acl, $row['forum_id']))
		{
			// List permission?
...
function size_select_options($size_compare)
{
	...
	$size_types_text = array($mx_user->lang['BYTES'], $mx_user->lang['KIB'], $mx_user->lang['MIB']);
...
function group_select_options($group_id, $exclude_ids = false, $manage_founder = false)
{
	...
	$sql_and = (!$board_config['coppa_enable']) ? (($exclude_sql) ? ' AND ' : ' WHERE ') . "group_name <> 'REGISTERED_COPPA'" : '';
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$s_group_options .= '<option' . (($row['group_type'] == GROUP_SPECIAL) ? ' class="sep"' : '') . ' value="' . $row['group_id'] . '"' . $selected . '>' . (($row['group_type'] == GROUP_SPECIAL) ? $mx_user->lang['G_' . $row['group_name']] : $row['group_name']) . '</option>';
	}
	$db->sql_freeresult($result);
...
function get_forum_list($acl_list = 'f_list', $id_only = true, $postable_only = false, $no_cache = false)
{
	...
	foreach ($forum_rows as $row)
	{
		...
		if ($acl_list == '' || ($acl_list != '' && $phpbb_auth->acl_gets($acl_list, $row['forum_id'])))
		{
...
function delete_topics($where_type, $where_ids, $auto_sync = true, $post_count_sync = true, $call_delete_posts = true)
{
	...
	if ($approved_topics)
	{
		set_config('num_topics', $board_config['num_topics'] - $approved_topics, true);
	}
...
function delete_posts($where_type, $where_ids, $auto_sync = true, $posted_sync = true, $post_count_sync = true, $call_delete_topics = true)
{
	...
	$search_type = basename($board_config['search_type']);
	...
	if ($approved_posts)
	{
		set_config('num_posts', $board_config['num_posts'] - $approved_posts, true);
	}
...
function delete_attachments($mode, $ids, $resync = true)
{
	...
	set_config('upload_dir_size', $board_config['upload_dir_size'] - $space_removed, true);
	set_config('num_files', $board_config['num_files'] - $files_removed, true);
...
function update_posted_info(&$topic_ids)
{
	...
	if (empty($topic_ids) || !$board_config['load_db_track'])
	{
		return;
	}
...
function phpbb_unlink($filename, $mode = 'file', $entry_removed = false)
{
	...
	return @unlink($phpbb_root_path . $board_config['upload_path'] . '/' . $filename);
}
...
{
	global $db, $mx_cache, $phpbb_auth, $phpbb_root_path, $phpEx;
	...
	$hold_ary = $phpbb_auth->acl_user_raw_data(false, 'm_%', false);
	...
	if (sizeof($hold_ary))
	{
		...
		if (sizeof($hold_ary))
		{
			...
			foreach ($hold_ary as $user_id => $forum_id_ary)
			{
				...
				foreach ($forum_id_ary as $forum_id => $phpbb_auth_ary)
				{
					$sql_ary[] = array(
	...
	// Now to the groups...
	$hold_ary = $phpbb_auth->acl_group_raw_data(false, 'm_%', false);
...
{
	global $db, $mx_user, $phpbb_auth, $phpEx, $phpbb_root_path, $phpbb_admin_path;

	$topic_id_list = $reportee_id_list = $is_auth = $is_mod = array();

	$profile_url = (defined('IN_ADMIN')) ? mx3_append_sid("{$phpbb_admin_path}index.$phpEx", 'i=users&amp;mode=overview') : mx3_append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=viewprofile');
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$log[$i] = array(
			...
			'username_full'		=> mx_get_username_string('full', $row['user_id'], $row['username'], $row['user_colour'], false, $profile_url),
			...
			'viewforum'			=> ($row['forum_id'] && $phpbb_auth->acl_get('f_read', $row['forum_id'])) ? mx3_append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $row['forum_id']) : false,
			'action'			=> (isset($mx_user->lang[$row['log_operation']])) ? $mx_user->lang[$row['log_operation']] : '{' . ucfirst(str_replace('_', ' ', $row['log_operation'])) . '}',
		);
		...
		if (!empty($row['log_data']))
		{
			...
			if (isset($mx_user->lang[$row['log_operation']]))
			{
	...
	if (sizeof($topic_id_list))
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			if (!$row['forum_id'])
			{
				if ($phpbb_auth->acl_getf_global('f_read'))
				{
					if (!$default_forum_id)
					{
						$sql = 'SELECT forum_id
			...
			if ($phpbb_auth->acl_gets('a_', 'm_', $row['forum_id']))
			{
		...
		{
			$log[$key]['viewtopic'] = (isset($is_auth[$row['topic_id']])) ? mx3_append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $is_auth[$row['topic_id']] . '&amp;t=' . $row['topic_id']) : false;
			$log[$key]['viewlogs'] = (isset($is_mod[$row['topic_id']])) ? mx3_append_sid("{$phpbb_root_path}mcp.$phpEx", 'i=logs&amp;mode=topic_logs&amp;t=' . $row['topic_id'], true, $mx_user->session_id) : false;
		}
	}
	...
	if (sizeof($reportee_id_list))
	{
		...
		foreach ($log as $key => $row)
		{
			...
			$log[$key]['reportee_username_full'] = mx_get_username_string('full', $row['reportee_id'], $reportee_names_list[$row['reportee_id']]['username'], $reportee_names_list[$row['reportee_id']]['user_colour'], false, $profile_url);
		}
	}
...
function update_foes($group_id = false, $user_id = false)
{
	...
	$perms = array();
	foreach ($phpbb_auth->acl_get_list(false, array('a_', 'm_'), false) as $forum_id => $forum_ary)
	{
		foreach ($forum_ary as $phpbb_auth_option => $user_ary)
		{
...
function view_inactive_users(&$users, &$user_count, $limit = 0, $offset = 0, $limit_days = 0, $sort_by = 'user_inactive_time DESC')
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		$row['inactive_reason'] = $mx_user->lang['INACTIVE_REASON_UNKNOWN'];
		switch ($row['user_inactive_reason'])
		{
			case INACTIVE_REGISTER:
				$row['inactive_reason'] = $mx_user->lang['INACTIVE_REASON_REGISTER'];
			break;

			case INACTIVE_PROFILE:
				$row['inactive_reason'] = $mx_user->lang['INACTIVE_REASON_PROFILE'];
			break;

			case INACTIVE_MANUAL:
				$row['inactive_reason'] = $mx_user->lang['INACTIVE_REASON_MANUAL'];
			break;

			case INACTIVE_REMIND:
				$row['inactive_reason'] = $mx_user->lang['INACTIVE_REASON_REMIND'];
			break;
		}
...
{
	global $db, $mx_user, $table_prefix;
	...
	$database_size = ($database_size !== false) ? get_formatted_filesize($database_size) : $mx_user->lang['NOT_AVAILABLE'];
...
function tidy_warnings()
{
	...
	$expire_date = time() - ($board_config['warnings_expire_days'] * 86400);
...
function add_permission_language()
{
	...
	$mx_user->add_lang('acp/permissions_phpbb');
	...
	{
		$dh = @opendir($mx_user->lang_path . $path);
