...
class version_helper
{
	...
	/** @var \mxp\cache\service */
	protected $mx_cache;

	/** @var \mxp\config\config */
	protected $board_config;

	/** @var \mxp\includes\shared\phpbb3\includes\file_downloader */
	...
	public function __construct()
	{
		...
		$mx_cache->load_file('file_downloader', 'phpbb3');
		...
		$this->current_version = $board_config['version'];
	}
	...
	{
		return mx_version_compare($version1, $version2, $operator);
	}
		...
		else if ($info === false || $force_update)
		{
			...
			if (!empty($info))
			{
				$json_sanitizer = function (&$value, $key) {
					$type_cast_helper = new mx_request_vars();
			...
			foreach ($versions_data as $branch => &$version_data)
			{
				...
				foreach ($version_data as $key => &$value)
				{
					...
					switch ($this->version_schema[$stability_type][$key])
					{
						...
						case 'url':
							if (!empty($value) && !preg_match('#^' . $phpBB3->get_preg_expression('url') . '$#iu', $value) &&
								!preg_match('#^' . $phpBB3->get_preg_expression('www_url') . '$#iu', $value))
							{
						...
						case 'version':
							if (!empty($value) && !preg_match($phpBB3->get_preg_expression('semantic_version'), $value))
							{
