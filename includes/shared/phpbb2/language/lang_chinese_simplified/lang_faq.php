...
$faq[] = array("为什么我需要注册?", "您可以不注册,是否注册后才可以发表文章是由论坛管理员决定的. 无论如何注册将给您提供不对游客提供的功能.例如：头像功能、私人留言服务、发送电子信件给朋友、加入成员组等等…… 注册仅仅花费您一点点的时间,我们推荐您这样做.");
...
$faq[] = array("--","发帖问题");
$faq[] = array("我怎样可以在论坛中发表新主题?", "非常简单，您可以通过论坛或主题页面上的相关按钮发表一个新的主题，在您发表任何信息以前，您可能需要先注册。您可以用到的论坛功能会在论坛页面的底部列出(如您可以发表新主题,您可以发起新民意调查等等)");
...
$faq[] = array("怎样发起一次民意调查?", "发起民意调查的方法非常简单,当您发表一个新的主题时(或者编辑主题的首个帖子,假如您有这样的权限)您会在文章发表栏的下方看到[添加民意调查](如果您不能看见那么您或许没有权限发起民意调查).您应该为民意调查定义一个标题然后至少二个选项,设定民意调查的选项类型并且点一下[添加选项]按钮,选项的数目会有限制,由管理员决定.您也能设定一个投票期限.0 代表无限期的民意调查");
$faq[] = array("怎样编辑或删除民意调查?", "和文章一样,民意调查只能被最初的发起人,长老(论坛管理员)或版主编辑. 要编辑民意调查请点击该调查主题的第一个帖子(它总是和民意调查关联的).如果没有人投过票,该民意调查的发起人能够删除投票或者编辑任何调查选择,但是如果已经有投票了,那么就只有版主或者管理员能够编辑或者删除它.");
$faq[] = array("为什么我不能访问某个论坛?", "有些论坛可能会限制用户和成员组去查看,阅读和发表等.您可能需要特别的授权,版主和管理员才能赋予访问权限,您可以与他们联系.");
$faq[] = array("为什么我不能参加民意调查?", "只有注册会员可以民意调查(为了避免欺骗结果).假如您已经注册仍然不能投票那么或许您没有适当的权限.");
...
$faq[] = array("什么是锁定的主题?", "管理员或版主有时会将某主题锁定,您不能回复该锁定的主题,它所包含的任何民意调查会被自动结束.主题可能会被为许多原因锁定.");


$faq[] = array("--","用户级别和成员组");
$faq[] = array("什么是管理员?", "管理员是这个论坛的最高控制人,他们拥有控制论坛的一切权限,包括禁止用户,建立成员组,管理论坛板块等,在所有的论坛中也有完整的版主管理能力.");
$faq[] = array("什么是长老?", "长老是独立的(或是成员组),他们可以编辑或删除帖子,并且可以在他们所管理的版面中锁定,解锁,转移,删除或者分割主题.长老会阻止会员发表错误,离题或有争议的内容.");
$faq[] = array("什么是成员组?", "成员组是一个论坛管理员分祖管理会员的方式.(这可能不同于其他的论坛),而且每个成员组能被分配个别的权限.这使得管理员容易去建立几个类似长老功能的用户,或是限定成员组成员进入限定版面等等.");
$faq[] = array("怎样加入一个成员组?", "要加入一个成员组请点击页面上方(不同的模板设计位置可能不同)的成员组连接,您可以预览所有的成员组,但不是所有的成员组都是开放访问的,一些会是关闭的,一些可能是隐藏成员组,如果成员组是开放的,您可以按申请钮请求加入.您将需要得到该成员组的组长同意您的请求才可以加入,他们可能会问为什么您想要参加成员组.如果请求不幸被驳回,请不要一直重复申请.");
$faq[] = array("我怎样才能成为组长?", "成员组是由论坛的管理员建立的,同时他们也会分配一位组长. 如果您有兴趣创建一个成员组首先应该联系管理员,尝试给他们发一封邮件吧.");


$faq[] = array("--","私人留言 (PM)");
$faq[] = array("我不能发送私人留言!", "有3个可能的原因:您还没有注册/登陆.这个论坛的管理员已经设定整个论坛的站内信件功能为失效.或者管理员禁止您使用站内信件功能.如果是最后面的情况,您可以联络管理者.");
$faq[] = array("我经常收到没用的留言!", "以后我们会把一个忽视列表功能加入到私人留言系统.如果现在您持续收到从某人发来的不必要的信件请告知论坛管理员,他们有能力阻止一个用户乱发站内信件.");
