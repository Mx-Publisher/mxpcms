...
$faq[] = array("要如何在我的帳號下秀出個人圖像?", "在觀看文章時可能有兩個圖像在帳號下. 首先的圖像是關於您的 \"群組&身份\", 一般而言,這些會是版面上的星星或者是區塊, 用來顯示您已發表了多少篇文章, 或是您在版面上的某些 \"狀態\". 在此之下, 或許是一個很大的個人圖像, 一般而言這圖像是獨一無二的或可以代表個人的. 這將取決於系統管理員, 是否有啟動這個圖像功能, 而且他們具有選擇圖像限制的權利. 如果您無法使用個人圖像, 這是系統管理員所決定的, 所以您可以詢間他們原因為何? (我們相信他們會為您解釋的!)");
$faq[] = array("要如何改變我的等級?", "基本上您無法直接更改任何有關階級身份的顯示文字 (階級身份將在您所發表的文章標題的帳號下顯示出來, 並且您的個人資料將使用此身份風格). 許多討論版使用階級身份功能去指出哪些文章是您所發表的, 並且鑑定您是否為可疑的使用者, 例如: 版主跟討論區管理者都有個特別的身份. 請不要為了提升您的身份而胡亂張貼文章, 您將會因此而看到版主或討論版管理人員將您的文章標上警告標語.");
...
$faq[] = array("什麼是公告?", "公告通常包含重要的訊息, 您必須儘可能的去閱讀所有的公告. 公告可以在每個版面的最上方找到. 您要發佈公告則視您的權限程度, 這也是取決於系統管理員.");
...
$faq[] = array("什麼是會員群組?", "會員群組是討論版管理員分組管理會員的一種方法 (這是和其它許多討論版不同的地方) 而且每個群組可以設定不同的權力. 這可以讓系統管理員更容易的去建立數個類似版主功能的帳號管理者, 或是讓限定群組進入限製版面等等.");
...
$faq[] = array("誰寫了這個討論版?", "這個程式 (未經修改的格式) 由 phpBB Group 所開發及釋出, 著作版權歸 <a href=\"http://www.phpbb.com/\" target=\"_blank\">phpBB Group</a> 所有. 依照「革奴大眾公有版權」(GNU General Public License) 的聲明, 這個程式可以自由的使用及散佈, 如果您需要更多的資訊可以參考 <a href=\"http://www.gnu.org/copyleft/gpl.html\" target=\"_blank\">GNU General Public License</a>.");
