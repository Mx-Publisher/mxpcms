...
{
	// Initialize template
	$layouttemplate = new mx_Template($mx_root_path . 'templates/'. $mx_user->theme['template_name'], $board_config, $db);
}
...
switch (PORTAL_BACKEND)
{
	...
	case 'mybb':
	case 'phpbb2':
		$admin = ($userdata['session_logged_in'] && $userdata['user_level'] == ADMIN) ? true : false;
		break;
...
$layouttemplate->assign_vars(array(
	...
	'S_USER_LANG' => isset($lang['USER_LANG']) ? $lang['USER_LANG'] : $mx_user->encode_lang($board_config['default_lang']),
	...
	'S_BOARD_DISABLED'		=> ($board_config['portal_status']) ? true : false,
	...
	'T_PHPBB_STYLESHEET' => $mx_user->theme['head_stylesheet'],
	'T_STYLESHEET_LINK'	=> (!isset($mx_user->theme['theme_storedb'])) ? "{$phpbb_root_path}styles/" . $mx_user->template_name . '/theme/stylesheet.css' : "{$phpbb_root_path}style.$phpEx?sid=$mx_user->session_id&amp;id=" . $mx_user->theme['style_id'] . '&amp;lang=' . $mx_user->encode_lang($board_config['default_lang']),
...
if ($mx_page->auth_view || $mx_page->auth_mod)
{
	...
	if(!empty($block_id) )
	{
		...
		$template = new mx_Template($mx_root_path . 'templates/'. $mx_user->theme['template_name']);
