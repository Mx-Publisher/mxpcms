...
*
* @package dbal
* @version $Id: db_tools.php,v 1.1 2013/06/23 21:46:50 orynider Exp $
...
class tools  
{
	...
	* @param bool		$return_statements	True if only statements should be returned and no SQL being executed
	*/
	function mx_db_tools(&$db, $return_statements = false)
	{
		$this->db = $db;
		$this->return_statements = $return_statements;
	...
	function sql_column_add($table_name, $column_name, $column_data, $inline = false)
	{
		...
		switch ($this->sql_layer)
		{
			...
			case 'sqlite':
				...
				{
					return $column_name . ' ' . (!empty($column_data[$column_name]) ? $column_data[$column_name] : $column_data['column_type_sql']);
				}
			...
			case 'sqlite3':
				...
				$statements[] = 'ALTER TABLE ' . $table_name . ' ADD ' . $column_name . ' ' . (!empty($column_data[$column_name]) ? $column_data[$column_name] : $column_data['column_type_sql']);
			break;
		}
...
* @note currently not used within phpBB3, but may be utilized later.
*/
class mx_db_tools extends tools
{
	/**
	* Current sql layer
	*/
