...
if( isset($HTTP_POST_VARS['add_name']) )
{
	...
	if ($disallowed_user == '')
	{
		mx_message_die(MESSAGE, $lang['Fields_empty']);
	}
	...
	else
	{
		...
		if ( !$result )
		{
			mx_message_die(GENERAL_ERROR, "Could not add disallowed user.", "",__LINE__, __FILE__, $sql);
		}
...
else if( isset($HTTP_POST_VARS['delete_name']) )
{
	...
	if( !$result )
	{
		mx_message_die(GENERAL_ERROR, "Couldn't removed disallowed user.", "",__LINE__, __FILE__, $sql);
	}
...
if( !$result )
{
	mx_message_die(GENERAL_ERROR, "Couldn't get disallowed users.", "", __LINE__, __FILE__, $sql );
}
