...
*
* @package page_header
* @version $Id: page_header.php,v 1.2 2009/01/24 16:47:51 orynider Exp $
...
$s_last_visit = ( $mx_user->data['session_logged_in'] ) ? create_date($board_config['default_dateformat'], $mx_user->data['user_lastvisit'], $board_config['board_timezone']) : '';
...
if (defined('SHOW_ONLINE'))
{
	...
	while($row = $db->sql_fetchrow($result))
	{
		...
		if ($mx_userdata['user_id'] != ANONYMOUS)
		{
...
else
{
	...
	//{
		...
		$template->assign_block_vars('notifications', array(
			...
			'AVATAR'			=> phpbb_get_user_avatar($mx_user->data),
...
$layouttemplate->assign_vars(array(
	...
	'CURRENT_USER_AVATAR'			=> phpbb_get_user_avatar($mx_user->data),
	'CURRENT_USERNAME_SIMPLE'		=> get_username_string('no_profile', $mx_user->data['user_id'], $mx_user->data['username'], $mx_user->data['user_colour']),
	'CURRENT_USERNAME_FULL'			=> get_username_string('full', $mx_user->data['user_id'], $mx_user->data['username'], $mx_user->data['user_colour']),				
	...
	'U_USER_PROFILE'		=> get_username_string('profile', $mx_user->data['user_id'], $mx_user->data['username'], false),	
	...
	'U_MCP'					=> (($phpbb_auth->acl_get('m_') || $phpbb_auth->acl_getf_global('m_')) ? mx_append_sid("{$phpbb_root_path}modcp.$phpEx?i=main&amp;mode=front" . $mx_user->session_id) : ''),	
	...
	'U_TEAM'				=> ($mx_user->data['user_id'] != ANONYMOUS && !$phpbb_auth->acl_get('u_viewprofile')) ? '' : mx_append_sid("{$phpbb_root_path}memberlist.$phpEx?mode=team"),
	...
	'U_RESTORE_PERMISSIONS'	=> ($mx_user->data['user_perm_from'] && $auth->acl_get('a_switchperm')) ? mx_append_sid("{$phpbb_root_path}profile.$phpEx?mode=restore_perm") : '',
	'U_FEED'				=> '',	
	...
	'S_TIMEZONE'			=> sprintf($mx_user->lang['All_times'], $l_timezone),
