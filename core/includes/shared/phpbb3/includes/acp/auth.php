...
*
* @package phpBB3
* @version $Id: auth.php,v 1.2 2008/01/18 10:39:05 orynider Exp $
...
* @package phpBB3
*/
class phpbb_auth_admin extends phpbb_auth
{
	...
	*/
	function get_mask($mode, $user_id = false, $group_id = false, $forum_id = false, $auth_option = false, $scope = false, $acl_fill = ACL_NEVER)
	{
		...
		if ($view_user_mask)
		{
			...
			while ($mx_userdata = $db->sql_fetchrow($result))
			{
				if ($mx_user->data['user_id'] != $mx_userdata['user_id'])
				{
					$auth2 = new auth();
					$auth2->acl($mx_userdata);
				}
				else
				{
					global $auth;
				...
				$hold_ary[$mx_userdata['user_id']] = array();
				foreach ($forum_ids as $f_id)
				{
					$hold_ary[$mx_userdata['user_id']][$f_id] = array();
					foreach ($compare_options as $option)
					{
						$hold_ary[$mx_userdata['user_id']][$f_id][$option] = $auth2->acl_get($option, $f_id);
					}
				}
			}
			$db->sql_freeresult($result);
	...
	*/
	function display_mask($mode, $permission_type, &$hold_ary, $mx_user_mode = 'user', $local = false, $group_display = true)
	{
		global $template, $mx_user, $db, $phpbb_root_path, $phpEx;
		...
		$l_acl_type = (isset($mx_user->lang['ACL_TYPE_' . (($local) ? 'LOCAL' : 'GLOBAL') . '_' . strtoupper($permission_type)])) ? $mx_user->lang['ACL_TYPE_' . (($local) ? 'LOCAL' : 'GLOBAL') . '_' . strtoupper($permission_type)] : 'ACL_TYPE_' . (($local) ? 'LOCAL' : 'GLOBAL') . '_' . strtoupper($permission_type);

		// Allow trace for viewing permissions and in user mode
		$show_trace = ($mode == 'view' && $mx_user_mode == 'user') ? true : false;

		// Get names
		if ($mx_user_mode == 'user')
		{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$ug_names_ary[$row['ug_id']] = ($mx_user_mode == 'user') ? $row['ug_name'] : (($row['group_type'] == GROUP_SPECIAL) ? $mx_user->lang['G_' . $row['ug_name']] : $row['ug_name']);
		}
		$db->sql_freeresult($result);
		...
		$cur_roles = $this->acl_role_data($mx_user_mode, $permission_type, array_keys($hold_ary));
		...
		$mx_user_groups_default = $mx_user_groups_custom = array();
		if ($mx_user_mode == 'user' && $group_display)
		{
			...
			if ($memberships)
			{
				foreach ($memberships as $row)
				{
					...
					{
						$mx_user_groups_default[$row['user_id']][] = $mx_user->lang['G_' . $groups[$row['group_id']]['group_name']];
					}
					else
					{
						$mx_user_groups_custom[$row['user_id']][] = $groups[$row['group_id']]['group_name'];
					}
				}
			}
		...
		if (sizeof($forum_ids) == 1 || ($local && sizeof($ug_names_ary) > 1))
		{
			...
			foreach ($hold_ary as $forum_id => $forum_array)
			{
				...
				$template->assign_block_vars($tpl_pmask, array(
					...
					'S_USER_MODE'	=> ($mx_user_mode == 'user') ? true : false,
					'S_GROUP_MODE'	=> ($mx_user_mode == 'group') ? true : false)
				);
				...
				while (list($ug_id, $ug_array) = each($content_array))
				{
					...
					{
						$role_description = (!empty($mx_user->lang[$role_row['role_description']])) ? $mx_user->lang[$role_row['role_description']] : nl2br($role_row['role_description']);
						$role_name = (!empty($mx_user->lang[$role_row['role_name']])) ? $mx_user->lang[$role_row['role_name']] : $role_row['role_name'];
					...
					if ($s_role_options)
					{
						$s_role_options = '<option value="0"' . ((!$current_role_id) ? ' selected="selected"' : '') . ' title="' . htmlspecialchars($mx_user->lang['NO_ROLE_ASSIGNED_EXPLAIN']) . '">' . $mx_user->lang['NO_ROLE_ASSIGNED'] . '</option>' . $s_role_options;
					}
		...
		else
		{
			foreach ($ug_names_ary as $ug_id => $ug_name)
			{
				...
				$template->assign_block_vars($tpl_pmask, array(
					...
					'USER_GROUPS_DEFAULT'	=> ($mx_user_mode == 'user' && isset($mx_user_groups_default[$ug_id]) && sizeof($mx_user_groups_default[$ug_id])) ? implode(', ', $mx_user_groups_default[$ug_id]) : '',
					'USER_GROUPS_CUSTOM'	=> ($mx_user_mode == 'user' && isset($mx_user_groups_custom[$ug_id]) && sizeof($mx_user_groups_custom[$ug_id])) ? implode(', ', $mx_user_groups_custom[$ug_id]) : '',
					'L_ACL_TYPE'			=> $l_acl_type,
	...
	*/
	...
	{
		global $db, $template, $mx_user, $phpbb_root_path, $phpbb_admin_path, $phpEx;
		...
		foreach ($forum_names as $forum_id => $forum_name)
		{
			...
			$template->assign_block_vars('role_mask', array(
				'NAME'				=> ($forum_id == 0) ? $mx_user->lang['GLOBAL_MASK'] : $forum_name,
				'FORUM_ID'			=> $forum_id)
			);
			...
			if (isset($auth_ary['groups']) && sizeof($auth_ary['groups']))
			{
				...
				while ($row = $db->sql_fetchrow($result))
				{
					$template->assign_block_vars('role_mask.groups', array(
						...
						'GROUP_NAME'	=> ($row['group_type'] == GROUP_SPECIAL) ? $mx_user->lang['G_' . $row['group_name']] : $row['group_name'],
	...
	*/
	...
	{
		global $template, $mx_user, $phpbb_admin_path, $phpEx;
		...
		while (list($cat, $cat_array) = each($category_array))
		{
			$template->assign_block_vars($tpl_cat, array(
				...
				'CAT_NAME'	=> $mx_user->lang['permission_cat'][$cat])
			);
			...
			*
			$key_array = array_intersect(array_keys($mx_user->lang), array_map(create_function('$a', 'return "acl_" . $a;'), array_keys($cat_array['permissions'])));
			...
			while (list($permission, $allowed) = each($cat_array['permissions']))
			{
				if ($s_view)
				{
					$template->assign_block_vars($tpl_cat . '.' . $tpl_mask, array(
						...
						'PERMISSION'	=> $mx_user->lang['acl_' . $permission]['lang'])
					);
				}
				else
				{
	...
	*/
	function build_permission_array(&$permission_row, &$content_array, &$categories, $key_sort_array)
	{
		...
		foreach ($key_sort_array as $forum_id)
		{
			...
			{
				if (!isset($mx_user->lang['acl_' . $permission]))
				{
					$mx_user->lang['acl_' . $permission] = array(
						'cat'	=> 'misc',
						'lang'	=> '{ acl_' . $permission . ' }'
					);
				}
			
				$cat = $mx_user->lang['acl_' . $permission]['cat'];
			
				// Build our categories array
				if (!isset($categories[$cat]))
				{
					$categories[$cat] = $mx_user->lang['permission_cat'][$cat];
				}
