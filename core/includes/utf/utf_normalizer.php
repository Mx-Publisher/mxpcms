...
*
* @package utf
* @version $Id: utf_normalizer.php,v 1.2 2009/01/24 16:47:52 orynider Exp $
...
class utf_normalizer
{
	...
	*/
	function cleanup(&$str)
	{
		...
		if (!isset($GLOBALS['utf_nfc_qc']))
		{
			...
			include($mx_root_path . 'includes/utf/data/utf_nfc_qc.' . $phpEx);
		}

		if (!isset($GLOBALS['utf_canonical_decomp']))
		{
			global $mx_root_path, $phpEx;
			include($mx_root_path . 'includes/utf/data/utf_canonical_decomp.' . $phpEx);
		}
	...
	*/
	function nfkc(&$str)
	{
		...
		{
			global $mx_root_path, $phpEx;
			include($mx_root_path . 'includes/utf/data/utf_nfkc_qc.' . $phpEx);
		}

		if (!isset($GLOBALS['utf_compatibility_decomp']))
		{
			global $mx_root_path, $phpEx;
			include($mx_root_path . 'includes/utf/data/utf_compatibility_decomp.' . $phpEx);
		}
	...
	*/
	function recompose($str, $pos, $len, &$qc, &$decomp_map)
	{
		...
		{
			global $mx_root_path, $phpEx;
			include($mx_root_path . 'includes/utf/data/utf_normalizer_common.' . $phpEx);
		}
		...
		{
			global $mx_root_path, $phpEx;
			include($mx_root_path . 'includes/utf/data/utf_canonical_comp.' . $phpEx);
		}
