...
*
* @package BlockCP
* @version $Id: mx_functions_blockcp.php,v 1.2 2009/01/24 16:47:51 orynider Exp $
...
class mx_blockcp extends mx_block
{
	...
	{
		global $blockcptemplate, $lang, $db, $board_config, $theme, $_GET, $_POST, $_COOKIE, $phpEx, $mx_root_path, $s_hidden_fields, $userdata, $cookie_states, $module_nav_icon_url, $portalpage, $mx_request_vars, $images;
		...
		$l_setting = $mx_request_vars->request('setting', MX_TYPE_NO_TAGS, $lang['Settings']);
		//
		...
		$block_title = ( isset($_POST[$block_keys['block_title']]) ) ? stripslashes(htmlspecialchars($_POST[$block_keys['block_title']])) : $this->$block_keys['block_title'];
		$block_desc = ( isset($_POST[$block_keys['block_desc']]) ) ? stripslashes(htmlspecialchars($_POST[$block_keys['block_desc']])) : $this->$block_keys['block_desc'];
		$show_block = ( isset($_POST[$block_keys['show_block']]) ) ? intval($_POST[$block_keys['show_block']]) : intval($this->$block_keys['show_block']);
		$show_title = ( isset($_POST[$block_keys['show_title']]) ) ? intval($_POST[$block_keys['show_title']]) : intval($this->$block_keys['show_title']);
		$show_stats = ( isset($_POST[$block_keys['show_stats']]) ) ? intval($_POST[$block_keys['show_stats']]) : intval($this->$block_keys['show_stats']);
		...
		*/
		...
		$admin_icon['page'] = $module_nav_icon_url . 'icon_page.gif';
		$admin_icon['page_column'] = $module_nav_icon_url . 'icon_page_column.gif';		
		...
		$blockcptemplate->assign_vars(array(
			...
			'U_BLOCK_SETTINGS' => append_sid(PORTAL_URL . "admin/admin_mx_blockcp.$phpEx?block_id=$block_id"),
			'U_BLOCK_DELETE' => append_sid(PORTAL_URL . "admin/admin_mx_block.$phpEx?mode=delete_block&amp;block_id=$block_id"),
		...
		$message_delete = $lang['Delete_block'] . ' - ' . $block_title
					...
					. '<br /><br />' . sprintf($lang['Click_block_delete_yes'], '<a href="' . append_sid($s_action_file . $deletemode) . '">', '</a>')
					. '<br /><br />';
