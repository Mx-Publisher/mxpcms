...
* @version $Id: admin_mx_module_cp.php,v 1.2 2009/01/24 16:45:47 orynider Exp $
...
if( !empty($setmodules) )
{
	...
	$module['2_CP']['2_2_ModuleCP'] = 'admin/' . $file;
	return;
}
...
if ( isset( $_POST['include_block_quickedit'] ) )
{
	switch ( $_POST['include_block_quickedit'] )
	{
		case '0':
...
if ( isset( $_POST['include_block_private'] ) )
{
	switch ( $_POST['include_block_private'] )
	{
		case '0':
...
$template->assign_vars(array(
	...
	'S_ACTION' => append_sid("admin_mx_module_cp.$phpEx"),
	'S_ACTION_DEFAULT' => append_sid("admin_mx_module_cp.$phpEx"),
...
for( $module_count = 0; $module_count < $total_modules + 1; $module_count++ )
{
	...
	if ( file_exists( $mx_root_path . $module_rows[$module_count]['module_path'] . 'admin/mx_module_defs.php' ) )
	{
		include_once( $mx_root_path . $module_rows[$module_count]['module_path'] . 'admin/mx_module_defs.php' );
	...
	$template->assign_block_vars('module', array(
		...
		'U_MODULE_EDIT' => append_sid(PORTAL_URL . "admin/admin_mx_module.$phpEx?module_id=" . $module_rows[$module_count]['module_id']),
	...
	else
	{
		$template->assign_block_vars('module.reload', array(
			'U_MODULE_EDIT' => append_sid(PORTAL_URL . "admin/admin_mx_module_cp.$phpEx?module_id=" . $module_id),
		));
	...
	for( $function_count = 0; $function_count < $total_functions + 1; $function_count++ )
	{
		...
		$message_delete = $lang['Delete_function'] . ' - ' . $function_title
					...
					. '<br /><br />' . sprintf($lang['Click_function_delete_yes'], '<a href="' . append_sid("admin_mx_module_cp.$phpEx" . $deletemode) . '">', '</a>')
					. '<br /><br />';
		...
		$template->assign_block_vars('module.function', array(
			...
			'U_FUNCTION_DELETE' => append_sid(PORTAL_URL . "admin/admin_mx_module_cp.$phpEx" . $deletemode),
		...
		for( $parameter_count = 0; $parameter_count < $total_parameters + 1; $parameter_count++ )
		{
			...
			$message_delete = $lang['Delete_parameter'] . ' - ' . $parameter_title
						...
						. '<br /><br />' . sprintf($lang['Click_parameter_delete_yes'], '<a href="' . append_sid("admin_mx_module_cp.$phpEx" . $deletemode) . '">', '</a>')
						. '<br /><br />';
			...
			$template->assign_block_vars('module.function.parameter', array(
				...
				"U_DELETE" => append_sid("admin_mx_module_cp.php" . $deletemode),
				"U_MOVE_UP" => append_sid("admin_mx_module_cp.php" . $upmode),
				"U_MOVE_DOWN" => append_sid("admin_mx_module_cp.php" . $downmode),
		...
		for( $block_count = 0; $block_count < $total_blocks + 1; $block_count++ )
		{
			...
			$message_delete = $lang['Delete_block'] . ' - ' . $block_title
						...
						. '<br /><br />' . sprintf($lang['Click_block_delete_yes'], '<a href="' . append_sid("admin_mx_module_cp.$phpEx" . $deletemode) . '">', '</a>')
						. '<br /><br />';
			...
			$editor_name_tmp = get_userdata($block_editor_id);
			$editor_name = $editor_name_tmp['username'];

			$block_time = !$new_block ? $block_rows[$block_count]['block_time'] : time();
			$edit_time = create_date( $board_config['default_dateformat'], $block_time, $board_config['board_timezone'] );
			...
			$template->assign_block_vars('module.function.block', array(
				...
				'U_BLOCK_SETTINGS' => append_sid(PORTAL_URL . "admin/admin_mx_block_cp.$phpEx?block_id=$block_id"),
				'U_BLOCK_DELETE' => append_sid(PORTAL_URL . "admin/admin_mx_module_cp.$phpEx" . $deletemode),
				'U_BLOCK_PERMISSIONS' => append_sid(PORTAL_URL . "admin/admin_mx_block_auth.$phpEx?cat_id=$block_id"),
