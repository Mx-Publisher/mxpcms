...
* @version $Id: admin_mx_chkobjs.php,v 1.2 2009/01/24 16:45:47 orynider Exp $
...
//			[ GLOBAL DATA ]
// ======================================================
$main_title = "Check mxBB Objects";
$main_desc  = "This utility allows you to check mxBB object relationships.";
$main_info  = "This report shows the relations between mxBB tables. " .
...
//
if( isset($_POST['check_mode']) || isset($_GET['check_mode']) )
{
	$check_mode = ( isset($_POST['check_mode']) ? $_POST['check_mode'] : $_GET['check_mode'] );
}
else
{
	if( !isset($_GET['mode']) )
	{
		...
		{
			$selected = ( $i == $_GET['table'] ? ' selected="selected"' : '' );
		...
		mx_message_die(GENERAL_MESSAGE,
			'<p>'.$main_desc.'</p>' .
			'<form action="' . append_sid(PORTAL_URL . "admin/admin_mx_chkobjs.$phpEx") . '" method="post"> ' .
			$select_options .
		...
		case 'delete':
			...
			$message = '<form method="post" action="' . basename(__FILE__) . '">' .
				...
				'<div style="width:80%; padding:10px; border:solid red 1px;">' .
					...
					'<br />&nbsp;<br />' .
					'[ <a href="'.append_sid(basename(__FILE__)."?check_mode=$tb").'">'.$lang['No'].'</a> ]' .
					'&nbsp;' .
					'[ <a href="'.append_sid(basename(__FILE__).'?mode=db_delete'.$get_vars).'">'.$lang['Yes'].'</a> ]' .
				'</div><br />&nbsp;' .
				'</form>';
...
function check_relationship($tb)
{
	...
	$html.="\t" . '</tr>' . "\n";
	$html.="\t" . '<tr>' . "\n";
	$html.=tbHead('td', 'cat', 5, '<a href="'.append_sid(basename(__FILE__)."?table=$tb").'"><b>Go Back</b></a>');
	$html.="\t" . '</tr>' . "\n";
	...
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		$html.=tbItem('td', 'row1', 'center', '<a href="'.append_sid(basename(__FILE__).'?mode=delete'.$get_vars).'" '.$onclick.'>'.$lang['Delete'].'</a>');
		$html.= "\t" . '</tr>' . "\n";
		$row_count++;
	}

	$html.="\t" . '<tr>' . "\n";
	$html.=tbHead('td', 'catBottom', 5, '<a href="'.append_sid(basename(__FILE__)."?table=$tb").'"><b>Go Back</b></a>');
	$html.="\t" . '</tr>' . "\n";
	$html.='</table><br />' . "\n";
