...
* @version $Id: admin_mx_page_cp.php,v 1.2 2009/01/24 16:45:47 orynider Exp $
...
$template->assign_vars(array(
	...
	// General
	//
	'S_ACTION'					=> append_sid(PORTAL_URL . "admin/admin_mx_page_cp.$phpEx"),
...
for( $template_count = 0; $template_count < $total_templates + 1; $template_count++ )
{
	...
	$message_delete = $lang['Delete_page_template']
			...
			. '<br /><br />' . sprintf($lang['Click_page_template_delete_yes'], '<a href="' . append_sid("admin_mx_page_cp.$phpEx" . $deletemode) . '">', '</a>')
			. '<br /><br />';
	...
	$template->assign_block_vars('templates', array(
		...
		'U_DELETE'					=> append_sid("admin_mx_page_cp.$phpEx" . $deletemode),

		'L_TEMPLATE_DELETE' 		=> $lang['Page_template_delete'],
		'U_TEMPLATE_DELETE' 		=> append_sid("admin_mx_page_cp_template_setting.$phpEx?mode=deletetemplate&amp;page_template_id=$page_template_id"),
	...
	for( $column = 0; $column < $total_column + 1; $column++ )
	{
		...
		$message_delete = $lang['Delete_page_template_column'] . ' - ' . $column_title
				...
				. '<br /><br />' . sprintf($lang['Click_page_template_column_delete_yes'], '<a href="' . append_sid("admin_mx_page_cp.$phpEx" . $deletemode) . '">', '</a>')
				. '<br /><br />';
		...
		$template->assign_block_vars('templates.columnrow', array(
			...
			'U_COLUMN_DELETE' 		=> append_sid("admin_mx_page_cp.$phpEx" . $deletemode),
			'U_COLUMN_MOVE_UP' 		=> append_sid("admin_mx_page_cp.$phpEx" . $upmode),
			'U_COLUMN_MOVE_DOWN' 	=> append_sid("admin_mx_page_cp.$phpEx" . $downmode),
...
for( $page_count = -1; $page_count < $total_pages; $page_count++ )
{
	...
	$navigation_block_list = get_list_formatted('block_list', $page_rows[$page_count]['navigation_block'], 'navigation_block', 'mx_menu_nav.' . $phpEx, false, 'mx_site_nav.' . $phpEx);
	...
	$message_delete = $lang['Delete_page']
			...
			. '<br /><br />' . sprintf($lang['Click_page_delete_yes'], '<a href="' . append_sid("admin_mx_page_cp.$phpEx" . $deletemode) . '">', '</a>')
			. '<br /><br />';
	...
	$template->assign_block_vars('pages', array(
		...
		'U_DELETE'					=> append_sid("admin_mx_page_cp.$phpEx" . $deletemode),
		'U_PREVIEW'					=> append_sid($mx_root_path . "index.$phpEx" . '?page=' . $page_id),
	...
	else
	{
		$template->assign_block_vars('pages.reload', array(
			'U_PAGE_EDIT' => append_sid(PORTAL_URL . "admin/admin_mx_page_cp.$phpEx?page_id=" . $page_id),
		));
	...
	for( $column = 0; $column < $total_column + 1; $column++ )
	{
		...
		$message_delete = $lang['Delete_page_column']
				...
				. '<br /><br />' . sprintf($lang['Click_page_column_delete_yes'], '<a href="' . append_sid("admin_mx_page_cp.$phpEx" . $deletemode) . '">', '</a>')
				. '<br /><br />';
		...
		for( $block = 0; $block < $total_blocks; $block++ )
		{
			...
			if( !empty($mx_block->block_edit_file) )
			{
				...
				$u_setting = append_sid(PORTAL_URL . $mx_block->block_edit_file . "?block_id=$block_id");
			}
			else
			{
				$u_setting = '';
				...
				//
				$l_setting = $lang['Block_cp'];
				$u_setting = append_sid("admin_mx_block_cp.$phpEx?block_id=$block_id");
			}
			...
			$template->assign_block_vars('pages.columnrow.blockrow', array(
				...
				'U_BLOCK_DELETE' => append_sid("admin_mx_page_cp.$phpEx" . $deletemode),
				'U_BLOCK_MOVE_UP' => append_sid("admin_mx_page_cp.$phpEx" . $upmode),
				'U_BLOCK_MOVE_DOWN' => append_sid("admin_mx_page_cp.$phpEx" . $downmode),
				'U_BLOCK_RESYNC' => append_sid("admin_mx_page_cp.$phpEx" . $syncmode)
			));
		} // for ... blocks
