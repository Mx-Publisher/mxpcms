 ...
 *
 *   $Id: admin_mx_page_setting.php,v 1.1 2010/10/14 02:00:02 orynider Exp $
 *
 ***************************************************************************/
...
function mx_renumber_order($mode, $column = 0, $page_id)
{
  global $db;
...
if( !empty($mode) ) 
{
  switch($mode)
  {
    case 'addpage':
      ...
      $message = $lang['Page_Config_updated'] . "<br /><br />" . sprintf($lang['Click_return_page_admin'], "<a href=\"" . mx_append_sid("admin_mx_page_setting.php?page_id=$page_id") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . mx_append_sid("index.php?pane=right") . "\">", "</a>");
    ...
    case 'block_order':
      ...
      mx_renumber_order('block', $column_id, $page_id );
      $show_index = TRUE;
    ...
    case 'column_order':
      ...
      mx_renumber_order('column', 0, $page_id);
      $show_index = TRUE;
    ...
    case 'editcolumn':
      ...
      else
      {
        ...
        $newmode = 'modcolumn';
        $row = mx_get_info( COLUMN_TABLE, 'column_id', $column_id );
        $column_title = $row['column_title'] ;
        $column_size =  $row['column_size']  ;
      }
      $template->set_filenames(array(
        "edit_column" => "admin/mx_pagesetting_admin_column.tpl")
      );
...
//
$template->set_filenames(array(
  "admin_page" => "admin/mx_pagesetting_admin_body.tpl")
);


$pagelist = mx_get_list( "page_id", PAGE_TABLE, 'page_id', 'page_name', $page_id, TRUE);


$template->assign_vars(array(
  'S_ACTION'           => mx_append_sid("admin_mx_page_setting.$phpEx?mode=addblock&amp;page_id=$page_id"),
  'S_PAGE_ACTION'      => mx_append_sid("admin_mx_page_setting.$phpEx?page_id=$page_id"),
...
$sql = "SELECT cbl.*, blk.block_title, blk.block_desc, function_admin, blk.function_id 
  ...
  for($i = 0; $i < $total_column; $i++)
  {
    ...
    $blocklist = mx_get_list( "block_id[$column_id]", BLOCK_TABLE, 'block_id', 'block_title', 0, TRUE);
    ...
    $template->assign_block_vars("columnrow", array( 
      ...
      'U_COLUMN_EDIT'      => mx_append_sid("admin_mx_page_setting.$phpEx?mode=editcolumn&amp;column_id=$column_id&amp;page_id=$page_id"),
      'U_COLUMN_DELETE'    => mx_append_sid("admin_mx_page_setting.$phpEx?mode=deletecolumn&amp;column_id=$column_id&amp;page_id=$page_id"),
      'U_COLUMN_MOVE_UP'   => mx_append_sid("admin_mx_page_setting.$phpEx?mode=column_order&amp;move=-15&amp;column_id=$column_id&amp;page_id=$page_id"),
      'U_COLUMN_MOVE_DOWN' => mx_append_sid("admin_mx_page_setting.$phpEx?mode=column_order&amp;move=15&amp;column_id=$column_id&amp;page_id=$page_id"),
      'U_VIEWCOLUMN'       => mx_append_sid( PHPBB_URL ."index.$phpEx?column_id=$column_id&amp;page_id=$page_id"))
    );
    ...
    for($j = 0; $j < $total_blocks; $j++)
    {
      ...
      {
        $l_setting = $lang['Settings'];
        $u_setting = mx_append_sid( PORTAL_URL . $block_rows[$j]['function_admin'] . "?block_id=$block_id&amp;page=$page_id");
      }
      else
      {
        $u_setting = '';
        ...
        {
          $l_setting = $lang['Settings'];
          $u_setting = mx_append_sid("admin_mx_block_cp.$phpEx?mode=setting&amp;block_id=$block_id");
        }
      }
      ...
      if ($block_rows[$j]['column_id'] == $column_id)
      {
        $template->assign_block_vars("columnrow.blockrow",  array(
          ...
          'ROW_COLOR'          => $row_color,
          'U_BLOCK_EDIT'       => mx_append_sid("admin_mx_block_cp.php?mode=edit_block&amp;block_id=$block_id"),
          'U_BLOCK_SETTING'    => $u_setting,

          'U_BLOCK_DELETE'    => mx_append_sid("admin_mx_page_setting.$phpEx?mode=deleteblock&amp;block_id=$block_id&amp;column_id=$column_id&amp;page_id=$page_id"),
          'U_BLOCK_MOVE_UP'   => mx_append_sid("admin_mx_page_setting.$phpEx?mode=block_order&amp;move=-15&amp;block_id=$block_id&amp;column_id=$column_id&amp;page_id=$page_id"),
          'U_BLOCK_MOVE_DOWN' => mx_append_sid("admin_mx_page_setting.$phpEx?mode=block_order&amp;move=15&amp;block_id=$block_id&amp;column_id=$column_id&amp;page_id=$page_id"),
          'U_BLOCK_RESYNC'    => mx_append_sid("admin_mx_page_setting.$phpEx?mode=block_sync&amp;block_id=$block_id&amp;column_id=$column_id&amp;page_id=$page_id"))
        );
