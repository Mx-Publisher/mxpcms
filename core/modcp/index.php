...
if( isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane'] == 'left' )
{
	...
	$template->assign_vars(array(
		...
		"U_PORTAL_INDEX" => append_sid(PORTAL_URL . "index.$phpEx"),
		"L_PORTAL_INDEX" => $lang['Portal_index'],
		"L_PREVIEW_PORTAL" => $lang['Preview_portal'],

		"U_FORUM_INDEX" => append_sid(PHPBB_URL . "index.$phpEx"),
	...
	while( list($cat, $action_array) = each($module_portal) )
	{
		...
		while( list($action, $file) = each($action_array) )
		{
			...
			$template->assign_block_vars('module_portal.catrow.modulerow', array(
				...
				//-MOD: DHTML Menu for ACP
				"ADMIN_MODULE" => $action,
				"U_ADMIN_MODULE" => append_sid(PORTAL_URL . $file)
			));
			$row_count++;
		}
		//+MOD: DHTML Menu for ACP
	...
	while( list($cat, $action_array) = each($module_phpbb) )
	{
		...
		while( list($action, $file) = each($action_array) )
		{
			...
			$template->assign_block_vars('module_phpbb.catrow.modulerow', array(
				...
				//-MOD: DHTML Menu for ACP
				"ADMIN_MODULE" => $action,
				"U_ADMIN_MODULE" => append_sid($mx_root_path . 'modcp/' . $file))
			);
			$row_count++;
		}
		//+MOD: DHTML Menu for ACP
...
elseif( isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane'] == 'right' )
{
	...
	//
	$total_posts = mx_get_db_stat('postcount');
		...
		for($i = 0; $i < count($onlinerow_reg); $i++)
		{
			if( !inarray($onlinerow_reg[$i]['user_id'], $reg_userid_ary) )
			{
				...
				else
				{
					...
					//-MOD: ViewOnline extension for mxBB Portal Pages
					$location_url = append_sid(PHPBB_URL . "admin_forums.$phpEx?mode=editforum&amp;" . POST_FORUM_URL . "=" . $onlinerow_reg[$i]['user_session_page']);
				...
				$template->assign_block_vars("reg_user_row", array(
					...
					"U_WHOIS_IP" => "http://whois.domaintools.com/$reg_ip",
					"U_USER_PROFILE" => append_sid(PHPBB_URL . "admin_users.$phpEx?mode=edit&amp;" . POST_USERS_URL . "=" . $onlinerow_reg[$i]['user_id']),
	...
	if( count($onlinerow_guest) )
	{
		...
		for($i = 0; $i < count($onlinerow_guest); $i++)
		{
			...
			{
				$location_url = append_sid(PHPBB_URL . "admin_forums.$phpEx?mode=editforum&amp;" . POST_FORUM_URL . "=" . $onlinerow_guest[$i]['session_page']);
	...
	if ($fsock = @fsockopen('www.mx-system.com', 80, $errno, $errstr))
	{
		...
		if ($minor_mxbb_revision == $latest_mxbb_minor_revision)
		{
