...
if( $mode != "" )
{
	if( $mode == "edit" || $mode == "add" )
	{
		...
		if( $mode == "edit" )
		{
			if( empty($rank_id) )
			{
				mx_message_die(GENERAL_MESSAGE, $lang['Must_select_rank']);
			}
			...
			{
				mx_message_die(GENERAL_ERROR, "Couldn't obtain rank data", "", __LINE__, __FILE__, $sql);
			}
	...
	else if( $mode == "save" )
	{
		...
		if ($rank_id)
		{
			if (!$special_rank)
			{
				...
				if( !$result = $db->sql_query($sql) ) 
				{
					mx_message_die(GENERAL_ERROR, $lang['No_update_ranks'], "", __LINE__, __FILE__, $sql);
				}
			}
		...
		if( !$result = $db->sql_query($sql) )
		{
			mx_message_die(GENERAL_ERROR, "Couldn't update/insert into ranks table", "", __LINE__, __FILE__, $sql);
		}
	...
	else if( $mode == "delete" )
	{
		...
		if( $rank_id )
		{
			...
			if( !$result = $db->sql_query($sql) )
			{
				mx_message_die(GENERAL_ERROR, "Couldn't delete rank data", "", __LINE__, __FILE__, $sql);
			}
	...
	else
	{
		...
		if( !$result = $db->sql_query($sql) )
		{
			mx_message_die(GENERAL_ERROR, "Couldn't obtain ranks data", "", __LINE__, __FILE__, $sql);
		}
