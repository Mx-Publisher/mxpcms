...
{
	global $mx_page, $board_config, $template, $mx_request_vars, $phpEx, $images, $block_id, $mx_root_path;
		...
		foreach( $mx_page->subpage_rowset[$page_parent] as $subpage_id => $page_data )
		{
			...
			if ($_auth_ary['auth_view'] && $page_data['menu_active'])
			{
				if ( $depth == 0 )
				{
					...
					//
					$cat_on = $current_parent_page ? true : ( isset($_COOKIE['mxNavCat_' . $block_id . $page_data['page_id']]) ? $_COOKIE['mxNavCat_' . $block_id . $page_data['page_id']] == 1 : true );
					$menu_sep = $first_menu ? '' : '|';
