...
* @version $Id: navmenu_functions.php,v 1.1 2010/10/16 04:07:19 orynider Exp $
 ...
 * Enter description here...
 *
 * @param unknown_type $page_parent
 * @param unknown_type $depth
 * @param unknown_type $current_parent_page
 */
		...
		foreach( $mx_page->subpage_rowset[$page_parent] as $subpage_id => $page_data )
		{
			...
			if ($_auth_ary['auth_view'] && $page_data['menu_active'])
			{
				if ( $depth == 0 )
				{
					...
					if (true)
					{
						$icon_tmp = ( file_exists($current_template_images . '/' . $page_data['menu_icon']) ? $page_data['menu_icon'] : 'icon_blank.gif' );
						$icon_url_hot = str_replace('.gif', '_hot.gif', $icon_tmp);

						if ( file_exists($current_template_images . '/' . $icon_url_hot) )
						{
						...
						$menu_icon = ( !empty($page_data['menu_icon']) && $page_data['menu_icon'] != 'none' ) ? '<img class="'.$icon_style.'" border="0" align="absmiddle" src="' . PORTAL_URL . $mx_images['mx_menu_images'] . '/' . $icon_url . '" alt="' . $desc . '" />&nbsp;' : '';
					}
					else
					{
					...
					if ( (isset($_GET['cat_link']) ? intval($_GET['cat_link']) == $page_data['page_id'] : false) || $current_parent_page)
					{
						...
						$HTTP_COOKIE_VARS['mxNavCat_' . $block_id . $page_data['page_id']] = 1;
					}

					//
					// Generate the fold/unfold categories switches
					//
					$cat_on = $current_parent_page ? true : ( isset($HTTP_COOKIE_VARS['mxNavCat_' . $block_id . $page_data['page_id']]) ? $HTTP_COOKIE_VARS['mxNavCat_' . $block_id . $page_data['page_id']] == 1 : $catData[0]['cat_show'] == 1 );
					$menu_sep = $first_menu ? '' : '|';
				...
				else
				{
					...
					// Generate Links
					//
					$menu_link = mx_append_sid(PORTAL_URL . 'index.php?page=' . $page_data['page_id']);
