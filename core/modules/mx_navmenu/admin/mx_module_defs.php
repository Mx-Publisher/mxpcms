 ...
 * -------------------------------------------------------------------------
 * 
 *    $Id: mx_module_defs.php,v 1.3 2010/10/16 04:07:19 orynider Exp $
 */
...
class mx_module_defs
{
	...
	{
		global $template, $board_config, $db, $theme, $lang, $images, $mx_blockcp, $mx_root_path, $HTTP_POST_VARS, $HTTP_COOKIE_VARS, $userdata, $mx_request_vars, $dynamic_block_id, $portalpage, $mx_cache, $phpEx;
		global $mx_user, $module_root_path;
		
		// 
		// Extend User Style with module lang and images
		//
		if (empty($mx_user->current_template_path))
		{
			$mx_user->current_template_path = $mx_user->default_current_template_path = TEMPLATE_ROOT_PATH;
		}
		
		if (empty($mx_user->current_template_path))
		{
			$this->default_template_name = 'subSilver';
			$this->default_template_path = 'templates/' . $this->default_template_name . '/';
		...
		$admin_icon_url = PORTAL_URL . $current_template_path . 'images/admin_icons/';
		...
		for( $cat_count = 0; $cat_count < $total_cats + 1; $cat_count++ )
		{
			...
			$template->assign_block_vars('catrow', array(
				...
				'U_CAT_MOVE_UP' 			=> append_sid($mx_root_path . $s_action_file . $upmode . '&amp;sid=' . $userdata['session_id']),
				'U_CAT_MOVE_DOWN' 			=> append_sid($mx_root_path . $s_action_file . $downmode . '&amp;sid=' . $userdata['session_id'] ),
				...
				'BBCODE_STATUS' 			=> sprintf($bbcode_status, '<a href="' . append_sid(PHPBB_URL . "faq.$phpEx?mode=bbcode") . '" target="_phpbbcode">', '</a>'),
			...
			*/
			...
			for( $menu_count = 0; $menu_count < $total_menus + 1; $menu_count++ )
			{
				...
				$message_delete = $lang['Delete_nav_menu'] . ' - ' . $menuname
							...
							. '<br /><br />' . sprintf($lang['Click_nav_menu_delete_yes'], '<a href="' . append_sid($mx_root_path . $s_action_file . $deletemode . '&amp;sid=' . $userdata['session_id']) . '">', '</a>')
							. '<br /><br />';
				...
				$template->assign_block_vars('catrow.menurow', array(
					...
					'U_MENU_MOVE_UP' 	=> append_sid($mx_root_path . $s_action_file . $upmode . '&amp;sid=' . $userdata['session_id']),
					'U_MENU_MOVE_DOWN' 	=> append_sid($mx_root_path . $s_action_file . $downmode . '&amp;sid=' . $userdata['session_id']),
	...
	{
		global $mx_root_path, $phpbb_root_path, $template, $lang, $db, $board_config, $theme, $phpEx, $HTTP_GET_VARS, $HTTP_POST_VARS, $userdata, $mx_request_vars;
		...
		switch ( $type )
		{
			case MX_MENU_CAT_TYPE:
				...
				if ( !MX_PANEL_DEBUG )
				{
					if ( empty( $HTTP_POST_VARS['cat_title'] ) )
					{
					...
					//
					$cat_title = prepare_message($mx_request_vars->post('cat_title', MX_TYPE_NO_STRIP | MX_TYPE_SQL_QUOTED, ''));
					$cat_desc = prepare_message($mx_request_vars->post('cat_desc', MX_TYPE_NO_STRIP | MX_TYPE_SQL_QUOTED, ''));
			...
			case MX_MENU_TYPE:
				...
				if ( !MX_PANEL_DEBUG )
				{
					if ( empty( $HTTP_POST_VARS['menuname'] ) )
					{
						mx_message_die( GENERAL_ERROR, $lang['error_no_field'] );
					}
					...
					// Encode for db storage
					//
					$menu_title = $mx_request_vars->post('menuname', MX_TYPE_NO_STRIP | MX_TYPE_SQL_QUOTED, '');
					$menu_desc = prepare_message($mx_request_vars->post('message', MX_TYPE_NO_STRIP | MX_TYPE_SQL_QUOTED, ''), $html_on, $bbcode_on, $smilies_on, $bbcode_uid);
					$bbcode_uid = make_bbcode_uid();
					...
					$sql = "INSERT INTO " . MENU_NAV_TABLE . " (menu_id, menu_name, cat_id, function_id, block_id, page_id, menu_desc, menu_links, bbcode_uid, menu_order, menu_icon, menu_alt_icon, menu_alt_icon_hot, auth_view, auth_view_group, link_target)
						...
						'" . $HTTP_POST_VARS['auth_view_group'] . "',
						'" . $HTTP_POST_VARS['link_target'] . "' )";
	...
	{
		global $mx_root_path, $phpbb_root_path, $template, $lang, $db, $board_config, $theme, $phpEx, $HTTP_GET_VARS, $HTTP_POST_VARS, $userdata, $mx_request_vars, $mx_cache;
		...
		switch ( $type )
		{
			...
			case MX_MENU_PAGE_TYPE:
				...
				if ( !MX_PANEL_DEBUG )
				{
					if ( empty( $HTTP_POST_VARS['menuname'] ) && empty( $HTTP_POST_VARS['cat_title'] ))
					{
						mx_message_die( GENERAL_ERROR, $lang['error_no_field'] );
					}
					...
					{
						$menu_title = $mx_request_vars->post('cat_title', MX_TYPE_NO_STRIP | MX_TYPE_SQL_QUOTED, '');
					}
