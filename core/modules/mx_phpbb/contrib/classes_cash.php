 ...
 *   email                : mods@xore.ca
 *
 *   $Id: classes_cash.php,v 1.2 2010/10/16 04:07:43 orynider Exp $
 *
 *
 ***************************************************************************/
...
if ( defined('CM_VIEWTOPIC') )
{
	class cash_viewtopic
	{
		...
		function post_vars(&$postdata,&$userdata,$forum_id)
		{
			...
			global $board_config, $lang, $phpEx, $cash, $phpbb_root_path;
			if ( $board_config['cash_disable'])
			{
				return;
			}
			...
			{
				$template->assign_block_vars('cashlinks',array(	'U_LINK' => append_sid($phpbb_root_path."cash.$phpEx"),
			...
			{
				$template->assign_block_vars('cashlinks',array(	'U_LINK' => append_sid($phpbb_root_path.'cash.'.$phpEx.'?mode=donate&amp;ref=viewtopic&'.POST_USERS_URL.'='.$postdata['user_id'].'&amp;'.POST_POST_URL.'='.$postdata['post_id']),
																'L_NAME' => $lang['Donate']));
			}
			...
			{
				$template->assign_block_vars('cashlinks',array(	'U_LINK' => append_sid($phpbb_root_path.'cash.'.$phpEx.'?mode=modedit&amp;ref=viewtopic&'.POST_USERS_URL.'='.$postdata['user_id'].'&amp;'.POST_POST_URL.'='.$postdata['post_id']),
	...
	class cash_viewprofile
	{
		function post_vars(&$old_template,&$profiledata,&$userdata)
		{
			...
			if ( $userdata['session_logged_in'] && (($cash->currency_count(CURRENCY_ENABLED | CURRENCY_EXCHANGEABLE) >= 2) || ($cash->currency_count(CURRENCY_ENABLED | CURRENCY_DONATE) && ($userdata['user_id'] != $profiledata['user_id'])) || ($cash->currency_count() && (($userdata['user_level'] == ADMIN) || (($userdata['user_level'] == MOD) && $cash->currency_count(CURRENCY_ENABLED | CURRENCY_MODEDIT))))) )
			{
				...
				{
					$template->assign_block_vars('switch_cashlinkson.cashlinks',array(	'U_LINK' => append_sid($phpbb_root_path."cash.$phpEx"),
																						'L_NAME' => $lang['Exchange']));
				}
				if ( $cash->currency_count(CURRENCY_ENABLED | CURRENCY_DONATE) && ($userdata['user_id'] != $profiledata['user_id']) )
				{
					$template->assign_block_vars('switch_cashlinkson.cashlinks',array(	'U_LINK' => append_sid($phpbb_root_path.'cash.'.$phpEx.'?mode=donate&amp;ref=viewprofile&amp;'.POST_USERS_URL.'='.$profiledata['user_id']),
																						'L_NAME' => $lang['Donate']));
				}
				...
				{
					$template->assign_block_vars('switch_cashlinkson.cashlinks',array(	'U_LINK' => append_sid($phpbb_root_path.'cash.'.$phpEx.'?mode=modedit&amp;ref=viewprofile&amp;'.POST_USERS_URL.'='.$profiledata['user_id']),
