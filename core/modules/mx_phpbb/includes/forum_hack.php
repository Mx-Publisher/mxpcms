...
// Subcalls
//
if ($_GET['mode'] == 'topicreview' || $_GET['mode'] == 'smilies' || $_GET['mode'] == 'newpm' )
{
	return;
}
...
class mx_forum
{
	...
	function init()
	{
		global $images, $board_config, $lang, $block_id, $userdata, $template, $module_root_path, $mx_root_path, $HTTP_POST_VARS, $mx_request_vars, $mx_block, $db;
	...
	{
		global $phpEx, $mx_root_path, $HTTP_GET_VARS, $userdata;
			...
			switch ($script_name)
			{
				...
				case 'privmsg':
					...
					for ($i = 0; $i < count($get_vars); $i++)
					{
						...
						if (isset($HTTP_GET_VARS[$get_vars[$i]]))
						{
							$s_call .= '&' . $key . '=' . $HTTP_GET_VARS[$get_vars[$i]];
						}
					}
				...
				case 'profile':
					...
					if ( isset($HTTP_GET_VARS['mode']) || isset($HTTP_POST_VARS['mode']) )
					{
						...
						$user_id = ( isset($HTTP_GET_VARS['u']) ) ? $HTTP_GET_VARS['u'] : $userdata['user_id'];
...
//		}
	...
	{
		global $phpbb_root_path, $mx_root_path, $phpEx, $template, $HTTP_POST_VARS, $HTTP_POST_FILES, $HTTP_COOKIE_VARS, $HTTP_GET_VARS, $db, $userdata, $mode, $theme, $lang, $table_prefix, $mx_table_prefix, $board_config, $portal_config;
