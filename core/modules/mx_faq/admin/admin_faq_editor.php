<?php
/***************************************************************************
*                             admin_faq_editor.php
*                              -------------------
*     begin                : Mon Jul 31, 2002
*     copyright            : (C) 2007 Selven
*     email                : N/A
*
*     $Id: admin_faq_editor.php,v 1.5 2008/02/09 00:02:19 jonohlsson Exp $
*
****************************************************************************/
...
if( !empty($setmodules) )
{
   ...
   $module['Faq_&amp;_Rules']['Portal_Faq'] = 'modules/mx_faq/admin/'."$file?file=portal_faq";
   $module['Faq_&amp;_Rules']['Board_faq'] = 'modules/mx_faq/admin/'."$file?file=faq";
   $module['Faq_&amp;_Rules']['BBcode_faq'] = 'modules/mx_faq/admin/' ."$file?file=bbcode";
   $module['Faq_&amp;_Rules']['Rules'] = 'modules/mx_faq/admin/' ."$file?file=rules";
...
include($module_root_path . 'language/lang_' . $board_config['default_lang'] . '/lang_editor.' . $phpEx);

//
//Starting tinyMCE integration
//
	$tiny_path = $mx_root_path . 'modules/mx_shared/tinymce/jscripts/tiny_mce/';
	include($module_root_path . 'includes/functions_tiny.' . $phpEx);
...
// initially include the current FAQ or BBCode guide, depending on the file= in the query_string
$file = isset($HTTP_GET_VARS['file']) ? htmlspecialchars($HTTP_GET_VARS['file']) : 'portal_faq';
if($file == 'portal_faq' || $file == 'rules' )
{
$file_root_path = "$module_root_path";
}
else
{
$file_root_path = "$phpbb_root_path";
}
...
if( !isset($HTTP_GET_VARS['language']) && !isset($HTTP_POST_VARS['language']) )
{
	...
	$template->assign_vars(array(
		'L_LANGUAGE' => $lang['select_language'],
		'LANGUAGE_SELECT' => language_select($board_config['default_lang'], 'language', $phpbb_realpath.'language'),
		'S_ACTION' => mx_append_sid("admin_faq_editor.$phpEx?file=$file"),
		'L_SUBMIT' => $lang['retrieve'],
		'L_TITLE' => $lang['editor'],
		'L_EXPLAIN' => $lang['editor_explain']
	));
...
$language = phpBB2::phpbb_ltrim(basename(phpBB2::phpbb_rtrim($language)), "'");

if(!is_writable($file_root_path . 'language/lang_' . $language . '/lang_' . $file . '.' . $phpEx))
{
	mx_message_die(GENERAL_ERROR, $lang['write_file_explain'], $lang['write_file'], __LINE__, __FILE__);
}

// the FAQ which will generate our $faq array
include($file_root_path . 'language/lang_' . $language . '/lang_' . $file . '.' . $phpEx);
...
if(isset($HTTP_GET_VARS['mode']) || isset($HTTP_POST_VARS['mode']))
{
	...
	switch($mode)
	{
		...
		case 'block_del':
			...
			$template->assign_vars(array(
				...
				"MESSAGE_TEXT" => $lang['block_delete'],

				"L_YES" => $lang['Yes'],
				"L_NO" => $lang['No'],

				"S_CONFIRM_ACTION" => mx_append_sid("admin_faq_editor.$phpEx?file=$file&language=$language"),
		...
		case 'block_edit':
			...
			$template->assign_vars(array(
				'L_TITLE' => $lang['block_rename'],
				'L_EXPLAIN' => $lang['block_rename_explain'],
				'L_SUBMIT' => $lang['Submit'],
				'L_BLOCK_NAME' => $lang['block_name'],
				...
				'S_ACTION' => mx_append_sid("admin_faq_editor.$phpEx?file=$file&language=$language")
			));
		...
		case 'quest_new':
			...
			for($i = 0; $i < count($blocks); $i++)
			{
				$is_selected = ($s_selected_block == $i) ? 'selected="selected"' : '';
				$s_block_list .= '<option value="'.$i.'"'.$is_selected.'>' . $blocks[$i] . '</option>';
			}

			$template->assign_vars(array(
				'L_TITLE' => $lang['quest_create'],
				'L_EXPLAIN' => $lang['quest_create_explain'],
				'L_BLOCK' => $lang['block'],
				'L_QUESTION' => $lang['quest'],
		...
		case 'quest_edit':
			...
			$template->assign_vars(array(
				'L_TITLE' => $lang['quest_edit'],
				'L_EXPLAIN' => $lang['quest_edit_explain'],
				'L_BLOCK' => $lang['block'],
				'L_QUESTION' => $lang['quest'],
				'L_ANSWER' => $lang['answer'],
				'L_SUBMIT' => $lang['Submit'],
		...
		case 'quest_del':
			...
			$template->assign_vars(array(
				"MESSAGE_TITLE" => $lang['Confirm'],
				"MESSAGE_TEXT" => $lang['quest_delete'],
	...
	$fp = fopen($file_root_path . 'language/lang_' . $language . '/lang_' . $file . '.' . $phpEx, 'w');
...
$template->assign_vars(array(
	...
	'L_ADD_BLOCK' => $lang['block_add'],
	'L_ADD_QUESTION' => $lang['quest_add'],
...
if(count($blocks) > 0)
{
	for($i = 0; $i < count($blocks); $i++)
	{
		$template->assign_block_vars("blockrow", array(
			...
			'U_BLOCK_EDIT' => mx_append_sid("admin_faq_editor.$phpEx?mode=block_edit&block=$i&file=$file&language=$language"),
			'U_BLOCK_MOVE_UP' => mx_append_sid("admin_faq_editor.$phpEx?mode=block_up&block=$i&file=$file&language=$language"),
			'U_BLOCK_MOVE_DOWN' => mx_append_sid("admin_faq_editor.$phpEx?mode=block_dn&block=$i&file=$file&language=$language"),
			'U_BLOCK_DELETE' => mx_append_sid("admin_faq_editor.$phpEx?mode=block_del&block=$i&file=$file&language=$language")
		));
		...
		if(count($quests[$i]) > 0)
		{
			for($j = 0; $j < count($quests[$i]); $j++)
			{
				$template->assign_block_vars("blockrow.questrow", array(
					...
					'U_QUEST' => mx_append_sid("admin_faq_editor.$phpEx?mode=quest_edit&block=$i&quest=$j&file=$file&language=$language"),
					'U_QUEST_EDIT' => mx_append_sid("admin_faq_editor.$phpEx?mode=quest_edit&block=$i&quest=$j&file=$file&language=$language"),
					'U_QUEST_MOVE_UP' => mx_append_sid("admin_faq_editor.$phpEx?mode=quest_up&block=$i&quest=$j&file=$file&language=$language"),
					'U_QUEST_MOVE_DOWN' => mx_append_sid("admin_faq_editor.$phpEx?mode=quest_dn&block=$i&quest=$j&file=$file&language=$language"),
					'U_QUEST_DELETE' => mx_append_sid("admin_faq_editor.$phpEx?mode=quest_del&block=$i&quest=$j&file=$file&language=$language")
				));
...
include($module_root_path . 'language/lang_' . $board_config['default_lang'] . '/lang_admin_faq_editor.' . $phpEx);
