 ...
 *
 *   $Id: admin_groupcp.php,v 1.2 2009/01/24 16:48:09 orynider Exp $
 *
 *
 ***************************************************************************/
...
// -------------------------
//
function mx_generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$icq_status_img, &$icq_img, &$icq, &$aim_img, &$aim, &$msn_img, &$msn, &$yim_img, &$yim)
{
	...
	{
		$email_uri = ( $board_config['board_email_form'] ) ? mx_append_sid("profile.$phpEx?mode=email&amp;" . POST_USERS_URL .'=' . $row['user_id']) : 'mailto:' . $row['user_email'];
	...
	*/
	...
	/*
	$temp_url = mx_append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']);
	$profile_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_profile'] . '" alt="' . $lang['Read_profile'] . '" title="' . $lang['Read_profile'] . '" border="0" /></a>';
	$profile = '<a href="' . $temp_url . '">' . $lang['Read_profile'] . '</a>';

	$temp_url = mx_append_sid("privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . "=" . $row['user_id']);
	...
	$temp_url = mx_append_sid("search.$phpEx?search_author=" . urlencode($row['username']) . "&amp;showresults=posts");
...
{
	$group_id = $mx_request_vars->request(POST_GROUPS_URL, MX_TYPE_NO_TAGS);
}
else
{
	$group_id = '';
...
if ($mx_request_vars->is_post('groupstatus') && $group_id )
{
	if ( !$userdata['session_logged_in'] )
	{
		mx_redirect(mx_append_sid("login.$phpEx?redirect=admin_groupcp.$phpEx&" . POST_GROUPS_URL . "=$group_id", true));
	}
	...
	{
		mx_message_die(GENERAL_ERROR, 'Could not obtain user and group information', '', __LINE__, __FILE__, $sql);
	}
	...
	if ( $row['group_moderator'] != $userdata['user_id'] && $userdata['user_level'] != ADMIN )
	{
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="3;url=' . mx_append_sid("index.$phpEx") . '">')
		);

		$message = $lang['Not_group_moderator'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');

		mx_message_die(GENERAL_MESSAGE, $message);
	}

	$sql = "UPDATE " . GROUPS_TABLE . "
		SET group_type = " . $mx_request_vars->post('group_type', MX_TYPE_INT) . "
		WHERE group_id = $group_id";
	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, 'Could not obtain user and group information', '', __LINE__, __FILE__, $sql);
	...
	$template->assign_vars(array(
		'META' => '<meta http-equiv="refresh" content="3;url=' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">')
	);

	$message = $lang['Group_type_updated'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');

	mx_message_die(GENERAL_MESSAGE, $message);

}
else if ($mx_request_vars->is_post('joingroup') && $group_id )
{
	//
	...
	if (	$row = $db->sql_fetchrow($result) )
	{
		if ( $row['group_type'] == GROUP_OPEN )
		{
			do
			{
				if ( $userdata['user_id'] == $row['user_id'] )
				{
					...
					$message = $lang['Already_member_group'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');
		...
		else
		{
			...
			$message = $lang['This_closed_group'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');
	...
	{
		mx_message_die(GENERAL_MESSAGE, $lang['No_groups_exist']);
	}
	...
	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, "Error inserting user group subscription", "", __LINE__, __FILE__, $sql);
	}
	...
	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, "Error getting group moderator data", "", __LINE__, __FILE__, $sql);
	}
	...
	$emailer->assign_vars(array(
		...
		'U_GROUPCP' => mx_append_sid("admin_groups.$phpEx". '?' . POST_GROUPS_URL . "=$group_id&validate=true"))
	);
	$emailer->send();
	$emailer->reset();
	*/
	...
	$message = $lang['Group_joined'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');

	mx_message_die(GENERAL_MESSAGE, $message);
}
else if ($mx_request_vars->is_post('unsub') || $mx_request_vars->is_post('unsubpending') && $group_id )
{
	//
	...
	if ( $cancel )
	{
		mx_redirect(mx_append_sid("admin_groupcp.$phpEx", true));
	}
	...
	if ( $confirm )
	{
		...
		if ( !($result = $db->sql_query($sql)) )
		{
			mx_message_die(GENERAL_ERROR, 'Could not delete group memebership data', '', __LINE__, __FILE__, $sql);
		}
		...
		if ( $userdata['user_level'] != ADMIN && $userdata['user_level'] == MOD )
		{
			...
			if ( !($result = $db->sql_query($sql)) )
			{
				mx_message_die(GENERAL_ERROR, 'Could not obtain moderator status', '', __LINE__, __FILE__, $sql);
			}
			...
			if ( !($row = $db->sql_fetchrow($result)) || $row['is_auth_mod'] == 0 )
			{
				...
				if ( !($result = $db->sql_query($sql)) )
				{
					mx_message_die(GENERAL_ERROR, 'Could not update user level', '', __LINE__, __FILE__, $sql);
				}
			}
		}
		*/
		...
		$message = $lang['Unsub_success'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');

		mx_message_die(GENERAL_MESSAGE, $message);
	}
	else
	{
		$unsub_msg = $mx_request_vars->is_post('unsub') ? $lang['Confirm_unsub'] : $lang['Confirm_unsub_pending'];
		...
		$template->assign_vars(array(
			...
			'L_YES' => $lang['Yes'],
			'L_NO' => $lang['No'],
			'S_CONFIRM_ACTION' => mx_append_sid("admin_groupcp.$phpEx"),
...
else if ( $group_id )
{
	...
	//
	if ($mx_request_vars->is_get('validate') )
	{
		if ( !$userdata['session_logged_in'] )
		{
			mx_redirect(mx_append_sid("login.$phpEx?redirect=admin_groupcp.$phpEx&" . POST_GROUPS_URL . "=$group_id", true));
	...
	*/
	...
	$sql = "SELECT group_moderator, group_type
		FROM " . GROUPS_TABLE . "
		WHERE group_id = $group_id";

	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, 'Could not get moderator information', '', __LINE__, __FILE__, $sql);
	}
	...
	if ( $group_info = $db->sql_fetchrow($result) )
	{
		...
		//
		if (!$mx_request_vars->is_empty_post('add') || !$mx_request_vars->is_empty_post('remove') || $mx_request_vars->is_post('approve') || $mx_request_vars->is_post('deny'))
		{
			if ( !$userdata['session_logged_in'] )
			{
				mx_redirect(mx_append_sid("login.$phpEx?redirect=admin_groupcp.$phpEx&" . POST_GROUPS_URL . "=$group_id", true));
			...
			if ( !$is_moderator )
			{
				...
				$message = $lang['Not_group_moderator'] . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');
			...
			if ($mx_request_vars->is_post('add'))
			{
				...
				if ( !($result = $db->sql_query($sql)) )
				{
					mx_message_die(GENERAL_ERROR, "Could not get user information", $lang['Error'], __LINE__, __FILE__, $sql);
				}
				...
				if ( !($row = $db->sql_fetchrow($result)) )
				{
					...
					$message = $lang['Could_not_add_user'] . "<br /><br />" . sprintf($lang['Click_return_group'], "<a href=\"" . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_index'], "<a href=\"" . mx_append_sid("index.$phpEx") . "\">", "</a>");
				...
				if ( $row['user_id'] == ANONYMOUS )
				{
					...
					$message = $lang['Could_not_anon_user'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');
				...
				if ( !($result = $db->sql_query($sql)) )
				{
					mx_message_die(GENERAL_ERROR, 'Could not get user information', '', __LINE__, __FILE__, $sql);
				}
				...
				if ( !($db->sql_fetchrow($result)) )
				{
					...
					if ( !$db->sql_query($sql) )
					{
						mx_message_die(GENERAL_ERROR, 'Could not add user to group', '', __LINE__, __FILE__, $sql);
					}
					...
					{
						mx_message_die(GENERAL_ERROR, 'Could not get group information', '', __LINE__, __FILE__, $group_sql);
					}
					...
					$emailer->assign_vars(array(
						...
						'U_GROUPCP' => mx_append_sid("admin_groups.$phpEx" . '?' . POST_GROUPS_URL . "=$group_id"))
					);
					$emailer->send();
					$emailer->reset();
					*/
				...
				else
				{
					...
					$message = $lang['User_is_member_group'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . mx_append_sid("index.$phpEx") . '">', '</a>');
			...
			{
				if ( ( ($mx_request_vars->is_post('approve') || $mx_request_vars->is_post('deny') ) && $mx_request_vars->is_post('pending_members') ) || ( $mx_request_vars->is_post('remove') && $mx_request_vars->is_post('members') ) )
				{

					$members = ($mx_request_vars->is_post('approve') || $mx_request_vars->is_post('deny')) ? $mx_request_vars->post('pending_members', MX_TYPE_NO_TAGS) : $mx_request_vars->post('members', MX_TYPE_NO_TAGS);
					...
					if ($mx_request_vars->is_post('approve'))
					{
					...
					else if ($mx_request_vars->is_post('deny') || $mx_request_vars->is_post('remove') )
					{
						if ( $group_info['auth_mod'] )
						{
					...
					if ( !$db->sql_query($sql) )
					{
						mx_message_die(GENERAL_ERROR, 'Could not update user group table', '', __LINE__, __FILE__, $sql);
					}
					...
					if ($mx_request_vars->is_post('approve') )
					{
						...
						{
							mx_message_die(GENERAL_ERROR, 'Could not get user email information', '', __LINE__, __FILE__, $sql);
						}
	...
	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, 'Error getting user pending information', '', __LINE__, __FILE__, $sql);
	}
	...
	//
	$template->set_filenames(array(
		'info' => 'admin/groupcp_info_body.tpl',
		'pendinginfo' => 'admin/groupcp_pending_info.tpl')
	);
	...
	mx_generate_user_info($group_moderator, $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $icq_status_img, $icq_img, $icq, $aim_img, $aim, $msn_img, $msn, $yim_img, $yim);
	...
	$template->assign_vars(array(
		...
		'U_MOD_VIEWPROFILE' => mx_append_sid("admin_users.$phpEx?mode=edit&amp;" . POST_USERS_URL . "=$user_id"),
		...
		'S_GROUPCP_ACTION' => mx_append_sid("admin_groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id"))
	);
	...
	for($i = $start; $i < min($board_config['topics_per_page'] + $start, $members_count); $i++)
	{
		...
		mx_generate_user_info($group_members[$i], $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $icq_status_img, $icq_img, $icq, $aim_img, $aim, $msn_img, $msn, $yim_img, $yim);
		...
		if ( $group_info['group_type'] != GROUP_HIDDEN || $is_group_member || $is_moderator )
		{
			...
			$template->assign_block_vars('member_row', array(
				...
				'U_VIEWPROFILE' => mx_append_sid("admin_users.$phpEx?mode=edit&amp;" . POST_USERS_URL . "=$user_id"))
			);
	...
	if ( $is_moderator )
	{
		...
		if ( $modgroup_pending_count )
		{
			for($i = 0; $i < $modgroup_pending_count; $i++)
			{
				...
				mx_generate_user_info($modgroup_pending_list[$i], $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $icq_status_img, $icq_img, $icq, $aim_img, $aim, $msn_img, $msn, $yim_img, $yim);
...
{
	//
	// Show the main admin_groupcp.php screen where the user can select a group.
	//
	...
	if ( $s_group_list_opt != '' || $s_pending_groups_opt != '' || $s_member_groups_opt != '' )
	{
		...
		$template->set_filenames(array(
			'user' => 'admin/groupcp_user_body.tpl')
		);
		...
		$template->assign_vars(array(
			...
			'S_USERGROUP_ACTION' => mx_append_sid("admin_groupcp.$phpEx"),
			'S_HIDDEN_FIELDS' => $s_hidden_fields,
