...
* @version $Id: admin_userlist.php,v 1.2 2009/01/24 16:48:09 orynider Exp $
...
// confirm
//
if ($mx_request_vars->is_request('confirm'))
{
	$confirm = true;
}
else
...
// cancel
//
if ($mx_request_vars->is_request('cancel'))
{
	$cancel = true;
	$mode = '';
}
...
// get show amount
//
if ($mx_request_vars->is_request('show'))
{
	$show = $mx_request_vars->request('show', MX_TYPE_INT, 0);
}
else
{
	$show = isset($board_config['posts_per_page']) ? $board_config['posts_per_page'] : 20;
}

//
// sort method
//
if ($mx_request_vars->is_request('sort'))
{
	$sort = $mx_request_vars->request('sort', MX_TYPE_NO_TAGS);
...
// sort order
//
if ($mx_request_vars->is_request('order'))
{
	$sort_order = $mx_request_vars->request('order', MX_TYPE_NO_TAGS) == 'ASC' ? 'ASC' : 'DESC';
}
else
{
	$sort_order = 'ASC';
...
// alphanumeric stuff
//
if ($mx_request_vars->is_request('alphanum'))
{
	$alphanum = $mx_request_vars->request('alphanum', MX_TYPE_NO_HTML);
...
{
	$user_ids = $mx_request_vars->request(POST_USERS_URL);
}
else
{
	unset($user_ids);
...
switch( $mode )
{
	case 'delete':
		...
		if ( $cancel )
		{
			mx_redirect($module_root_path . 'admin/admin_userlist.'.$phpEx);
		}
		...
		if ( !$confirm )
		{
			...
			$template->assign_vars(array(
				...
				'S_CONFIRM_ACTION' => mx_append_sid('admin_userlist.'.$phpEx.'?mode=delete'),
		...
		else
		{
			...
			$message = $lang['User_deleted_successfully'] . "<br /><br />" . sprintf($lang['Click_return_userlist'], "<a href=\"" . mx_append_sid("admin_userlist.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . mx_append_sid("index.$phpEx?pane=right") . "\">", "</a>");
	...
	case 'ban':
		...
		if ( !$confirm )
		{
			...
			$template->assign_vars(array(
				...
				'S_CONFIRM_ACTION' => mx_append_sid('admin_userlist.'.$phpEx.'?mode=ban'),
				'S_HIDDEN_FIELDS' => $hidden_fields)
			);
		}
		else
		{
			...
			while( $i < count($user_ids) )
			{
				...
				if( !($result = $db->sql_query($sql)) )
				{
					mx_message_die(GENERAL_ERROR, 'Could not obtain ban user', '', __LINE__, __FILE__, $sql);
				}
			...
			$message = $lang['User_banned_successfully'] . "<br /><br />" . sprintf($lang['Click_return_userlist'], "<a href=\"" . mx_append_sid("admin_userlist.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . mx_append_sid("index.$phpEx?pane=right") . "\">", "</a>");
	...
	case 'activate':
		...
		while( $i < count($user_ids) )
		{
			...
			if( !($result = $db->sql_query($sql)) )
			{
				mx_message_die(GENERAL_ERROR, 'Could not obtain user information', '', __LINE__, __FILE__, $sql);
			}
			$row = $db->sql_fetchrow($result);
			...
			if( !($result = $db->sql_query($sql)) )
			{
				mx_message_die(GENERAL_ERROR, 'Could not update user status', '', __LINE__, __FILE__, $sql);
			}
		...
		$message = $lang['User_status_updated'] . "<br /><br />" . sprintf($lang['Click_return_userlist'], "<a href=\"" . mx_append_sid("admin_userlist.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . mx_append_sid("index.$phpEx?pane=right") . "\">", "</a>");
	...
	case 'group':
		...
		if ( !$confirm )
		{
			...
			$template->assign_vars(array(
				...
				'S_ACTION' => mx_append_sid($module_root_path . 'admin/admin_userlist.'.$phpEx.'?mode=group'),
				'L_GO' => $lang['Go'],
			...
			if( !($result = $db->sql_query($sql)) )
			{
				mx_message_die(GENERAL_ERROR, 'Could not query groups', '', __LINE__, __FILE__, $sql);
			}
		...
		else
		{
			...
			$message = $lang['User_add_group_successfully'] . "<br /><br />" . sprintf($lang['Click_return_userlist'], "<a href=\"" . mx_append_sid("admin_userlist.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . mx_append_sid("index.$phpEx?pane=right") . "\">", "</a>");
	...
	default:
		...
		while( $i < count($alpha_range) )
		{
			...
			if ( $alpha_range[$i] != $lang['All'] )
			{
				...
				$alphanum_search_url = mx_append_sid($module_root_path . "admin/admin_userlist.$phpEx?sort=$sort&amp;order=$sort_order&amp;show=$show&amp;alphanum=$temp");
			}
			else
			{
				$alphanum_search_url = mx_append_sid($module_root_path . "admin/admin_userlist.$phpEx?sort=$sort&amp;order=$sort_order&amp;show=$show");
			}
		...
		$template->assign_vars(array(
			...
			'S_ACTION' => mx_append_sid($module_root_path . 'admin/admin_userlist.'.$phpEx),
			'L_GO' => $lang['Go'],
			'L_SELECT' => $lang['Select_one'],
			...
			'SORT_USER_ID' => ( $sort == 'user_id' ) ? 'selected="selected"' : '',
			'SORT_ACTIVE' => ( $sort == 'user_active' ) ? 'selected="selected"' : '',
			'SORT_USERNAME' => ( $sort == 'username' ) ? 'selected="selected"' : '',
			'SORT_JOINED' => ( $sort == 'user_regdate' ) ? 'selected="selected"' : '',
			'SORT_ACTIVTY' => ( $sort == 'user_session_time' ) ? 'selected="selected"' : '',
			'SORT_USER_LEVEL' => ( $sort == 'user_level' ) ? 'selected="selected"' : '',
			'SORT_POSTS' => ( $sort == 'user_posts' ) ? 'selected="selected"' : '',
			'SORT_RANK' => ( $sort == 'user_rank' ) ? 'selected="selected"' : '',
			'SORT_EMAIL' => ( $sort == 'user_email' ) ? 'selected="selected"' : '',

			'ORDER_ASC' => ( $sort_order == 'ASC' ) ? 'selected="selected"' : '',
			'ORDER_DESC' => ( $sort_order == 'DESC' ) ? 'selected="selected"' : '',

			'L_ADD_USER' => 'Add New User',
			'U_ADD_USER' => mx_append_sid('admin_users.'.$phpEx.'?mode=add'),
		...
		if( !($result = $db->sql_query($sql)) )
		{
			mx_message_die(GENERAL_ERROR, 'Could not query users', '', __LINE__, __FILE__, $sql);
		}
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			//
			$row['username_orig'] = $row['username'];
			$style_color = '';
			...
			$template->assign_block_vars('user_row', array(
				'ROW_NUMBER' => $i + ($mx_request_vars->get('start', MX_TYPE_INT) + 1 ),
				...
				//'U_PROFILE' => mx_append_sid('admin_users.'.$phpEx.'?mode=edit&amp;' . POST_USERS_URL . '=' . $row['user_id']),
				'U_PROFILE' => mx_append_sid('admin_users.'.$phpEx.'?mode=edit&amp;username=' . $row['username_orig']),
				...
				'U_PM' => mx_append_sid($phpbb_root_path . 'privmsg.' . $phpEx . '?mode=post&amp;' . POST_USERS_URL . '='. $row['user_id']),
				'U_MANAGE' => mx_append_sid('admin_users.'.$phpEx.'?mode=edit&amp;' . POST_USERS_URL . '=' . $row['user_id']),
				'U_PERMISSIONS' => mx_append_sid($phpbb_root_path . 'admin/admin_ug_auth.'.$phpEx.'?mode=user&amp;' . POST_USERS_URL . '=' . $row['user_id']))
			);

			if (PORTAL_BACKEND != 'internal')
			{
				$template->assign_block_vars('noninternal', array());
			}
			...
			{
				mx_message_die(GENERAL_ERROR, 'Could not query groups', '', __LINE__, __FILE__, $group_sql);
			}
			$g = 0;
			while ( $group_row = $db->sql_fetchrow($group_result) )
			{
				...
				$template->assign_block_vars('user_row.group_row', array(
					...
					'U_GROUP' => mx_append_sid('admin_groupcp.'.$phpEx.'?'.POST_GROUPS_URL.'='.$group_row['group_id']))
				);
				$g++;
			}
		...
		{
			mx_message_die(GENERAL_ERROR, 'Error getting total users', '', __LINE__, __FILE__, $sql);
		}
