 ...
 * -------------------------------------------------------------------------
 * 
 *    $Id: mx_textblock_blog.php,v 1.3 2010/10/16 04:06:37 orynider Exp $
 */
...
if( $blog_validate || $iss_auth_ary['auth_edit'] )
{
	...
	$editt_url = mx_append_sid( $mx_root_path . $splitt_admin_file . "?sid=" . $userdata['session_id'] );
...
if( count($art_pages) > 1 )
{
	...
	while( $i < count($art_pages) )
	{
		...
		if( $page_num != $i )
		{
			$temp_url = mx_append_sid(PORTAL_URL . "index.php?page=$page_id&amp;mode=article&amp;page_num=$page_number");
