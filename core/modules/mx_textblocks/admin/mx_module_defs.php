...
* @version $Id: mx_module_defs.php,v 1.3 2011/07/19 11:30:24 orynider Exp $
...
class mx_module_defs
{
	...
	function get_parameters($type_row = '')
	{
		...
		$type_row['BBText'] = !empty($lang['ParType_BBText']) ? $lang['ParType_BBText'] : "UserBlog Textblock";
	...
	function submit_module_parameters( $parameter_data, $block_id )
	{
		...
		//
		//
		$this->allow_all_html_tags = $parameter_data['parameter_type'] = 'WysiwygTextBlock' ? true : false;
		...
		mx_add_search_words('single', $block_id, $parameter_value, $block_title);
	...
	function display_module_parameters( $parameter_data, $block_id )
	{
		...
		global $mx_cache, $mx_page, $mx_request_vars, $mx_block;
		//$parameter_data = $mx_cache->_get_block_config($block_id, 0, 'block_parameters');
		/**/
		/** Virtual Hack
		/**/
		if ($mx_page->is_virtual && $mx_request_vars->request('virtual'))
		{
			...
			foreach($parameter_data = $mx_blockcp->block_virtual_parameters as $parameter_name => $data)
			{
				//Rewrite for php5
	...
	{
		global $mx_cache, $template, $board_config, $db, $theme, $lang, $images, $mx_blockcp, $mx_root_path, $phpbb_root_path, $phpEx, $mx_table_prefix, $table_prefix;
		...
		if ($mx_request_vars->is_post('preview'))
		{
			...
			// Encode for preview
			//
			$this->preview($parameter_value, $html_on, $bbcode_on, $smilies_on);
		...
		else
		{
			...
			//
			$parameter_value = $this->decode($parameter_value, $bbcode_uid);
	...
	{
		global $mx_cache, $template, $board_config, $db, $theme, $lang, $images, $mx_blockcp, $mx_root_path, $phpbb_root_path, $phpEx, $mx_table_prefix, $table_prefix, $html_entities_match, $html_entities_replace;
		global $mx_request_vars, $mx_bbcode;
		$parameter_data = $mx_cache->_get_block_config($block_id, 0, 'block_parameters');
		$parameter_value = $mx_request_vars->is_post('preview') ? $mx_request_vars->post($parameter_data['parameter_name'], MX_TYPE_NO_TAGS) : $parameter_data['parameter_value'];
		$bbcode_uid = $mx_request_vars->is_post('preview') ? '' : $parameter_data['parameter_opt'];
	...
	function decode($text = '', $bbcode_uid = '')
	{
		if( $bbcode_uid != '' )
		{
	...
	// Preview Textblock
	// ===================================================
	function preview($text, $html_on, $bbcode_on, $smilies_on, $is_html_textblock = false)
	{
		global $board_config, $template, $lang, $html_entities_match, $html_entities_replace;
