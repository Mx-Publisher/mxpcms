 ...
 * -------------------------------------------------------------------------
 * 
 *    $Id: mx_statistics.php,v 1.3 2010/10/16 04:06:37 orynider Exp $
 */
...
//
$vote_left = PHPBB_URL . TEMPLATE_ROOT_PATH . "/images/vote_lcap.gif";
$vote_right = PHPBB_URL . TEMPLATE_ROOT_PATH . "/images/vote_rcap.gif";
$vote_bar = PHPBB_URL . TEMPLATE_ROOT_PATH . "/images/voting_bar.gif";
...
// Authorization SQL
//
$auth_data_sql_stats = get_auth_forum();
...
for( $i = 0; $i < $user_count; $i++ )
{
	...
	$template->assign_block_vars('users', array(
		...
		"BAR" => $bar_percent,
		"URL" => mx_append_sid(PHPBB_URL . "profile.php?mode=viewprofile&amp;u=" . $user_data[$i]['user_id']),
...
// Also, takes into mxBB tables (DB Size) and Attachment MOD
//
	//
...
$row = $db->sql_fetchrow($result);
$most_users_date = ( $row['config_value'] > 0 ) ? create_date($board_config['default_dateformat'], $row['config_value'], $board_config['board_timezone']) : $lang['Not_available'];
$row = $db->sql_fetchrow($result);
...
$value_array = array($total_posts, $posts_per_day, $total_topics, $topics_per_day, $total_users, $users_per_day, $start_date, sprintf('%.2f', $boarddays), $dbsize, $avatar_dir_size, create_date($board_config['default_dateformat'], $newest_user_date, $board_config['board_timezone']), sprintf('<a href="' . mx_append_sid(PHPBB_URL . "profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$newest_uid") . '">' . $newest_user . '</a>'), $most_users_date, $most_users, ( $board_config['gzip_compress'] ) ? $lang['Enabled'] : $lang['Disabled']);
