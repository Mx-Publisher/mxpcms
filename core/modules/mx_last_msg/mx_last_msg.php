 ...
 * -------------------------------------------------------------------------
 * 
 *    $Id: mx_last_msg.php,v 1.3 2010/10/16 04:06:36 orynider Exp $
 */
...
// Authorization SQL
//
$auth_data_sql_msg = get_auth_forum();
...
$msg_start = ( isset( $HTTP_GET_VARS['lmsg_start'] ) ) ? intval( $HTTP_GET_VARS['lmsg_start'] ) : 0;
...
for( $row_count = 0; $row_count < count($postrow); $row_count++ )
{
	...
	{
		$message_date = create_date($board_config['default_dateformat'], $postrow[$row_count]['post_time'], $board_config['board_timezone']);
	}
	else
	{
		$message_date = '';
	...
	{
		$last_post_url = '<a href="' . mx_append_sid(PHPBB_URL . "viewtopic.$phpEx?". "lmsg_start=" . $msg_start . "&" . POST_POST_URL . '=' . $postrow[$row_count]['topic_last_post_id']) . '#' . $postrow[$row_count]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" border="0" /></a>';
	}
	else
	{
		$last_post_url = '';
