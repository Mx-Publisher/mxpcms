...
* @version $Id: xs_include_import2.php,v 1.3 2010/10/16 04:08:34 orynider Exp $
	...
	else
	{
		if($data['filename'])
		{
			...
			elseif(!empty($get_file) && $get_file === $data['filename'])
			{
				...
				{
					// show as text
					$str = '<div align="left">' . $lang['xs_import_list_contents'] . $f . ' [<a href="' . mx_append_sid('xs_import.' . $phpEx . '?list=1&import=' . urlencode($filename) . '&get_file=' . urlencode($f) . '&get_content=1') . '">' . $lang['xs_import_download_lc'] . '</a>]<br /><br />';
...
if($list_only)
{
	...
	foreach($list_data as $var => $value)
	{
		...
		if($value['size'] > 0)
		{
			...
			{
				$str .= '[<a href="' . mx_append_sid('xs_import.' . $phpEx . '?list=1&import=' . urlencode($filename) . '&get_file=' . urlencode($var)) . '">' . $lang['xs_import_view_lc'] . '</a>] ';
			}
			$str .= '[<a href="' . mx_append_sid('xs_import.' . $phpEx . '?list=1&import=' . urlencode($filename) . '&get_file=' . urlencode($var)) . '&get_content=1">' . $lang['xs_import_download_lc'] . '</a>] ';
		}
