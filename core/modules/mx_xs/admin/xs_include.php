...
* @version $Id: xs_include.php,v 1.3 2010/10/16 04:08:34 orynider Exp $
...
{
	global $mx_root_path, $module_root_path, $board_config, $phpEx, $lang;

	//$module_root_path = '../modules/mx_xs/';
	//$mx_root_path = $module_root_path . '../../../';
	//$phpEx = substr(strrchr(__FILE__, '.'), 1);

	$xs_lang_file = $module_root_path . 'language/lang_' . $board_config['default_lang'] . '/lang_xs.' . $phpEx;
	if( @file_exists($xs_lang_file) )
	{	// load english version if there is no translation to current language
		$xs_lang_file = $xs_lang_file;
	}
	else if( @file_exists('../modules/mx_xs/' . 'language/lang_' . $board_config['default_lang'] . '/lang_xs.' . $phpEx) )
	{	// load english version if there is no translation to current language
		$xs_lang_file = '../modules/mx_xs/' . 'language/lang_' . $board_config['default_lang'] . '/lang_xs.' . $phpEx;
	}
	else if( @file_exists('../modules/mx_xs/' . 'language/lang_english/lang_xs.' . $phpEx) )
	{	// load english version if there is no translation to current language
		$xs_lang_file = '../modules/mx_xs/' . 'language/lang_english/lang_xs.' . $phpEx;
	}
	else if( @file_exists($module_root_path . 'language/lang_english/lang_xs.' . $phpEx) )
	{	// load english version if there is no translation to current language
		$xs_lang_file = $module_root_path . 'language/lang_english/lang_xs.' . $phpEx;
	}
...
if(!@function_exists('xs_admin_override'))
{
	function xs_admin_override($modded = false)
	{
		global $module, $phpEx, $xs_shownav_action, $board_config, $lang, $tplEx;
		// remove default phpBB styles management

		$module_name = 'eXtreme_Styles';

		$module['$module_name']['Menu'] = 'modules/mx_xs/admin/' . 'xs_frameset.'.$phpEx.'?action=menu&showwarning=1';

		// add new menu

		$module[$module_name]['Styles_Management'] = 'modules/mx_xs/admin/' . 'xs_frameset.'.$phpEx.'?action=menu';
		for($i=0; $i<XS_SHOWNAV_MAX; $i++)
		{
			$num = pow(2, $i);
			if($i != XS_SHOWNAV_DOWNLOAD && ($board_config['xs_shownav'] & $num) > 0)
			{
				$module[$module_name][$lang['xs_config_shownav'][$i]] = 'modules/mx_xs/admin/' . 'xs_frameset.'.$phpEx.'?action=' . $xs_shownav_action[$i];
			}
		}
		...
		foreach($board_config as $var => $value)
		{
			...
			{
				$str = substr($var, 9);
				$module['Template_Config'][$str] = 'modules/mx_xs/admin/' . 'xs_frameset.'.$phpEx.'?action=style_config&tpl='.urlencode($str);
			}
		}
		return true;
	}
...
if(!empty($setmodules))
{
	...
	*/
	...
	foreach($board_config as $var => $value)
	{
		...
		{
			$str = substr($var, 9);
			$module[$module_name][$str] = 'modules/mx_xs/admin/' . 'xs_frameset.'.$phpEx.'?action=style_config&tpl='.urlencode($str);
		}
	}
	return;
}
...
//
// Global defines for eXtreme Styles module administration panel
//
...
$template->assign_vars(array(
	...
	'XS_UL2'	=> '</table>',
	'XS_LI'		=> '<tr><td width="20" align="center" valign="middle"><img src="../images/dot.gif" border="0" alt="" /></td><td align="left" valign="middle" width="100%"><span class="gen">',
	'XS_LI2'	=> '</span></td></tr>',
...
if(!defined('NO_XS_HEADER'))
{
	$template->set_filenames(array(
		'xs_header' => XS_TPL_PATH . 'xs_header.'.$tplEx,
		'xs_footer' => XS_TPL_PATH . 'xs_footer.'.$tplEx,
		));
	$template->preparse = 'xs_header';
	$template->postparse = 'xs_footer';
	$template->assign_block_vars('nav_left',array('ITEM' => '<a href="' . mx_append_sid('xs_index.'.$phpEx) . '">' . $lang['xs_menu'] . '</a>'));
}
...
//
if(!@function_exists('get_ftp_config'))
{
	function get_ftp_config($action, $post = array(), $allow_local = false, $show_error = '')
	{
		global $template, $board_config, $db, $lang, $HTTP_POST_VARS, $HTTP_SERVER_VARS, $tplEx;
		$board_config['xs_ftp_local'] = false;
		// check if ftp can be used
		if(!@function_exists('ftp_connect'))
		{
			if($allow_local && xs_dir_writable($mx_root_path . 'templates/'))
			{
		...
		$template->set_filenames(array('config' => XS_TPL_PATH . 'ftp.'.$tplEx));
	...
	function xs_get_themeinfo($tpl)
	{
		...
		include($mx_root_path . 'templates/' . $tpl . '/theme_info.cfg');
	...
	function xs_generate_themeinfo($theme_rowset, $export, $exportas, $total)
	{
		...
		$theme_data = '<?php'."\n\n";
		$theme_data .= "//\n// eXtreme Styles module (compatible with phpBB 2.0.x) auto-generated theme config file for $exportas\n// Do not change anything in this file unless you know exactly what you are doing!\n//\n\n";
	...
	{
		global $template, $lang, $tplEx;
		if($line || $file)
		{
			$error = basename($file) . '(' . $line . '): ' . $error;
		}
		$template->set_filenames(array('errormsg' => XS_TPL_PATH . 'message.'.$tplEx));
		$template->assign_vars(array(
	...
	{
		global $template, $tplEx;
		$template->set_filenames(array('msg' => XS_TPL_PATH . 'message.'.$tplEx));
		$template->assign_vars(array(
				'MESSAGE_TITLE'	=> $title,
	...
	function pack_style($name, $newname, $themes, $comment)
	{
		...
		entries:
		  ...
		  - style names
		*/
		global $mx_root_path, $template_dir;
		$data = gzcompress(pack_dir($mx_root_path . $template_dir . $name, '', $name, $newname));
	...
	function pack_dir($dir1, $dir2, $search, $replace)
	{
		...
		for($i=0; $i<count($files); $i++)
		{
			...
			if($search !== $replace && substr($header['filename'], strlen($header['filename']) - 4) !== '.'.$tplEx)
			{
	...
	/*		echo 'filename: ', $header['filename'], '<br />';
			if($header['filename'] === './overall_header.'.$tplEx)
			{
				echo 'overall_header'.$tplEx.':<br /><hr />', nl2br(htmlspecialchars($file_str)), '<br /><hr /><br />';
			}*/
