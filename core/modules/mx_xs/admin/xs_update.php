...
* @version $Id: xs_update.php,v 1.3 2010/10/16 04:08:34 orynider Exp $
...
$template->assign_block_vars('nav_left',array('ITEM' => '&raquo; <a href="' . mx_append_sid('xs_update.'.$phpEx) . '">' . $lang['xs_check_for_updates'] . '</a>'));
...
// read template files
if(($dir = @opendir($mx_root_path. 'templates/')) !== false)
{
	...
	if(($sub_dir !== '.') && ($sub_dir !== '..') && ($sub_dir !== 'CVS'))
	{
		...
		$file = $mx_root_path . 'templates/' . $sub_dir . '/xs.cfg';
		if(@file_exists($file))
		{
			include_update($file, $sub_dir);
		}
		// check contrib/xs_{file}.txt
		$file = $mx_root_path . 'contrib/xs_' . $sub_dir . '.txt';
		if(@file_exists($file))
		{
			include_update_txt($file, $sub_dir);
		}
		$file = $mx_root_path . 'contrib/xs_' . strtolower($sub_dir) . '.txt';
		if($lower && @file_exists($file))
		{
			include_update_txt($file, $sub_dir);
		}
		// check templates/contrib/xs_{file}.txt
		$file = $mx_root_path . 'templates/contrib/xs_' . $sub_dir . '.txt';
		if(@file_exists($file))
		{
			include_update_txt($file, $sub_dir);
		}
		$file = $mx_root_path . 'templates/contrib/xs_' . strtolower($sub_dir) . '.txt';
		if($lower && @file_exists($file))
		{
			include_update_txt($file, $sub_dir);
		}
		// check templates/{file}/xs.txt
		$file = $mx_root_path . 'templates/' . $sub_dir . '/xs.txt';
		if(@file_exists($file))
		{
			include_update_txt($file, $sub_dir);
		}
...
{
	$template->set_filenames(array('body' => XS_TPL_PATH . 'update.'.$tplEx));
	$template->assign_vars(array(
		'UPDATE_URL'			=> mx_append_sid('xs_update.'.$phpEx.'?doupdate=1'),
...
$template->set_filenames(array('body' => XS_TPL_PATH . 'update2.'.$tplEx));
...
foreach($updates as $var => $item)
{
	if(isset($item['data']) && is_array($item['data']))
	{
		...
		if(!empty($item['data']['version']))
		{
			...
			{
				$count_update++;
				$u_import = (isset($item['data']['style']) && substr($item['data']['style'], 0, 7) === 'http://') ? mx_append_sid('xs_import.'.$phpEx.'?get_web=' . urlencode($item['data']['style'])) : '';
