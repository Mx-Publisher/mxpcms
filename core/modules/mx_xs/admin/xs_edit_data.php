...
* @version $Id: xs_edit_data.php,v 1.3 2010/10/16 04:08:34 orynider Exp $
...
$template->assign_block_vars('nav_left',array('ITEM' => '&raquo; <a href="' . mx_append_sid('xs_edit_data.'.$phpEx) . '">' . $lang['xs_edit_styles_data'] . '</a>'));

$lang['xs_edittpl_back_list'] = str_replace('{URL}', mx_append_sid('xs_edit_data.'.$phpEx), $lang['xs_edittpl_back_list']);
...
if(!empty($HTTP_POST_VARS['edit']) && !defined('DEMO_MODE'))
{
	...
	$lang['xs_edittpl_back_edit'] = str_replace('{URL}', mx_append_sid('xs_edit_data.'.$phpEx.'?edit='.$id), $lang['xs_edittpl_back_edit']);
	$data_item = array();
...
if(!empty($HTTP_GET_VARS['edit']))
{
	...
	$template->assign_vars(array(
		'U_ACTION'	=> mx_append_sid('xs_edit_data.'.$phpEx),
	...
	$template->set_filenames(array('body' => XS_TPL_PATH . 'edit_data.'.$tplEx));
...
$template->set_filenames(array('body' => XS_TPL_PATH . 'edit_data_list.'.$tplEx));
for($i=0; $i<count($style_rowset); $i++)
{
	...
	$template->assign_block_vars('styles', array(
		...
		'STYLE'			=> htmlspecialchars($item['style_name']),
		'U_EDIT'		=> mx_append_sid('xs_edit_data.'.$phpEx.'?edit='.$item['themes_id'])
		)
	);
}
