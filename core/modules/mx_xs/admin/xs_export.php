...
* @version $Id: xs_export.php,v 1.3 2010/10/16 04:08:34 orynider Exp $
...
$template->assign_block_vars('nav_left',array('ITEM' => '&raquo; <a href="' . mx_append_sid('xs_export.'.$phpEx) . '">' . $lang['xs_export_styles'] . '</a>'));

$lang['xs_export_back'] = str_replace('{URL}', mx_append_sid('xs_export.'.$phpEx), $lang['xs_export_back']);
...
if(!empty($export) && @file_exists($mx_root_path . $template_dir . $export . '/theme_info.cfg'))
{
	// Get list of styles
	...
	$template->set_filenames(array('body' => XS_TPL_PATH . 'export2.'.$tplEx));
	...
	$template->assign_vars(array(
			'FORM_ACTION'		=> mx_append_sid('xs_export.'.$phpEx),
...
$export = xs_tpl_name($export);
if(!empty($export) && @file_exists($mx_root_path . $template_dir . $export . '/theme_info.cfg') && !defined('DEMO_MODE'))
{
...
$template->set_filenames(array('body' => XS_TPL_PATH . 'export.'.$tplEx));
