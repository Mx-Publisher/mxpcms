...
if( ( $total_categories = count($category_rows) ) )
{
	...
	$is_auth_data = array();
	$is_auth_data = auth(AUTH_VIEW, AUTH_LIST_ALL, $userdata, $forum_data);
	...
	for($i = 0; $i < $total_categories; $i++)
	{
		...
		for($j = 0; $j < $total_forums; $j++)
		{
			...
			if ( $is_auth_data[$forum_data[$j]['forum_id']]['auth_view'] && $forum_data[$j]['cat_id'] == $cat_id )
			{
				$display_forums = true;
			}
		}
			...
			if ( $viewcat == $cat_id || $viewcat == -1 )
			{
				for($j = 0; $j < $total_forums; $j++)
				{
					if ( $forum_data[$j]['cat_id'] == $cat_id )
					{
						...
						if ($is_auth_data[$forum_id]['auth_view'])
						{
