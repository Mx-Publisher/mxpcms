 ...
 * -------------------------------------------------------------------------
 * 
 *    $Id: mx_poll.php,v 1.3 2010/10/16 04:06:36 orynider Exp $
 */
...
$auth_data_sql_poll = get_auth_forum();
...
//
	...
	// Vote in a poll
	//
	if ( !empty($HTTP_POST_VARS[$vote_id_name]) )
	{
		$vote_option_id = intval($HTTP_POST_VARS[$vote_id_name]);
		...
		mx_block_message($lang['Surveys/Polls'], $message);
		return;
	}
...
//if ( !empty($forum_topic_data['topic_vote']) )
{
	...
	if ( $vote_info = $db->sql_fetchrowset($result) )
	{
		...
		if ( ( isset($HTTP_GET_VARS['vote']) || isset($HTTP_POST_VARS['vote']) ) || ( !$userdata['session_logged_in'] ) )
		{
			$view_result = ( ( ( ( isset($HTTP_GET_VARS['vote']) ) ? $HTTP_GET_VARS['vote'] : $HTTP_POST_VARS['vote'] ) == 'viewresult' ) || ( !$userdata['session_logged_in'] ) ) ? TRUE : 0;
		}
		else
		{
			$view_result = 0;
