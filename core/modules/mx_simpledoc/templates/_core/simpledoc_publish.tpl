		...
		<table cellspacing="0" cellpadding="0" width="100%">
			...
			<tr>
			    <td id="top" class="simpledoc_title">
			        <span class="nomargin">{L_PROJECT_NAME} - {L_PUBLISH}</span>
			    </td>
			</tr>
			<tr>
			    <td id="left" class="row1">
				    ...
				    <p>
				        <form name="zip" action="{MODE_PUBLISH_EXPORT_URL}" method="get">
			   	        <input type="hidden" name="page" value="{PAGE_ID}">
			   	        <input type="hidden" name="mode" value="publish_export">

				        <table>
				        <tr>
				            <td>{L_SENDTEMPLATES}</td>
				            <td>
				                <select name="template">
				                    <option value="tree">{L_OPTIONMENU}</option>
				                    <!--<option value="drop">Drop Down Menu</option>-->
				                    <option value="raw">{L_OPTIONRAW}</option>
				                </select>
				            </td>
				        </tr>
				        <tr>
				            <td colspan="2">
				                <input type="submit" name="submit" value="{L_SENDZIP}" onclick="this.blur(); setCookie('publish-zip-template', this.form.template.selectedIndex, COOKIE_YEAR);">
				            </td>
				        </tr>
				        </form>
				        ...
				        <form name="dir" action="{MODE_PUBLISH_EXPORT_URL}" method="get" onsubmit="this.submit.disabled=true;">
				        <input type="hidden" name="publish_dir" value="1">
				        <input type="hidden" name="page" value="{PAGE_ID}">
			   	        <input type="hidden" name="mode" value="publish_export">

				        <tr>
				            <td>{L_PUBLISHTEMPLATES}</td>
				            <td>
				                <select name="template">
			                    <option value="tree">{L_OPTIONMENU}</option>
				                    <!--<option value="drop">Drop Down Menu</option>-->
				        ...
				        <tr>
				            <td>{L_OPTIONSECTION}</td>
				            <td>
				                <select name="section"><option value="">(optional)</option>{TEMPLATE_OPTIONS}</select>
				            </td>
				        </tr>
				        <tr>
				            <td>{L_PUBLISHDIR}</td>
				        ...
				        <tr>
				            <td colspan="2">
				                <input type="submit" name="submit" value="{L_PUBLISH}" onclick="this.blur(); setCookie('publish-dir-template', this.form.template.selectedIndex, COOKIE_YEAR);" <!-- IF PUBLISH_DIR_ERROR -->'disabled="disabled"'<!-- ENDIF --> >
				                {PUBLISH_DIR_ERROR}
				            </td>
				        </tr>
				        </table>
