		...
		<table cellspacing="0" cellpadding="0" width="100%">
			...
			<tr>
			    <td id="top" class="simpledoc_title">
			        <span class="nomargin">{L_PROJECT_NAME} - {L_SETTINGS}</span>
			    </td>			
			</tr>
			<tr>
			    <td id="left" class="row1">
				    ...
				    <p>
				        <form action="{MODE_SETTINGS_URL}" method="post">
				        <table>
				        <tr>
				            <td>Encoding:</td>
				            <td><input type="text" name="encoding" value="{ENCODING}" {DISABLED}></td>
				        </tr>
				        <tr>
				            <td>Editor width:</td>
				            <td><input type="text" name="editor-width" value="{EDITOR_WIDTH}" {DISABLED}></td>
				        </tr>
				        <tr>
				            <td>Editor height:</td>
				            <td><input type="text" name="editor-height" value="{EDITOR_HEIGHT}" {DISABLED}></td>
				        </tr>
				        <tr>
				            <td>Publish Dir:</td>
				            <td><input type="text" name="publish_dir" value="{PUBLISH_DIR}" {DISABLED}></td>
				        </tr>
				        <tr>
				            <td colspan="2">
				                <input type="submit" name="submit" value="{L_CHANGE_SETTINGS}" onclick="return validateForm(this.form)" {DISABLED}>
				            </td>
				        </tr>
				        </table>
				        </form>
				    ...
				    function validateForm(form) {
				        ...
				        if (!encoding.value.length) { alert("{L_ERROR_EMPTY}"); return false; }
				        if (!isNumber(editorWidth.value)) { alert("{L_ERROR_WIDTH}"); return false; }
				        if (!isNumber(editorHeight.value)) { alert("{L_ERROR_HEIGHT}"); return false; }
				        if (!publish_dir.value.length) { alert("{L_ERROR_DIR}"); return false; }
				        return true;
				    }
				    </script>
