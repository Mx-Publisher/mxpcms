        ...
        this.toHtml = function() {
            ...
            if (this.isFolder) {
                ...
                if (self.foldersAsLinks) {
                    ...
                    s += '<a style="text-decoration:none;" href="javascript:void(0)" ondblclick="?.nodeClick(\'?\')">'.format(self.id, this.id) + this.text;
                    s += '</a>';
                }
                if (this.childNodes.length) {
            ...
            if (this.isDoc) {
                ...
                s += '<a class="link" onclick="?.setActive(\'?\');this.blur();" id="?-link" href="?"??>?</a>'.format(self.id, this.id, this.id, this.href, (this.title ? ' title="?"'.format(this.title) : ""), (this.target ? ' target="?"'.format(this.target) : ""), this.text);
            }
            s += '</div>';
            return s;
        };
