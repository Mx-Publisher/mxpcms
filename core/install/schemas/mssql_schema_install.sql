CREATE TABLE mx_table_block (
    ...
    auth_moderator_group SMALLINT NOT NULL DEFAULT (0),
    show_title TINYINT NOT NULL DEFAULT (1),
    show_block TINYINT NOT NULL DEFAULT (1),
    show_stats TINYINT NOT NULL DEFAULT (0),
    block_time VARBINARY(255) NOT NULL DEFAULT (''),
...
CREATE TABLE mx_table_block_parameter (
    block_id SMALLINT NOT NULL DEFAULT (0),
    parameter_id SMALLINT NOT NULL DEFAULT (0),
    parameter_value TEXT NULL,
    parameter_opt TEXT NULL,
    sub_id INTEGER NOT NULL DEFAULT (0),
    CONSTRAINT mx_table_block_parameter_pk PRIMARY KEY (block_id, parameter_id, sub_id) ON [PRIMARY],
    CHECK (block_id>=0),
...
CREATE TABLE mx_table_column_templates (
    column_template_id SMALLINT NOT NULL IDENTITY(1, 1),
    column_title VARCHAR(100) NULL DEFAULT ('0'),
    column_order SMALLINT NOT NULL DEFAULT (0),
    column_size VARCHAR(5) NULL DEFAULT ('0'),
    page_template_id SMALLINT NOT NULL DEFAULT (0),
    CONSTRAINT mx_table_column_templates_pk PRIMARY KEY (column_template_id) ON [PRIMARY],
    CHECK (column_template_id>=0),
    CHECK (column_order>=0)
)  ON [PRIMARY];

CREATE INDEX mx_tablecolumntemplatescatorde ON mx_table_column_templates (column_order) ON [PRIMARY];
...
CREATE TABLE mx_table_menu_categories (
    ...
    cat_url SMALLINT NULL DEFAULT (0),
    cat_target TINYINT NOT NULL DEFAULT (0),
...
INSERT INTO mx_table_menu_nav VALUES("4","1","mxBB-Portal Home","","http://mxpcms.sourceforge.net","0","10","c40e66a4ae","icon_info.gif","0","0","0","0","0");
...
INSERT INTO mx_table_page VALUES("5","Site Search","Site Search page","nav_search.gif","0","0","0","overall_header.tpl","","","-1");

INSERT INTO mx_table_page VALUES("4","Sitestats","Sitestats page","nav_statistics.gif","0","0","0","overall_header.tpl","","","-1");

INSERT INTO mx_table_page VALUES("1","Home","This is the startpage","nav_home.gif","0","0","0","overall_header.tpl","","","-1");

INSERT INTO mx_table_page VALUES("2","Forum","This is the phpBB Forum startpage","nav_forum.gif","0","0","0","overall_header.tpl","","","1");

INSERT INTO mx_table_page VALUES("3","Demo Page","Block Demos","nav_develope.gif","0","0","0","overall_header.tpl","","","-1");

CREATE TABLE mx_table_page_templates (
    page_template_id SMALLINT NOT NULL IDENTITY(1, 1),
    template_name VARCHAR(255) NOT NULL DEFAULT (''),
    CONSTRAINT mx_table_page_templates_pk PRIMARY KEY (page_template_id) ON [PRIMARY],
    CHECK (page_template_id>=0)
)  ON [PRIMARY];
...
CREATE TABLE mx_table_parameter (
    ...
    parameter_order SMALLINT NOT NULL DEFAULT (0),
...
CREATE TABLE mx_table_portal (
    ...
    top_phpbb_links SMALLINT NOT NULL DEFAULT (0),
    mx_use_cache SMALLINT NOT NULL DEFAULT (1),
...
CREATE TABLE mx_table_search_results (
    ...
    CONSTRAINT mx_table_search_results_pk PRIMARY KEY (search_id) ON [PRIMARY],
    CHECK (search_id>=0)
)  ON [PRIMARY] TEXTIMAGE_ON [PRIMARY];

CREATE INDEX mx_tablesearchresultssessionid ON mx_table_search_results (session_id) ON [PRIMARY];

CREATE TABLE mx_table_wordlist (
    word_text VARBINARY(50) NOT NULL DEFAULT (''),
    word_id INTEGER NOT NULL IDENTITY(1, 1),
    word_common TINYINT NOT NULL DEFAULT (0),
    CONSTRAINT mx_table_wordlist_pk PRIMARY KEY (word_text) ON [PRIMARY],
    CHECK (word_id>=0),
    CHECK (word_common>=0)
)  ON [PRIMARY];

CREATE INDEX mx_table_wordlist_word_id ON mx_table_wordlist (word_id) ON [PRIMARY];
...
CREATE INDEX mx_table_wordmatch_block_id ON mx_table_wordmatch (block_id) ON [PRIMARY];

CREATE INDEX mx_table_wordmatch_word_id ON mx_table_wordmatch (word_id) ON [PRIMARY];
