...
function connect_check_db($error_connect, &$error, $dbms, $table_prefix, $dbhost, $dbuser, $dbpasswd, $dbname, $dbport, $prefix_may_exist = false, $load_dbal = true, $unicode_check = true)
{
	...
	if ($load_dbal)
	{
		...
		require_once($mx_root_path . 'install/db/' . $dbms . '.' . $phpEx); // Load dbal and initiate class		
	}
...
function open_phpbb_db(&$db, &$phpbb_info)
{
	...
	{
		case 'mysql':
			require_once($mx_root_path . "install/db/mysql.$phpEx");
		break;
		case 'mysql4':
			require_once($mx_root_path . "install/db/mysql4.$phpEx");
		break;
		case 'mysqli':
			require_once($mx_root_path . "install/db/mysqli.$phpEx");
		break;			
		case 'postgres':
			require_once($mx_root_path . "install/db/postgres7.$phpEx");
		break;
		default:
			require_once($mx_root_path . 'install/db/' . $dbms . '.' . $phpEx);
		break;			
	}
...
function exec_post_process($mode, $upgrade_mode)
{
	...
	global $mx_root_path, $phpbb_root_path, $portal_url, $portal_url;
...
if (!function_exists('mx_realpath'))
{
	//
