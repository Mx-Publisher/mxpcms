<?php
/** ------------------------------------------------------------------------
 *		subject				: mxBB-Portal Installation Script
 *		begin            	: june, 2002
 ...
 * -------------------------------------------------------------------------
 * 
 *    $Id: mx_install.php,v 1.4 2011/03/05 10:29:55 orynider Exp $
 */
...
// to get from there all the information required to install mxBB-Portal.
...
*/
$mx_portal_name = 'MX-Portal';
$mx_portal_version = '2.7.8-Beta';
$mx_php_version		= '4.8.0';
$mx_portal_copy 	= '<b>MX-Publisher eXtreme Modular Portal and Content Management System!</b> <br /><br/> MX-Portal or MX-Publisher (originally named MX-System) is a fully modular system, portal for phpBB2 since released in 2001, and CMS featuring dynamic pages, blocks, and themes, by means of a powerful yet flexible AdminCP. It is the classical phpBB2 portal add-on, and version 3.x is improved and enhanced for every phpBB3 version. <br /><br />Authors: The MX-Publisher Development Team. <br />Please visit <a href="http://mxpcms.sf.net/">mxpcms.sf.net/</a> for further information.';
...
*/
define('INSTALLER_VERSION', '2.0.5');
...
*/
define('U_MXBB_NEWS_INFO', 'http://lists.sourceforge.net/lists/listinfo/mxbb-news');
define('U_MXBB_NEWS_NOW', 'http://lists.sourceforge.net/lists/listinfo/mxbb-news');
...
// ================================================================================
include($mx_root_path . 'includes/mx_functions.' . $phpEx); // CORE class
...
* Send file --------------------------------------------------
*/
if (isset($HTTP_POST_VARS['send_file']) && ($mx_request_vars->post('send_file', MX_TYPE_INT) == 1))
{
...
if(empty($language))
{
	...
	{
		if( @file_exists($mx_root_path . "language/lang_$language/lang_admin.$phpEx") )
		{
...
if($confirm)
{
	...
	switch($install_mode)
	{
		case 'install':
			...
			if ($mx_request_vars->is_post('mxbb'))
			{
				...
				$phpbb_path = $mx_request_vars->post('phpbb_path', MX_TYPE_NO_TAGS, $mx_request_vars->post('backend_path', MX_TYPE_NO_TAGS, 'forum/'));				
				...
				$mx_table_prefix = $mx_request_vars->post('mx_prefix', MX_TYPE_NO_TAGS, 'mx_');
				$table_prefix = $mx_request_vars->post('prefix', MX_TYPE_NO_TAGS, 'phpbb_');				
				$dbcharacter_set = $mx_request_vars->post('dbcharacter_set_mxbb', MX_TYPE_NO_TAGS, 'utf-8');
			...
			if ($mx_request_vars->is_post('backend'))
			{
				...
				//$portal_backend = file_exists($mx_root_path . $backend_path . "modcp.$phpEx") ? 'phpbb2' : 'phpbb3';
			...
			*/
			if( !defined('MX_INSTALLED') || (MX_INSTALLED === false) )
			{
				...
				if(!@file_exists($mx_root_path . 'config.php') && @file_exists($mx_root_path . 'config.php.contrib'))
				{
		...
		case 'upgrade':
			...
			/*
			* Update config.php (if upgrading from 2.6.x)
			*/
			if( defined('MX_INSTALLED') && defined('PHPBB_INSTALLED') )
			{
				$upgrade_mode = 'from27x';
	...
	if(count($schemas) > 0)
	{
		...
		*/
		include($mx_root_path . $phpbb_path . "includes/constants.$phpEx");
		include($mx_root_path . 'install/db/' . $dbms . '.' . $phpEx); // Load dbal and initiate class
		...
		**/
		...
		{
			if( version_compare($mx_portal_version, '2.8.0', '<') )
			{
				//
				// FYI: This is how the cache worked before 2.8.0.
				//
				if( @file_exists($mx_root_path . 'cache/block_config.xml') )
				{
					include($mx_root_path . "cache/mx_cache.$phpEx");
					write_cache_config();
			...
			{
				//
				// FYI: update_session_cache() needs $portal_config to be filled.
				//
				$sql = 'SELECT * FROM '.PORTAL_TABLE.' WHERE portal_id = 1';
				if( ($result = $db->sql_query($sql)) )
				{
					$portal_config = $db->sql_fetchrow($result);
					include($mx_root_path . "includes/mx_cache.$phpEx");
...
if(defined('MX_INSTALLED') && (MX_INSTALLED === true))
{
	...
	*/
	...
	$upgrade_mode = defined('BACKEND_INSTALLED') ? 'from27x' : '';
...
if (file_exists($mx_root_path . "config.$phpEx"))
{
	...
	*/
	$template->assign_block_vars('datarow', array(
		'INFO'				=>	!empty($lang['Install_Instruction_mxBB']) ? $lang['Install_Instruction_mxBB'] : $lang['Welcome_install'],
		'BACKEND_PATH'		=>	'',
		...
		'DB_NAME'			=>	$mx_info['dbname'] ? $mx_info['dbname'] : '',
		'DB_USER'			=>	$mx_info['dbuser'] ? $mx_info['dbuser'] : '',
		'DB_PASSWD'			=>	$mx_info['dbpasswd'] ? $mx_info['dbpasswd'] : '',
		'DB_PREFIX'			=>	$mx_info['table_prefix'] ? $mx_info['table_prefix'] : '',
		'DB_CHARACTER_SET'	=>	$mx_info['dbcharacter_set'] ? $mx_info['dbcharacter_set'] : "utf8",	
		'BACKEND_ROOT'		=>	'',		
	));
}
else
{
	...
	*/
	...
	foreach ($available_dbms as $dbms => $database)
	{
		...
		if ($database['LABEL'] && defined('INSTALL_READONLY'))
		{
			...
			else
			{
				...
				$mx_info['dbms']			= $database['name'];
				$mx_info['dbhost']			= $database['default_host'];
				$mx_info['dbname']			= $database['name'];
				$mx_info['dbuser']			= $database['default_user'];
				$mx_info['dbpasswd']		= $database['default_password'];
				$mx_info['mx_table_prefix']	= $database['name'];
				$mx_info['dbcharacter_set']	= ($database['utf8_support'] = true) ? 'utf8' : 'latin1';		
				$mx_info['status']			= false;
	...
	$default_dbms = $mx_info['dbms'] ? $mx_info['dbms'] : "mysql";
	/**/	
	/*
	* First, provide the option of standalone install
	*/
	$template->assign_block_vars('datarow', array(
		...
		'DB_PASSWD'			=>	$mx_info['dbpasswd'] ? $mx_info['dbpasswd'] : '',
		'DB_PREFIX'			=>	$mx_info['table_prefix'] ? $mx_info['table_prefix'] : '',
		'DB_CHARACTER_SET'	=>	$mx_info['dbcharacter_set'] ? $mx_info['dbcharacter_set'] : "latin1",
		'BACKEND_ROOT'		=>	'',		
	));	
}
/*
...
for($i = 0; $i < $offset1; $i++)
{
	...
	*/	
	...
	$default_dbms = $phpbb_info['dbms'] ? $phpbb_info['dbms'] : "mysql4";
	...
	$template->assign_block_vars('datarow', array(
		...
		'DB_CHARACTER_SET'	=> (isset($available_dbms[$phpbb_info['dbms']]['utf8_default'])) ? "utf8" : "latin2",			
...
for($i = $offset1; $i < $offset2; $i++)
{
	...
	if (file_exists($smf_root_path . "Settings.$phpEx"))
	{
	...
	elseif (!file_exists($smf_root_path . "config.$phpEx") && !file_exists($smf_root_path . "Settings.$phpEx") && !file_exists($smf_root_path . "inc/config.$phpEx") && !file_exists($smf_root_path . "inc/settings.$phpEx"))				
	{	
...
for($i = $offset2; $i < $files_cnt; $i++)
{
	...
	if (file_exists($mybb_root_path . "inc/config.$phpEx") && file_exists($mybb_root_path . "inc/settings.$phpEx"))				
	{	
	...
	$default_dbms = $mybb_info['dbms'] ? $mybb_info['dbms'] : "mysqli";	
	// Save forums information for debuging purposes
	...
	$template->assign_block_vars('datarow', array(
		...
		'DBCHARACTER_SET'	=> (isset($available_dbms[$phpbb_info['dbms']]['utf8_default'])) ? "utf8" : "ISO-8859-2",		
...
$s_hidden_fields = '<input type="hidden" name="install_mode" value="' . $install_mode . '" />'.
	...
	'<input type="hidden" name="smf_root_path" value="' . $smf_root_path . '" />'.
	'<input type="hidden" name="mybb_root_path" value="' . $mybb_root_path . '" />';
/* -------------------- */	
$template->assign_vars(array(
	...
	'L_PHPBB_PATH'				=> $lang['Phpbb_path'],
	'L_PORTAL_URL'				=> $lang['Portal_url'],
	'L_PHPBB_URL'				=> $lang['Phpbb_url'],	
