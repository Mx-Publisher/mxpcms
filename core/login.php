...
* @version $Id: login.php,v 1.1 2009/01/21 15:45:37 orynider Exp $
			...
			else
			{
				...
				if( md5($password) == $row['user_password'] && $row['user_active'] )
				{
					...
					if( $session_id )
					{
						...
						mx_redirect(mx_append_sid($url, true));
					}
					else
					{
...
else
{
	...
	if( !$userdata['session_logged_in'] || (isset($HTTP_GET_VARS['admin']) && $userdata['session_logged_in'] && $userdata['user_level'] == ADMIN))
	{
		...
		$layouttemplate->assign_vars(array(
			...
			'U_SEND_PASSWORD' => mx_append_sid($phpbb_root_path . "profile.$phpEx?mode=sendpassword"),
