...
function emoticon(text) {
	text = ' ' + text + ' ';
	if (opener.document.forms['post'].block_info.createTextRange && opener.document.forms['post'].block_info.caretPos) {
		var caretPos = opener.document.forms['post'].block_info.caretPos;
		caretPos.text = caretPos.text.charAt(caretPos.text.length - 1) == ' ' ? text + ' ' : text;
		opener.document.forms['post'].block_info.focus();
	} else {
	opener.document.forms['post'].block_info.value  += text;
	opener.document.forms['post'].block_info.focus();
	}
}
//-->
